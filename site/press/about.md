...
Jグランツが申請ポータルである一方、jpcite は「発見 + 併用可否判定 + 実績確認 + 根拠法トレース + 判例・入札・適格事業者横断」の層を担います。Claude Desktop / ChatGPT / Cursor / Gemini などの MCP クライアントから直接呼び出せ、SDK は不要です。料金は完全従量 ¥3/req 税別 (税込 ¥3.30、匿名 3 req/日 per IP 無料・JST 翌日リセット)。運営は Bookyou株式会社。お問い合わせ: info@bookyou.net
