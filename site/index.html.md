...
Claude MCP / Custom GPT / Cursor / Codex / Anthropic API direct に 1 行で bundle。139 tools 即解禁。 

[1 行で接続 → ](/connect/claude-code.html)

### For 業界実務家 

税理士・会計士・行政書士・M&A・信金。AI agent 経由で月 ¥270〜¥1,125、取りこぼし 1 件回避時 ¥36 万〜¥2,500 万規模の節約 (historical ROI 37〜2,688 倍 表現を per-case 節約額に書換)。 
...
jpcite 側は LLM 推論を一切しない (CLAUDE.md `Never put LLM API imports …`)。agent dev の billing は自前 LLM token と完全独立で、jpcite は ¥3/unit 固定式。Claude Desktop / Cursor / Cline は MCP、ChatGPT Custom GPT は OpenAPI Actions、自前 RAG は REST から呼び出します。 
...
## 主要 metrics (2026-05-11 snapshot) 
...
Custom GPT / Anthropic MCP server / Cursor MCP / Codex / Anthropic API direct のどの surface でも、jpcite を 1 install 行で bundle すれば日本公的制度 11,601 + 法令 6,493 + 採択 2,286 + 行政処分 1,185 + 適格事業者 13,801 が evidence packet として届きます。LLM 推論は agent 側、jpcite は出典固定の retrieval 層に専念します。 

- [ChatGPT Custom GPT ](/integrations/openai-custom-gpt.html)— OpenAPI Action import (34 paths agent-safe subset) 
- [Claude Desktop / MCP ](/integrations/claude-desktop.html)— uvx autonomath-mcp 1 行で 139 tool 解禁 
- [Cursor ](/integrations/cursor.html)— .cursor/mcp.json に貼って再起動 
- [Cline / Codex ](/integrations/cline.html)— VS Code 拡張から MCP server 追加 
- [Anthropic API direct ](/docs/for-agent-devs/why-bundle-jpcite_2026_05_11.md)— REST 218 paths を自前 RAG 前段に 

## For 業界実務家 (AI agent 経由で利用) 

士業本人がブラウズする想定ではなく、Claude / GPT / Cursor 側に jpcite を bundle した AI agent を業務に組み込む 6 業種シナリオ。節約額 (純 LLM 単体運用との差) は [業種別 use case ](/docs/use_cases/by_industry_2026_05_11/) ・ [cost saving examples ](/docs/canonical/cost_saving_examples/) に詳細 (historical 表現の ROI 倍率と併記)。 

業種 月額 (税別) 節約額 / 取りこぼし 1 件回避時 (historical ROI 倍率) 

税理士事務所 (顧問先月次レビュー) ¥450 67-222 倍 

会計士事務所 (研究開発税制 / IT導入) ¥400 312 倍 

行政書士 (建設業許可 / 産廃 / 在留資格) ¥1,125 37-59 倍 

中小企業診断士 (経営診断 / 補助金) ¥270 185-555 倍 

M&A advisor (DD / 業法 fence) ¥465-¥775 2,688 倍 

信用金庫渉外 (顧客 watch / 融資前 DD) ¥690 55-549 倍 

回避コスト・sequence・envelope 仕様は [業種別 use case 詳細 ](/docs/use_cases/by_industry_2026_05_11/)・ [課金根拠 ](/docs/pricing/justification_2026_05_11/)を参照。最終判断は 8 業法の有資格者 (税理士法§52 / 弁護士法§72 / 行政書士法§1 / 司法書士法§3 / 公認会計士法§47条の2 / 社労士法§27 / 中小企業診断士登録規則 / 弁理士法§75)。 

- [会社フォルダ作成パック ](/products/company-folder): 1 社 ¥59.40 (18 req) 
- [顧問先月次レビュー ](/products/monthly-review): 100 社月 ¥5,940 (純 LLM 同 req 比 約 ¥15 万節約 / 月) 
- [BPO 1000 案件 triage ](/products/bpo-intake): 月 ¥52,800 (58% コスト削減) 
- [公開情報 DD ](/products/public-dd): 200 社月 ¥31,020 (90% コスト削減) 
- [相談前プレ診断 ](/products/pre-consult): 50 件月 ¥1,320 

匿名 curl で動作確認 (登録不要) · Playground 成果物プレビュー確認 · 自動化 API/MCP は月次レビュー・DD・BPO一括処理の実行手段 
...
利用者層 別の専用ページに、 節約額試算 + サンプル API + 既知の制約を載せています (旧称 ROI 試算)。 
...
3 回/日まで無料、 API キー不要。 1 回目で制度候補、 2 回目で Evidence Packet と事前算出バンドル、 3 回目で採算ライン到達判定を含む入力文脈削減見込みを確認します。 
...
事前計算済み Evidence Packet を返します。 LLM 呼び出しもライブの Web 検索も行われません。 呼び出し元が比較基準を指定しない限り、 削減見込みは表示されません。 
...
trace_program_to_law ・ combined_compliance_check ・ search_tax_incentives ・ related_programs など 139 MCP ツールで横断解決。 補助金から関連法令・通達・裁決事例・処分歴・登録番号を 1 コールで連結します。 

### 自前運用を減らす 

スクレイパ・正規化・差分検出・出典管理 

同等のデータ層を自前で組むには、 省庁・自治体・公庫の公開ページのスクレイパ運用、 スキーマ正規化、 法令改正の差分検出、 出典 URL の生存監視、 集約サイト誤掲載の排除を、 継続して維持する必要があります。 jpcite は ¥3/課金単位 の従量で、 開発・運用工数を肩代わりします。 
...
AI・RAG・SaaS に組み込みたい。OpenAPI 3.1、stdio 接続 (MCP)、¥3/課金単位 従量、登録不要の 3 リクエスト/日 で評価できる。 
...
成果物作成 (API/MCP入口) 3 req/日 per IP ¥ 3 / billable unit (税別) ¥ 3.30 / billable unit 
