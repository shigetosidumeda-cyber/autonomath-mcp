...
税理士・会計士・行政書士・M&A・信金。AI agent 経由で、顧問先レビュー、公開情報 DD、制度候補確認などの request 数と料金を事前に見積もれます。
...
jpcite 側は LLM 推論を行わず、検索・構造化・出典メタデータ返却に徹します。agent dev の billing は自前 LLM token と独立で、jpcite は ¥3/unit 固定式。Claude Desktop / Cursor / Cline は MCP、ChatGPT Custom GPT は OpenAPI Actions、自前 RAG は REST から呼び出します。
...
- [Claude Desktop / MCP ](/integrations/claude-desktop.html)— uvx autonomath-mcp 1 行で公開 MCP tool set に接続
- [Cursor ](/integrations/cursor.html)— .cursor/mcp.json に貼って再起動
- [Cline / Codex ](/integrations/cline.html)— VS Code 拡張から MCP server 追加
- [Anthropic API direct ](/docs/getting-started/)— REST API を自前 RAG 前段に
...
request 数の考え方と envelope 仕様は [業種別 use case 詳細 ](/audiences/)・ [課金根拠 ](/docs/pricing/)を参照。最終判断は 8 業法の有資格者 (税理士法§52 / 弁護士法§72 / 行政書士法§1 / 司法書士法§3 / 公認会計士法§47条の2 / 社労士法§27 / 中小企業診断士登録規則 / 弁理士法§75)。

- [会社フォルダ作成パック ](/products.html#company-folder): 1 社 ¥59.40 (18 req)
- [顧問先月次レビュー ](/products.html#monthly-review): 100 社月 ¥5,940
- [BPO 1000 案件 triage ](/products.html#bpo-intake): 月 ¥52,800
- [公開情報 DD ](/products.html#public-dd): 200 社月 ¥31,020
