(()=>{'use strict';const API_BASE=(typeof window!=='undefined'&&(window.AUTONOMATH_API_BASE||window.JPINTEL_API_BASE))||(typeof window!=='undefined'&&window.location&&window.location.hostname==='jpcite.com'?'https://api.jpcite.com':'');const api=(p)=>API_BASE.replace(/\/$/,'')+p;const KEY_NAME='am_api_key';const $=(sel,root=document)=>root.querySelector(sel);const escapeHtml=(s)=>String(s).replace(/[&<>"']/g,(c)=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));let storageMode='localStorage';const memStore=Object.create(null);const store={get(){try{const v=localStorage.getItem(KEY_NAME);if(v){storageMode='localStorage';return v;}}catch(_){}
