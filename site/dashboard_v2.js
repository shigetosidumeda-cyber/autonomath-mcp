...
async function loadSummary(){const data=await fetchJSON('/v1/me/dashboard?days=30');renderV2Dunning(data);renderV2PeriodEnd(data);const headline=$('#dash2-summary-headline');if(headline){headline.innerHTML=`${data.last_30_calls.toLocaleString()} <span class="unit">リクエスト / </span>`+`¥${data.last_30_amount_yen.toLocaleString()}<span class="unit"> 支払い (30日)</span>`;}
...
tbody.innerHTML=_lastInvoices.map((inv)=>`
      <tr style="border-bottom:1px solid var(--border);">
        ...
        <td style="padding:8px 4px;">${inv.hosted_invoice_url ? `<a href="${escapeHtml(inv.hosted_invoice_url)}"target="_blank"rel="noopener">表示</a>` : '—'}${inv.invoice_pdf ? `·<a href="${escapeHtml(inv.invoice_pdf)}"target="_blank"rel="noopener">PDF</a>` : ''}</td>
      </tr>
...
if(parsed.protocol!=='https:')return'webhook_url は https:// で始まる必要があります';if(!parsed.hostname)return'webhook_url にホスト名が必要です';if(_isInternalHost(parsed.hostname))return'プライベート IP / loopback は登録できません';return null;}
...
async function loadAll(){if(!store.get()){showSections(false);setStatus('APIキー 未保存 — 上記フォームから貼り付けてください。');return;}
setStatus('読み込み中…');showSections(true);try{await loadSummary();await yieldToMain();await loadToolUsage();await yieldToMain();await loadBillingHistory();await yieldToMain();try{await loadAlerts();}catch(e){if(e.status!==401){const tbody=document.getElementById('dash2-alerts-tbody');if(tbody){tbody.innerHTML='<tr><td colspan="7" style="padding:14px 4px;color:var(--danger);">'+'購読一覧の取得に失敗: '+escapeHtml(e.message||String(e))+'</td></tr>';}}else{throw e;}}
setStatus('読み込み完了。');}catch(e){if(e.status===401){store.clear();showSections(false);setStatus('APIキー が無効でした。再度入力してください。',true);}else{setStatus(`エラー: ${e.message || e}`,true);}}}
function bind(){const form=$('#dash2-key-form');if(form){form.addEventListener('submit',(e)=>{e.preventDefault();const inp=$('#dash2-key-input');const v=(inp&&inp.value||'').trim();if(!v)return;store.set(v);if(inp)inp.value='';renderStorageWarning();loadAll();});}
const clearBtn=$('#dash2-key-clear');if(clearBtn){clearBtn.addEventListener('click',()=>{store.clear();showSections(false);setStatus('APIキー を削除しました。');const warn=document.getElementById('dash2-key-storage-warning');if(warn)warn.remove();});}
...
catch(err){if(err.status===401){setStatus('APIキー が無効です。',true);}else{const ol=$('#dash2-rec-list');if(ol)ol.innerHTML=`<li style="color:var(--danger);">エラー: ${escapeHtml(err.message || String(err))}</li>`;}}});}
...
try{await saveCap(num);setStatus(num!=null?`月次予算を保存しました: ¥${num.toLocaleString()}`:'月次予算を解除しました。');await loadSummary();}catch(e){setStatus(`月次予算の保存に失敗しました: ${e.message || e}`,true);}});}
const capClear=$('#dash2-cap-clear');if(capClear){capClear.addEventListener('click',async()=>{try{await saveCap(null);const inp=$('#dash2-cap-input');if(inp)inp.value='';setStatus('月次予算を解除しました。');await loadSummary();}catch(e){setStatus(`月次予算の解除に失敗しました: ${e.message || e}`,true);}});}
const csv=$('#dash2-billing-csv');if(csv)csv.addEventListener('click',()=>downloadInvoices('csv'));const json=$('#dash2-billing-json');if(json)json.addEventListener('click',()=>downloadInvoices('json'));const alertsForm=$('#dash2-alerts-form');if(alertsForm){alertsForm.addEventListener('submit',async(e)=>{e.preventDefault();const honeypot=$('#dash2-alerts-company-url');if(honeypot&&honeypot.value){showAlertBanner('購読を登録しました。',false);return;}
...
const payload={filter_type:filterType,min_severity:severity,};if(filterType!=='all')payload.filter_value=filterValue;if(webhook)payload.webhook_url=webhook;if(email)payload.email=email;const submit=$('#dash2-alerts-submit');if(submit)submit.disabled=true;try{const created=await subscribeAlert(payload);showAlertBanner(`購読 #${created.id} を登録しました。`,false);const fv=$('#dash2-alerts-filter-value');const wb=$('#dash2-alerts-webhook');const em=$('#dash2-alerts-email');if(fv)fv.value='';if(wb)wb.value='';if(em)em.value='';await loadAlerts();}catch(err){if(err.status===401){showAlertBanner('APIキー が無効です。再度入力してください。',true);}else{showAlertBanner(`登録失敗: ${err.message || String(err)}`,true);}}finally{if(submit)submit.disabled=false;}});}
const alertsTbody=$('#dash2-alerts-tbody');if(alertsTbody){alertsTbody.addEventListener('click',async(e)=>{const target=e.target;if(!(target instanceof Element))return;const btn=target.closest('.dash2-alerts-delete');if(!btn)return;const id=btn.getAttribute('data-sub-id');if(!id)return;if(!window.confirm(`購読 #${id} を削除しますか？ 再開するには新規登録が必要です。`)){return;}
btn.disabled=true;try{await deleteAlert(id);showAlertBanner(`購読 #${id} を削除しました。`,false);await loadAlerts();}catch(err){if(err.status===404){showAlertBanner(`購読 #${id} は既に削除されています。`,true);await loadAlerts();}else if(err.status===401){showAlertBanner('APIキー が無効です。',true);}else{showAlertBanner(`削除失敗: ${err.message || String(err)}`,true);btn.disabled=false;}}});}}
