...
description: "AutonoMath ローンチ記事 (Article 2 of 2) — MCP サーバーで Claude Desktop に 13,578 件の補助金データを直接渡す。69 ツール (39 jpintel + 30 autonomath at default gates)、protocol 2025-06-18、全件 source_url 付き。"
tags:
  - mcp
  - claude
  - llm
...
ツール構成は 69 ツール (39 jpintel: 基本 + one-shot + 拡張 [法令・判例・入札・税務ruleset・適格事業者 + cross-dataset glue]、30 autonomath: entity-fact DB 503,930 entities + 6.12M facts + 23,805 relations を 税制優遇 / 認定制度 / 法令 / 処分 / 融資 / 共済 横断で検索)。
...
ハンマーアイコンの下に `autonomath` の 69 ツール (39 jpintel + 30 autonomath at default gates) が並びます。
...
## 69 ツール (39 jpintel + 30 autonomath at default gates) の構成（protocol 2025-06-18）

### 39 コア (jpintel.db: programs + case_studies + loan_programs + enforcement + expansion)
...
39 jpintel の内訳は 15 基本 + 7 one-shot + 17 拡張（法令 / 判例 / 入札 / 税務ruleset / 適格事業者 + cross-dataset glue）です。
...
メタデータエンドポイントに分離しない。
全 69 ツール (39 jpintel + 30 autonomath at default gates) の返り値、13,578 件中 99%以上に
...
- 69 ツール (39 jpintel + 30 autonomath at default gates)（protocol 2025-06-18）で検索・排他判定・融資・事例・法令・entity-fact lookup layer (503,930 entities / 6.12M facts / 23,805 relations) をカバー
