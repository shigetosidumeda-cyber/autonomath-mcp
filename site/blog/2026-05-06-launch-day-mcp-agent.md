---
title: "AutonoMath: 日本の公的制度を Claude に渡す MCP サーバー (72 ツール)"
...
# AutonoMath: 日本の公的制度を Claude に渡す MCP サーバー (72 ツール、protocol 2025-06-18)
...
ツール構成は 72 ツール (38 jpintel: 基本 + one-shot + 拡張 [法令・判例・入札・税務ruleset・適格事業者 + cross-dataset glue]、28 autonomath: entity-fact DB 503,930 entities + 6.12M facts + 23,805 relations を 税制優遇 / 認定制度 / 法令 / 処分 / 融資 / 共済 横断で検索)。
...
38 jpintel の内訳は 15 基本 + 7 one-shot + 16 拡張（法令 / 判例 / 入札 / 税務ruleset / 適格事業者 + cross-dataset glue）です。
...
FTS5 trigram + unicode61 をプライマリに、sqlite-vec の tiered index は段階的に有効化中。
...
メタデータエンドポイントに分離しない。
全 72 ツール (39 jpintel + 33 autonomath at default gates) の返り値、13,578 件中 99%以上に
