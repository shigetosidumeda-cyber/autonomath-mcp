---
title: "AutonoMath: 日本の公的制度を Claude に渡す MCP サーバー (93 ツール)"
description: "AutonoMath ローンチ記事 (Article 2 of 2) — MCP サーバーで Claude Desktop に 14,472 件の補助金データを直接渡す。93 tools at default gates、protocol 2025-06-18、99%以上 source_url 付き。"
tags:
  - mcp
  - claude
  - llm
...
# AutonoMath: 日本の公的制度を Claude に渡す MCP サーバー (93 ツール、protocol 2025-06-18)
...
ツール構成は 93 ツール : 基本 + one-shot + 拡張 [法令・判例・入札・税務ruleset・適格事業者 + cross-dataset glue]、30 autonomath: entity-fact DB 503,930 entities + 6.12M facts + 23,805 relations を 税制優遇 / 認定制度 / 法令 / 処分 / 融資 / 共済 横断で検索)。
...
!!! info "API キーは任意"
    API キーは匿名 3 req/日 per IP（JST 月初 00:00 リセット）まで不要です。
...
###  (jpintel.db: programs + case_studies + loan_programs + enforcement + expansion)
...
| インボイス・メタ | `lookup_invoice_registrant` / `get_meta` |
 ...
 の内訳は 15 基本 + 7 one-shot + 17 拡張（法令 / 判例 / 入札 / 税務ruleset / 適格事業者 + cross-dataset glue）です。

### 30 autonomath (entity-fact DB)

autonomath.db（8.29 GB, 読み取り専用）上の entity-fact lookup layer。
**503,930 entities + 6.12M facts + 23,805 relations + 335,605 aliases** を
税制優遇 / 認定制度 / 法令 / 処分 / 融資 / 共済 の 12 record_kind 横断で検索できます。
日本語の高速全文検索 (3-gram 分割 + Unicode 形態素の二重インデックス) をプライマリに、意味検索インデックスは段階的に有効化中。
...
メタデータエンドポイントに分離しない。
全 93 ツール  + 30 autonomath at default gates) の返り値、14,472 件中 99%以上に
...
- 93 ツール  + 30 autonomath at default gates)（protocol 2025-06-18）で検索・排他判定・融資・事例・法令・entity-fact lookup layer (503,930 entities / 6.12M facts / 23,805 relations) をカバー
