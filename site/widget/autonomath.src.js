...
(function (root, factory) {
  ...
  var DEFAULT_API_BASE = "https://api.autonomath.ai";
  var PROGRAM_PAGE_BASE = "https://autonomath.ai/programs/";
  ...
  var I18N = {
    ja: {
      ...
      error_quota: "月間リクエスト上限 (50 req/月) に達しました。JST 月初 00:00 にリセットされます。",
    ...
    en: {
      ...
      error_quota: "Monthly quota exceeded (50 req/month). Resets at JST first-of-month 00:00.",
  ...
  // stylesheet separately. Keep minimal; host CSS can override via
  // the prefixed .autonomath-widget-* class names.
  // -------------------------------------------------------------------------

  var STYLE_ID = "autonomath-widget-style";
  var CSS = [
    ".autonomath-widget{",
    ...
    ".autonomath-widget *{box-sizing:border-box}",
    ".autonomath-widget--dark{background:#0f172a;color:#f8fafc;border-color:#1e293b}",
    ".autonomath-widget__form{display:grid;gap:10px;margin-bottom:14px}",
    ".autonomath-widget__row{display:flex;gap:8px;flex-wrap:wrap}",
    ".autonomath-widget__field{flex:1 1 160px;display:flex;flex-direction:column;min-width:120px}",
    ".autonomath-widget__label{font-size:12px;font-weight:600;color:#374151;margin:0 0 4px}",
    ".autonomath-widget--dark .autonomath-widget__label{color:#cbd5e1}",
    ".autonomath-widget__input,.autonomath-widget__select{",
    ...
    ".autonomath-widget__submit:hover{background:#4338ca}",
    ".autonomath-widget__submit[disabled]{opacity:.55;cursor:not-allowed}",
    ".autonomath-widget__status{font-size:12px;color:#6b7280;margin:6px 0 8px;min-height:16px}",
    ".autonomath-widget--dark .autonomath-widget__status{color:#94a3b8}",
    ".autonomath-widget__error{color:#b91c1c;background:#fef2f2;border:1px solid #fecaca;",
    "padding:8px 12px;border-radius:6px;font-size:13px;margin:8px 0}",
    ".autonomath-widget--dark .autonomath-widget__error{color:#fecaca;background:#450a0a;border-color:#7f1d1d}",
    ".autonomath-widget__results{list-style:none;margin:0;padding:0;display:grid;gap:10px}",
    ".autonomath-widget__item{border:1px solid #e5e7eb;border-radius:8px;padding:12px 14px;background:#ffffff}",
    ".autonomath-widget--dark .autonomath-widget__item{background:#1e293b;border-color:#334155}",
    ".autonomath-widget__name{margin:0 0 4px;font-size:15px;font-weight:700;color:#111827}",
    ".autonomath-widget--dark .autonomath-widget__name{color:#f8fafc}",
    ".autonomath-widget__meta{margin:0 0 6px;font-size:12px;color:#6b7280;display:flex;gap:10px;flex-wrap:wrap}",
    ".autonomath-widget--dark .autonomath-widget__meta{color:#94a3b8}",
    ".autonomath-widget__tag{display:inline-block;padding:1px 8px;background:#eef2ff;color:#3730a3;",
    "border-radius:999px;font-size:11px}",
    ".autonomath-widget--dark .autonomath-widget__tag{background:#312e81;color:#c7d2fe}",
    ...
    ".autonomath-widget__footer{margin-top:10px;font-size:11px;color:#9ca3af;text-align:right}",
    ".autonomath-widget__footer a{color:inherit;text-decoration:none}",
    ".autonomath-widget__footer a:hover{text-decoration:underline}",
    ".autonomath-widget--dark .autonomath-widget__footer{color:#64748b}"
  ].join("");
  ...
  Widget.prototype._fatal = function (msg) {
    if (!this.container) {
      if (typeof console !== "undefined") console.error("[autonomath]", msg);
      return;
    }
    this.container.textContent = "";
    var box = h("div", { className: "autonomath-widget" }, [
      h("div", { className: "autonomath-widget__error" }, [msg])
    ]);
  ...
  Widget.prototype._render = function () {
    this.container.textContent = "";
    this.rootEl = h("div", {
      className: "autonomath-widget" + (this.theme === "dark" ? " autonomath-widget--dark" : ""),
      "data-autonomath-widget-mounted": "true"
    });
    var form = h("form", { className: "autonomath-widget__form", onsubmit: this._onSubmit.bind(this) });
    var row = h("div", { className: "autonomath-widget__row" });
    ...
    row.appendChild(this._renderField(
      ...
      h("input", {
        className: "autonomath-widget__input",
        type: "text",
        name: "q",
    ...
    for (var i = 0; i < this.filters.length; i++) {
      ...
      var sel = h("select", { className: "autonomath-widget__select", name: key, "aria-label": label }, [
    ...
    form.appendChild(h("div", { className: "autonomath-widget__row" }, [
      h("button", { type: "submit", className: "autonomath-widget__submit" },
        [t(this.language, "search")])
    ]));
    this.rootEl.appendChild(form);
    this.statusEl = h("div", { className: "autonomath-widget__status", role: "status", "aria-live": "polite" });
    this.rootEl.appendChild(this.statusEl);
    this.errorEl = h("div", { className: "autonomath-widget__error", style: "display:none", role: "alert" });
    this.rootEl.appendChild(this.errorEl);
    this.resultsEl = h("ul", { className: "autonomath-widget__results" });
    this.rootEl.appendChild(this.resultsEl);
    this.footerEl = h("div", { className: "autonomath-widget__footer" });
  ...
  Widget.prototype._renderField = function (name, label, input) {
    return h("div", { className: "autonomath-widget__field" }, [
      h("label", { className: "autonomath-widget__label", "for": "autonomath-f-" + name }, [label]),
      (function (inp) { inp.id = "autonomath-f-" + name; return inp; })(input)
    ]);
  };
  ...
  Widget.prototype._renderFooter = function () {
    ...
    if (this._branding) {
      var link = h("a", {
        href: "https://autonomath.ai/widget.html",
        target: "_blank",
        rel: "noopener"
  ...
  Widget.prototype._renderResults = function (data) {
    ...
    if (!results.length) {
      this.resultsEl.appendChild(h("li", { className: "autonomath-widget__item" }, [
        t(this.language, "no_results")
      ]));
      return;
    }
    // Update branding from widget envelope (server may flip it)
  ...
  Widget.prototype._renderItem = function (program) {
    ...
    var tags = [];
    if (program.prefecture) tags.push(h("span", { className: "autonomath-widget__tag" }, [program.prefecture]));
    if (program.authority_level) tags.push(h("span", { className: "autonomath-widget__tag" }, [program.authority_level]));
    if (program.program_kind) tags.push(h("span", { className: "autonomath-widget__tag" }, [program.program_kind]));

    var meta = h("div", { className: "autonomath-widget__meta" });
    if (tags.length) { for (var i = 0; i < tags.length; i++) meta.appendChild(tags[i]); }

    var amount = formatYen(program.amount_max_man_yen);
    if (amount) {
      meta.appendChild(h("span", { className: "autonomath-widget__amount" }, [
