-- Q53_allwave_grand_aggregate_top14_families.sql (Wave 98)
--
-- Full grand-aggregate — 14 wave families touched at the top level.
-- Successor to wave67/q11 (Wave 53-67), wave70/q21 (Wave 53-76 fy ×
-- jsic), wave82/q27 (Wave 53-82), wave85/q32 (Wave 53-85), wave88/q37
-- (Wave 53-88), wave91/q42 (Wave 53-91), wave94/q47 (Wave 53-94). This
-- is the **lean** grand-aggregate: 1 representative table per
-- family × 14 families × foundation = the per-family footprint
-- without the 80+ table broad sweep of q47.
--
-- Reads as the canonical "show me the 14 representative wave families
-- + foundation in one shot" surface — one row per (wave_family, src)
-- with row_count + approx_distinct (subject_id) for cardinality. Use
-- for pre-flight density check before drilling into a specific
-- family's full table set.
--
-- 14 wave families × 1 representative table each (all LIVE in Glue):
...
--   wave75 employment → packet_employment_program_eligibility_v1
--   wave76 startup → packet_business_lifecycle_stage_v1
--   wave80 supply_chain → packet_supplier_lifecycle_risk_v1
...
--
-- Scan target: ~300-1000MB (18 UNION ALL on representative tables,
-- COUNT + approx_distinct on subject.id only).
-- Expected row count: 18 (1 per representative table) + grand
-- rollup; LIMIT 1000 safety.
...
SELECT
  ...
  -- pct_of_grand_total: this row's share of the 14-family +
  -- foundation grand total — reads as "% of the union footprint
  -- that lives in this family/src".
  CASE
    WHEN gt.total_rows = 0 THEN 0.0
    ELSE CAST(g.row_count AS DOUBLE) / CAST(gt.total_rows AS DOUBLE)
  END AS pct_of_grand_total,
  -- coverage_rank: dense row rank by row_count DESC across the 18
  -- representative rows.
  ...
  -- wave_generation: ordinal bucket so downstream consumers can
  -- group by 'baseline (53)' / 'mid (60-76)' / 'modern (80-94)' /
  -- 'foundation'. Reads as wave-era proxy.
  CASE
