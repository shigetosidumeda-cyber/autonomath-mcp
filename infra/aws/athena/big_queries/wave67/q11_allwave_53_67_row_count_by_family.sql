-- q11_allwave_53_67_row_count_by_family.sql (Wave 67)
--
-- Wave 53-67 ultra-aggregate row count per (wave_family, src). Extends
-- wave60/q6_allwave_aggregation_53_62.sql to include Waves 61-67 surface.
--
-- Wave family scope (covers ~70 Glue tables registered as of 2026-05-16):
--   foundation = packet_houjin_360 / acceptance_probability / program_lineage
--   wave53     = 7 cross-source baseline packets (commonly populated)
--   wave53_3   = 4 cross-source deep packets
--   wave54     = 3 cross-source packets
--   wave55     = 10 cross-3-source analytics
--   wave56     = 10 time-series packets
--   wave57     = 10 geographic packets
--   wave58     = 10 relationship packets
--   wave60     = 12 cross-industry macro packets (subset in catalog)
--
-- This query is intentionally SELECT 1 per row + GROUP BY — Athena's
-- parquet column pruning keeps scan footprint to header-only on each
-- table, so total scan stays well below 1 GiB despite touching 60+
-- tables. Output is grouped to ~80 rows max.
...
WITH all_packets AS (
  ...
  -- wave60 (12 cross-industry macro)
