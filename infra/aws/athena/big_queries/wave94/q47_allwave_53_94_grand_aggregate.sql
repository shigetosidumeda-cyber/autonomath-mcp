-- q47_allwave_53_94_grand_aggregate.sql (Wave 92-94)
--
-- Grand aggregate row-count per wave_family across the full Wave 53
-- → 94 corpus. Successor to wave67/q11 (Wave 53-67), wave70/q21
-- (Wave 53-76 fy × jsic), wave82/q27 (Wave 53-82), wave85/q32 (Wave
-- 53-85), wave88/q37 (Wave 53-88), wave91/q42 (Wave 53-91), and the
-- Q43-Q46 family-shaped queries above. This is the canonical "show
-- me the footprint of everything through Wave 94" surface — one row
-- per (wave_family, packet_source) with row count + approx_distinct
-- (subject.id) for cardinality.
--
...
--   wave57     geographic     prefecture_program_heatmap + adjacent
...
--   wave92     product safety product_recall + product_safety_recall +
--                              ai_safety / consumer_protection / min_price (NEW)
--   wave93     real estate    retail_inbound + landslide_geotech +
--                              industry_x_prefecture_houjin (NEW)
--   wave94     insurance      ai_safety_cert + data_breach_history +
--                              cybersecurity_cert (NEW; pre-sync Glue
--                              for D&O / cyber_ins / EPLI etc.)
--
...
WITH grand AS (
  ...
  -- Wave 90 talent / workforce / leadership
  UNION ALL
  ...
  -- Wave 92 product safety / quality compliance (NEW)
  UNION ALL
  SELECT 'wave92_product_safety','product_recall_intensity', COUNT(*),
         approx_distinct(json_extract_scalar(subject, '$.id'))
  FROM jpcite_credit_2026_05.packet_product_recall_intensity_v1
  UNION ALL
  SELECT 'wave92_product_safety','product_safety_recall_intensity', COUNT(*),
         approx_distinct(json_extract_scalar(subject, '$.id'))
  FROM jpcite_credit_2026_05.packet_product_safety_recall_intensity_v1
  UNION ALL
  SELECT 'wave92_product_safety','product_lifecycle_pulse', COUNT(*),
         approx_distinct(json_extract_scalar(subject, '$.id'))
  FROM jpcite_credit_2026_05.packet_product_lifecycle_pulse_v1
  UNION ALL
  SELECT 'wave92_product_safety','product_diversification_intensity', COUNT(*),
         approx_distinct(json_extract_scalar(subject, '$.id'))
  FROM jpcite_credit_2026_05.packet_product_diversification_intensity_v1
  UNION ALL
  SELECT 'wave92_product_safety','ai_safety_certification', COUNT(*),
         approx_distinct(json_extract_scalar(subject, '$.id'))
  FROM jpcite_credit_2026_05.packet_ai_safety_certification_v1
  UNION ALL
  SELECT 'wave92_product_safety','consumer_protection_compliance', COUNT(*),
         approx_distinct(json_extract_scalar(subject, '$.id'))
  FROM jpcite_credit_2026_05.packet_consumer_protection_compliance_v1
  UNION ALL
  SELECT 'wave92_product_safety','min_price_violation_history', COUNT(*),
         approx_distinct(json_extract_scalar(subject, '$.id'))
  FROM jpcite_credit_2026_05.packet_min_price_violation_history_v1

  -- Wave 93 real estate / property (NEW; live proxies only)
  UNION ALL
  SELECT 'wave93_real_estate','retail_inbound_subsidy', COUNT(*),
         approx_distinct(json_extract_scalar(subject, '$.id'))
  FROM jpcite_credit_2026_05.packet_retail_inbound_subsidy_v1
  UNION ALL
  SELECT 'wave93_real_estate','landslide_geotechnical_risk', COUNT(*),
         approx_distinct(json_extract_scalar(subject, '$.id'))
  FROM jpcite_credit_2026_05.packet_landslide_geotechnical_risk_v1
  UNION ALL
  SELECT 'wave93_real_estate','industry_x_prefecture_houjin', COUNT(*),
         approx_distinct(json_extract_scalar(subject, '$.id'))
  FROM jpcite_credit_2026_05.packet_industry_x_prefecture_houjin_v1

  -- Wave 94 insurance / risk transfer (NEW; live proxies only)
  UNION ALL
  SELECT 'wave94_insurance','ai_safety_certification_proxy', COUNT(*),
         approx_distinct(json_extract_scalar(subject, '$.id'))
  FROM jpcite_credit_2026_05.packet_ai_safety_certification_v1
  UNION ALL
  SELECT 'wave94_insurance','data_breach_event_history_proxy', COUNT(*),
         approx_distinct(json_extract_scalar(subject, '$.id'))
  FROM jpcite_credit_2026_05.packet_data_breach_event_history_v1
  UNION ALL
  SELECT 'wave94_insurance','cybersecurity_certification_proxy', COUNT(*),
         approx_distinct(json_extract_scalar(subject, '$.id'))
  FROM jpcite_credit_2026_05.packet_cybersecurity_certification_v1
