...
def _section_header(programs: int, laws: int, cases: int, generated_at: str) -> list[str]:
    return [
        ...
        "> Operator: Bookyou Inc. (法人番号 T8010001213708) / Canonical: https://zeimu-kaikei.ai",
...
def _section_about() -> list[str]:
    return [
        ...
        "",
        "MCP exposes 89 tools at default gates (39 jpintel + 33 autonomath: 17 V1 + 4 V4 universal + 5 Phase A absorption + 7 graph/lifecycle/snapshot tools). The MCP server runs over stdio (protocol 2025-06-18) and is compatible with Claude Desktop, Cursor, and ChatGPT (Plus and above).",
        "",
...
def _section_pricing() -> list[str]:
    return [
        ...
        "",
        "Sign up at https://zeimu-kaikei.ai/pricing.html. API keys are issued once at Stripe checkout success and shown verbatim in the response - keep the key safe; the only re-issue path is to cancel the subscription via the billing portal and re-checkout.",
        "",
        "---",
        "",
    ]
...
def _section_audiences() -> list[str]:
    return [
        ...
        "5. AI agent developers - 89 MCP tools, ¥3 per request, 50 free per month. Primary tools: full surface area; see https://zeimu-kaikei.ai/docs/mcp-tools/.",
        "",
        "---",
        "",
    ]
...
def _section_footer() -> list[str]:
    return [
        ...
        "Product: AutonoMath. Canonical domain: https://zeimu-kaikei.ai. PyPI package: autonomath-mcp.",
        "",
        "Data attribution:",
        ...
        "- See https://zeimu-kaikei.ai/robots.txt for the canonical Allow / Disallow policy.",
        "- The Japanese-language sibling is at https://zeimu-kaikei.ai/llms-full.txt.",
        "- The shorter index file is at https://zeimu-kaikei.ai/llms.en.txt.",
        "",
        "No machine translation of program names, law titles, or case fields. They are Japanese legal entity names; inventing English names would be a fraud-risk vector. See https://zeimu-kaikei.ai/docs/i18n_strategy/ for the canonical bilingual policy.",
        "",
    ]
