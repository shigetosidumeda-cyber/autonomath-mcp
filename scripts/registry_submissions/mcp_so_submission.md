...
```
68 (at default gates). 4 additional tools gated off pending fix; 2 further tools held behind AUTONOMATH_36_KYOTEI_ENABLED for 社労士法 review. tools/list is the source of truth.
```
...
```
¥3.30/request (税込, fully metered via Stripe). First 50 requests/month free per IP (anonymous, JST monthly reset). No tier SKUs, no seat fees, no annual minimums, no signup required for the free tier.
```
...
```
Honest counts (verified 2026-04-29 audit, v0.3.1):
- 89 tools at default gates (4 additional gated off pending fix: query_at_snapshot, intent_of, reason_answer, related_programs)
...
Earlier strings of "89 tools" / "89 tools" / "11,547 programs" / "424,054 entities" exist in some downstream files as historical state markers and should be ignored when listing.
```
...
```
[Server submission] AutonoMath — 68-tool MCP for Japanese institutional data
```
...
- [ ] If the listing renders the wrong number of tools, re-submit with the audited 68 figure.
