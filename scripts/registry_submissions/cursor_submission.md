...
- [ ] Tested the server inside Cursor locally (Cursor → Settings → MCP → add `uvx autonomath-mcp`) and captured a screenshot of the tool list rendering 139 tools
...
```
139-tool MCP for Japanese institutional data — subsidies, laws, court, tax, invoice
```
...
```
139 at default gates. Source of truth: tools/list at runtime.
```
...
```
1. cursor-screenshot-tool-list-139.png — Cursor settings showing the 139 tools loaded under the AutonoMath server
...
- Honest tool count = 139 at default gates. Older snapshots ("55", "59", "66", "72") may appear in historical files and should be ignored.
