...
- [ ] Tested the server inside Cursor locally (Cursor → Settings → MCP → add `uvx autonomath-mcp`) and captured a screenshot of the tool list rendering 89 tools
...
```
68-tool MCP for Japanese institutional data — subsidies, laws, court, tax, invoice
```

### Short description (~160 chars)

```
Search 10,790 subsidies + 154 laws + 2,065 court decisions + 13,801 invoice registrants from Cursor. 89 MCP tools, primary-source URLs, ¥3.30/req, 50/月 free anon.
```

### Long description

```
AutonoMath exposes Japanese institutional public data via 89 MCP tools at default gates (protocol 2025-06-18, stdio). Drop it into Cursor and ask: 「東京都の農業 DX 補助金を期日順に教えて」or「houjin_bangou=… の DD プロファイルと適格事業者登録を確認して」.
...
Every row carries source_url + fetched_at; aggregator domains are banned. ¥3.30/req fully metered (税込), first 50 requests/month free per IP (anonymous, JST monthly reset), no tier SKUs.
...
```
68 at default gates (4 additional tools gated off pending fix; 2 further tools held behind AUTONOMATH_36_KYOTEI_ENABLED). Source of truth: tools/list at runtime.
```

### Pricing

```
¥3.30/request (税込, fully metered). First 50 requests/month free per IP (anonymous, JST monthly reset). No tier SKUs, no seat fees, no annual minimums, no signup required for the free tier.
```
...
```
1. cursor-screenshot-tool-list-68.png — Cursor settings showing the 89 tools loaded under the AutonoMath server
...
- Honest tool count = 68 at default gates. Older snapshots ("55", "59", "66", "72") may appear in historical files and should be ignored.
