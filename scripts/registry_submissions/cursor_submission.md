...
- [ ] Tested the server inside Cursor locally (Cursor → Settings → MCP → add `uvx autonomath-mcp`) and captured a screenshot of the tool list rendering 93 tools
...
```
93-tool MCP for Japanese institutional data — subsidies, laws, court, tax, invoice
```
...
```
93 at default gates (4 additional tools gated off pending fix; 2 further tools held behind AUTONOMATH_36_KYOTEI_ENABLED). Source of truth: tools/list at runtime.
```
...
```
1. cursor-screenshot-tool-list-93.png — Cursor settings showing the 93 tools loaded under the AutonoMath server
