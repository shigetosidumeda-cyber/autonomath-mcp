...
- [ ] Tested the server inside Cursor locally (Cursor → Settings → MCP → add `uvx autonomath-mcp`) and captured a screenshot of the tool list rendering 155 tools
...
```
155-tool MCP for Japanese institutional data — subsidies, laws, court, tax, invoice
```
...
```
1. cursor-screenshot-tool-list-139.png — Cursor settings showing the 155 tools loaded under the AutonoMath server
