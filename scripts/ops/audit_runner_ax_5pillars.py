...
def _check(name: str, passed: bool, ok_msg: str, miss_msg: str) -> ResilienceCell:
    return ResilienceCell(name=name, passed=passed,
...
def resilience_pillar() -> ResiliencePillar:
    ...
    idem_hits = (_grep_dir(SRC, r"class\s+IdempotencyCache|idempotency_key|@idempotent")
                 + _grep_dir(SCRIPTS_OPS, r"idempotency"))
    p.cells.append(_check(
        "1_idempotency",
    ...
    cb_hits = _grep_dir(SRC, r"circuit_breaker|CircuitBreaker|class\s+Circuit|HALF_OPEN|short.circuit")
    p.cells.append(_check(
        "3_circuit_breaker",
        bool(cb_hits),
    ...
    dlq_hits = (_grep_dir(SRC, r"dead_letter|DLQ|dlq_depth|dead-letter")
                + _grep_dir(SCRIPTS_CRON, r"dlq"))
    p.cells.append(_check(
        "4_dlq",
        bool(dlq_hits),
    ...
    canary_hits = (
        _grep_dir(REPO_ROOT / ".github" / "workflows",
                  r"canary|blue.green|gradual|rolling|strategy:\s*rolling", glob="**/*.yml")
        + _grep_dir(REPO_ROOT, r"strategy\s*=\s*[\"']rolling|canary_percent|max_unavailable",
                    glob="fly.toml")
    )
    p.cells.append(_check(
        "5_canary",
    ...
    failover_hits = _grep_dir(REPO_ROOT / "docs", r"failover|disaster.recovery|RPO|RTO",
                              glob="**/*.md") + _grep_dir(SCRIPTS_OPS, r"failover")
    p.cells.append(_check(
        "7_failover",
        bool(failover_hits),
    ...
    pm_base = list((REPO_ROOT / "docs" / "postmortem").glob("*.md")) if (REPO_ROOT / "docs" / "postmortem").exists() else []
    p.cells.append(_check(
    ...
    p.cells.append(_check(
        "10_sla_alert",
        _exists(sla_path) and _file_has_pattern(sla_path, r"METRICS\s*[:=]") and
        _file_has_pattern(sla_path, r"_send_telegram") and
        _file_has_pattern(sla_path, r"sla_breach_w43_3_10"),
    ...
    p.cells.append(_check(
        ...
        _exists(pm_path) and _file_has_pattern(pm_path, r"detect_incidents") and
        _file_has_pattern(pm_path, r"render_md") and
        _file_has_pattern(pm_path, r"open_draft_pr"),
    ...
    p.cells.append(_check(
        "12_backup_verify",
        _exists(bv_path) and _file_has_pattern(bv_path, r"_sha256_file") and
        _file_has_pattern(bv_path, r"_latest_r2_snapshot") and
        _file_has_pattern(bv_path, r"backup_verify_daily\.json"),
...
def _load_4pillars_audit() -> dict[str, Any]:
    ...
    if mod_name in sys.modules:
        ...
        if spec is None or spec.loader is None:
            return {"axis": "ax_4pillars", "total_score": 0.0, "max_score": 48.0,
                    "average_score": 0.0, "cell_count": 0, "pillars": {},
...
def run_audit() -> dict[str, Any]:
    ...
    avg10 = round(grand_total / 5.0, 2) if grand_max > 0 else 0.0
    verdict = "green" if avg10 >= 9.6 else ("yellow" if avg10 >= 7.2 else "red")
    return {
        "axis": "ax_5pillars",
        ...
        "verdict": verdict,
        "pillars": {
            **{k: v for k, v in four.get("pillars", {}).items()},
            "Resilience": {
                "score": resil.score,
...
def render_md(result: dict[str, Any]) -> str:
    ...
    for name, body in result["pillars"].items():
        lines.append(f"| {name} | {body['score']:.2f} / {body.get('max', 12.0):.0f} | {body.get('cells', '-')} |")
    lines.append("")
    for name, body in result["pillars"].items():
        lines += [
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--out-site-json", default=str(SITE_STATUS / "ax_5pillars.json"),
    ...
    site_json.write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    for name, body in result["pillars"].items():
        print(f"  - {name}: {body['score']:.2f}/{body.get('max', 12.0):.0f} ({body.get('cells', '-')} cells)")
    return 0
