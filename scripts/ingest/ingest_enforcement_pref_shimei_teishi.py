...
_R_DATE = re.compile(
    r"(?:R|令和)\s*(\d+)[\s.年]\s*(\d{1,2})[\s.月]\s*(\d{1,2})[\s.日]?"
)
# Heisei: H30.3.27
_H_DATE = re.compile(
    r"H\s*(\d+)[\s.]\s*(\d{1,2})[\s.]\s*(\d{1,2})[\s.]?"
)
...
def parse_aichi_pdf(body: bytes, source: Source) -> list[PrefRow]:
    ...
    for blk in blocks:
        ...
        if len(date_matches) < 2:
            continue
        start_iso = reiwa_to_iso(int(date_matches[0].group(1)),
                                 int(date_matches[0].group(2)),
                                 int(date_matches[0].group(3)))
        end_iso = reiwa_to_iso(int(date_matches[1].group(1)),
        ...
        for m in match_iter:
            ...
            if both_full_dates:
                ...
                start_iso = reiwa_to_iso(int(sm.group(1)), int(sm.group(2)), int(sm.group(3))) \
                    if "令和" in start_str else heisei_to_iso(int(sm.group(1)), int(sm.group(2)), int(sm.group(3)))
                end_iso = reiwa_to_iso(int(em.group(1)), int(em.group(2)), int(em.group(3))) \
                    if "令和" in end_str or "R" in end_str else heisei_to_iso(int(em.group(1)), int(em.group(2)), int(em.group(3)))
            else:
                start_iso = reiwa_to_iso(int(sm.group(1)), int(sm.group(2)), int(sm.group(3)))
...
def parse_shizuoka_html(body: bytes, source: Source) -> list[PrefRow]:
    ...
    cells = re.findall(
        r"<td\b[^>]*>(.*?)</td>", text, flags=re.DOTALL | re.IGNORECASE,
    )
...
def parse_gunma_html(body: bytes, source: Source) -> list[PrefRow]:
    ...
    for tr_m in re.finditer(r"<tr\b[^>]*>(.*?)</tr>", text, flags=re.DOTALL | re.IGNORECASE):
        ...
        period_cell = None
        for c in cells:
            if ("から" in c or "～" in c or "〜" in c or "~" in c) and len(find_dates_in_text(c)) >= 2:
                period_cell = c
                break
...
def load_existing_dedup(
    conn: sqlite3.Connection, prefectures: list[str],
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    ap.add_argument("--prefectures", type=str, default="",
...
def main(argv: list[str] | None = None) -> int:
        ...
        for source in sources:
            ...
            if not res.ok or not res.body:
                _LOG.warning(
                    ...
                    source.prefecture, source.url, res.status, res.skip_reason,
                )
                continue
            cat_stats["fetched"] = 1
            ...
            _LOG.info(
                ...
                source.prefecture, len(rows), source.parser,
            )
            ...
            if args.dry_run:
                for r in rows[:5]:
                    _LOG.info(
                        "DRY %s | %s | %s..%s | %s",
                        r.prefecture, r.target_name, r.period_start,
                        r.period_end, (r.reason_summary or "")[:80],
                    )
                continue
    ...
    if not args.dry_run and pending:
        ...
        for write_attempt in range(6):
            ...
            try:
                ...
                _LOG.warning("write lock contention attempt=%d wait=%ds err=%s",
    ...
    _LOG.info(
        ...
        total_built, total_inserted, total_skipped_dup, len(sources),
    )
    ...
    if args.log_file is not None:
        ...
        for k, v in stats.items():
            pref = k.split("|", 1)[0]
            agg = per_pref.setdefault(pref, {"sources": 0, "built": 0, "inserted": 0, "skipped_dup": 0})
            agg["sources"] += 1
            agg["built"] += v["built"]
