...
PREF_SLUG_MAP: dict[str, str] = {
    ...
    "福島県": "fukushima", "いわき市": "iwaki", "郡山市": "koriyama",
    "茨城県": "ibaraki",
    "栃木県": "tochigi", "宇都宮市": "utsunomiya",
    "群馬県": "gunma", "高崎市": "takasaki", "前橋市": "maebashi",
    "埼玉県": "saitama", "川越市": "kawagoe", "さいたま市": "saitamashi",
    "越谷市": "koshigaya", "川口市": "kawaguchi",
    "千葉県": "chiba", "千葉市": "chibashi", "船橋市": "funabashi",
    "柏市": "kashiwa",
    "東京都": "tokyo", "東京都区部": "tokyo23",
    "八王子市": "hachioji", "町田市": "machida",
    "神奈川県": "kanagawa", "横浜市": "yokohama", "川崎市": "kawasaki",
    "相模原市": "sagamihara", "横須賀市": "yokosuka", "藤沢市": "fujisawa",
    "茅ヶ崎市": "chigasaki",
    "新潟県": "niigata", "新潟市": "niigatashi",
    "富山県": "toyama", "富山市": "toyamashi",
    "石川県": "ishikawa", "金沢市": "kanazawa",
    "福井県": "fukui",
    "山梨県": "yamanashi", "甲府市": "kofu",
    "長野県": "nagano", "長野市": "naganoshi", "松本市": "matsumoto",
    "岐阜県": "gifu", "岐阜市": "gifushi",
    "静岡県": "shizuoka", "静岡市": "shizuokashi", "浜松市": "hamamatsu",
    "愛知県": "aichi", "名古屋市": "nagoya", "豊田市": "toyota",
    "豊橋市": "toyohashi", "岡崎市": "okazaki",
    "三重県": "mie",
    "滋賀県": "shiga", "大津市": "otsu",
    "京都府": "kyoto", "京都市": "kyotoshi",
    "大阪府": "osaka", "大阪市": "osakashi", "堺市": "sakai",
    "東大阪市": "higashiosaka", "高槻市": "takatsuki",
    "兵庫県": "hyogo", "神戸市": "kobe", "姫路市": "himeji",
    "尼崎市": "amagasaki", "西宮市": "nishinomiya", "明石市": "akashi",
    "奈良県": "nara", "奈良市": "narashi",
    "和歌山県": "wakayama", "和歌山市": "wakayamashi",
    "鳥取県": "tottori",
    "島根県": "shimane", "松江市": "matsue",
    "岡山県": "okayama", "岡山市": "okayamashi", "倉敷市": "kurashiki",
    "広島県": "hiroshima", "広島市": "hiroshimashi", "福山市": "fukuyama",
    "呉市": "kure",
    "山口県": "yamaguchi", "下関市": "shimonoseki",
    "徳島県": "tokushima",
    "香川県": "kagawa", "高松市": "takamatsu",
    "愛媛県": "ehime", "松山市": "matsuyama",
    "高知県": "kochi", "高知市": "kochishi",
    "福岡県": "fukuoka", "福岡市": "fukuokashi", "北九州市": "kitakyushu",
    "久留米市": "kurume",
    "佐賀県": "saga",
    "長崎県": "nagasaki", "長崎市": "nagasakishi", "佐世保市": "sasebo",
    "熊本県": "kumamoto", "熊本市": "kumamotoshi",
    "大分県": "oita", "大分市": "oitashi",
    "宮崎県": "miyazaki", "宮崎市": "miyazakishi",
    "鹿児島県": "kagoshima", "鹿児島市": "kagoshimashi",
    ...
    try:
        ...
        for r in rows:
            ...
            try:
                ...
                inserted += 1
                by_auth[r.issuing_authority] = (
                    by_auth.get(r.issuing_authority, 0) + 1
                )
            except sqlite3.Error as exc:
                _LOG.error(
                    "DB insert err pref=%s name=%s date=%s: %s",
                    r.issuing_authority, r.target_name, r.issuance_date, exc,
                )
                continue
        conn.commit()
    except sqlite3.Error as exc:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    ap.add_argument("--max-rows", type=int, default=250,
                    help="cap inserts at this number (default 250)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        for r in rows[:5]:
            _LOG.info(
                ...
                r.target_name, r.reason_summary[:100],
            )
        http.close()
        return 0
    ...
    inserted, dup_db, dup_batch, by_auth = write_rows(
        conn, rows, now_iso=now_iso, max_rows=args.max_rows,
    )
    try:
        conn.close()
    except sqlite3.Error:
