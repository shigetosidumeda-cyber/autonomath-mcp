...
CURATED_URLS: list[tuple[str, str]] = [
    ...
    ("徳島県",
     "https://www.pref.tokushima.lg.jp/kenseijoho/hodoteikyoshiryo/5051178/"),
    # Toyama (Nichi-Iko)
    ...
    # Osaka City
    ("大阪市",
     "https://www.city.osaka.lg.jp/hodoshiryo/kenko/0000657071.html"),
    ("大阪市",
     "https://www.city.osaka.lg.jp/hodoshiryo/kenko/0000638562.html"),
    ("大阪市",
     "https://www.city.osaka.lg.jp/hodoshiryo/kenko/0000672785.html"),
    # Hiroshima City
    ("広島市", "https://www.city.hiroshima.lg.jp/houdou/houdou/371332.html"),
    # Tokyo Metropolitan Government (older but still 一次資料)
    ("東京都",
     "https://www.metro.tokyo.lg.jp/tosei/hodohappyo/press/2018/04/09/03.html"),
    ("東京都",
     "https://www.metro.tokyo.lg.jp/tosei/hodohappyo/press/2018/03/08/09.html"),
    ("東京都",
     "https://www.metro.tokyo.lg.jp/tosei/hodohappyo/press/2017/04/12/09.html"),
    ("東京都",
     "https://www.metro.tokyo.lg.jp/tosei/hodohappyo/press/2017/03/28/11.html"),
    ("東京都",
     "https://www.metro.tokyo.lg.jp/tosei/hodohappyo/press/2018/11/01/17.html"),
    # Hyogo
    ("兵庫県", "https://web.pref.hyogo.lg.jp/press/20220328_9915.html"),
    # Osaka Pref
    ("大阪府",
     "https://www.pref.osaka.lg.jp/hodo/fumin/o100100/prs_49735.html"),
    ("大阪府",
     "https://www.pref.osaka.lg.jp/hodo/fumin/o100100/prs_50463.html"),
    ...
    ("石川県",
     "https://www.pref.ishikawa.lg.jp/kisya/r4/documents/0902yakuzi.pdf"),
    # Saitama (タキザワ漢方廠 2024-01-25)
    ("埼玉県",
     "https://www.pref.saitama.lg.jp/a0707/news/page/news2024012502.html"),
    # Fukui (薬務関連処分一覧)
...
KIND_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ("license_revoke", re.compile(r"許可(取消|の取消|取り消し)|承認(取消|取り消し)|登録(取消|取り消し)")),
    ("business_improvement", re.compile(r"業務改善命令|改善命令|改善措置命令|業務改善")),
    ("contract_suspend", re.compile(r"業務(の)?停止(命令)?|営業停止|販売停止|販売自粛|業務一部停止")),
...
def find_companies_in_text(text: str) -> list[str]:
    ...
    for m in _YAKKYOKU_RE.finditer(text):
        ...
        if (2 <= len(cand) <= 80 and cand not in seen
                and "薬局" in cand and _is_valid_company_name(cand)):
            seen.add(cand)
            out.append(cand)
    return out
...
def parse_hcrisis_archive(html: str, archive_url: str) -> list[EnfRow]:
    ...
    if not (PHARMA_LAW_RE.search(title_norm + body_norm)
    ...
    pref_action_marks = [
        f"{pref}が", f"{pref}は", f"{pref}より", f"{pref}知事",
        f"{pref} 知事", f"{pref}市長", f"{pref}保健",
        f"{pref}健康", f"{pref}庁", f"{pref}を",
    ]
    has_pref_action = any(mk in body_norm for mk in pref_action_marks)
    has_central_only = (
        ("厚生労働大臣" in body_norm or "厚労省より" in body_norm
         or "厚生労働省より" in body_norm or "厚生労働省は" in body_norm
    ...
    authority = pref
    for sub in ("薬務課", "薬事課", "薬務薬事課", "医薬安全課",
                "健康安全部", "健康局", "生活衛生課"):
        if sub in body_norm:
...
def parse_direct_text(
    text: str, *, source_url: str, default_pref: str,
) -> list[EnfRow]:
    ...
    return parse_direct_html(
        fake_html, source_url=source_url, default_pref=default_pref,
    )


def parse_direct_html(
    html: str, *, source_url: str, default_pref: str,
) -> list[EnfRow]:
    ...
    for attempt in range(6):
        try:
            ...
            for r in rows:
                ...
                canonical_id = (
                    f"enforcement:pref-yakumu-{date_compact}-{slug}"
                )
                primary_name = (
                    f"{r.target_name} ({r.issuance_date}) - 薬機法処分 / "
...
def harvest(
    http: HttpClient, *, limit_urls: int | None = None,
) -> list[EnfRow]:
    out: list[EnfRow] = []
    ...
    for i, (au, _title) in enumerate(archive_pairs, 1):
        ...
        out.extend(rows)
        if i % 25 == 0:
            _LOG.info("h-crisis progress %d/%d rows=%d",
                      i, len(archive_pairs), len(out))
        ...
        try:
            if is_pdf:
                ...
                rows = parse_direct_text(
                    text, source_url=url, default_pref=pref,
                )
            else:
                rows = parse_direct_html(
                    res.text, source_url=url, default_pref=pref,
                )
        except Exception as exc:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    ap.add_argument(
        "--limit-urls", type=int, default=None,
    ...
    if args.dry_run:
        for r in rows[:8]:
            _LOG.info(
                ...
                r.enforcement_kind, r.reason_summary[:80],
            )
        http.close()
