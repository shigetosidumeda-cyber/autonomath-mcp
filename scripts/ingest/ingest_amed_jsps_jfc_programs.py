...
class ProgramSeed:
    ...
    def unified_id(self) -> str:
        h = hashlib.sha256(f"{self.source_url}|{self.primary_name}".encode()).hexdigest()[:10]
        return f"UNI-ext-{h}"
...
def upsert_seed(conn: sqlite3.Connection, seed: ProgramSeed, *, http_status: int | None) -> str:
    ...
    checksum = hashlib.sha256(f"{uid}|{seed.primary_name}|{seed.source_url}".encode()).hexdigest()[
        :16
    ]
            ...
            for seed in seeds:
                ...
                if not args.no_verify_urls:
                    ...
                    if status == 0 or status >= 500:
                        ...
                        log.warning(
                            "URL dead %s for %s — still ingest, mark tier=B",
                            status,
                            seed.source_url,
                        )
...
def main() -> int:
    ...
    p.add_argument(
        "--no-verify-urls", action="store_true", help="Skip HEAD checks (faster local test)"
    )
