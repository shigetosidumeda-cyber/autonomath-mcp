#!/usr/bin/env bash
# =============================================================================
# scripts/verify/deep-31_verify.sh
# =============================================================================
# Minimal per-spec verify shell for DEEP-31.
#
# Fires the DEEP-31 slice of the acceptance suite
# (tests/test_acceptance_criteria.py) via pytest -k "DEEP-31".
#
# Exit code semantics (consumed by scripts/cron/aggregate_production_gate_status.py):
#   0       = every acceptance row for DEEP-31 passed -> dashboard RESOLVED
...
# -k pattern matches pytest ids like DEEP-31-1-file_existence, DEEP-31-2-..., etc.
exec $PYTEST_BIN tests/test_acceptance_criteria.py \
  -k "DEEP-31-" \
  --no-header -q --tb=line \
  -o cache_dir=/tmp/.pytest_cache_deep_31
