#!/usr/bin/env bash
# =============================================================================
# scripts/verify/deep-42_verify.sh
# =============================================================================
# Minimal per-spec verify shell for DEEP-42.
#
# Fires the DEEP-42 slice of the acceptance suite
# (tests/test_acceptance_criteria.py) via pytest -k "DEEP-42".
#
# Exit code semantics (consumed by scripts/cron/aggregate_production_gate_status.py):
#   0       = every acceptance row for DEEP-42 passed -> dashboard RESOLVED
...
# -k pattern matches pytest ids like DEEP-42-1-file_existence, DEEP-42-2-..., etc.
exec $PYTEST_BIN tests/test_acceptance_criteria.py \
  -k "DEEP-42-" \
  --no-header -q --tb=line \
  -o cache_dir=/tmp/.pytest_cache_deep_42
