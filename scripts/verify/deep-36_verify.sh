#!/usr/bin/env bash
# =============================================================================
# scripts/verify/deep-36_verify.sh
# =============================================================================
# Minimal per-spec verify shell for DEEP-36.
#
# Fires the DEEP-36 slice of the acceptance suite
# (tests/test_acceptance_criteria.py) via pytest -k "DEEP-36".
#
# Exit code semantics (consumed by scripts/cron/aggregate_production_gate_status.py):
#   0       = every acceptance row for DEEP-36 passed -> dashboard RESOLVED
...
# -k pattern matches pytest ids like DEEP-36-1-file_existence, DEEP-36-2-..., etc.
exec $PYTEST_BIN tests/test_acceptance_criteria.py \
  -k "DEEP-36-" \
  --no-header -q --tb=line \
  -o cache_dir=/tmp/.pytest_cache_deep_36
