#!/usr/bin/env bash
# =============================================================================
# scripts/verify/deep-48_verify.sh
# =============================================================================
# Minimal per-spec verify shell for DEEP-48.
#
# Fires the DEEP-48 slice of the acceptance suite
# (tests/test_acceptance_criteria.py) via pytest -k "DEEP-48".
#
# Exit code semantics (consumed by scripts/cron/aggregate_production_gate_status.py):
#   0       = every acceptance row for DEEP-48 passed -> dashboard RESOLVED
...
# -k pattern matches pytest ids like DEEP-48-1-file_existence, DEEP-48-2-..., etc.
exec $PYTEST_BIN tests/test_acceptance_criteria.py \
  -k "DEEP-48-" \
  --no-header -q --tb=line \
  -o cache_dir=/tmp/.pytest_cache_deep_48
