#!/usr/bin/env bash
# =============================================================================
# scripts/verify/deep-39_verify.sh
# =============================================================================
# Minimal per-spec verify shell for DEEP-39.
#
# Fires the DEEP-39 slice of the acceptance suite
# (tests/test_acceptance_criteria.py) via pytest -k "DEEP-39".
#
# Exit code semantics (consumed by scripts/cron/aggregate_production_gate_status.py):
#   0       = every acceptance row for DEEP-39 passed -> dashboard RESOLVED
...
# -k pattern matches pytest ids like DEEP-39-1-file_existence, DEEP-39-2-..., etc.
exec $PYTEST_BIN tests/test_acceptance_criteria.py \
  -k "DEEP-39-" \
  --no-header -q --tb=line \
  -o cache_dir=/tmp/.pytest_cache_deep_39
