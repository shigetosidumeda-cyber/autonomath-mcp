#!/usr/bin/env python3
"""Generate ``prefecture_industry_court_overlay_v1`` packets (Wave 70 #5 of 10).

都道府県 × 業種 × houjin に court_decisions 観測 (法令裁判例) overlay を
houjin universal key で記録。 adoption observation を持つ houjin に
overlay として 該 prefecture+industry に該当する court 件数を集計。

Cohort
------
::

    cohort = (houjin_bangou, prefecture, industry_jsic_medium, court_overlay_n)
"""
...
PACKAGE_KIND: Final[str] = "prefecture_industry_court_overlay_v1"

DEFAULT_DISCLAIMER: Final[str] = (
    "本 prefecture × industry × houjin に court_decisions overlay を重ねた "
    "descriptive packet で、判例の適用性判断は弁護士 一次確認が前提 "
    "(弁護士法 §72)。"
)
    ...
    known_gaps: list[dict[str, str]] = [
        {
            ...
            "description": "court overlay は地域+業種ごとの判例件数集計のみ、判断は弁護士確認",
        }
    ]
    if rows_in_packet == 0:
        known_gaps.append(
            {
                ...
                "description": "該 houjin × prefecture × industry の観測無し",
            }
        )
