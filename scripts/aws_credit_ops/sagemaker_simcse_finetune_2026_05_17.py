#!/usr/bin/env python3
"""Lane M5 — Submit SageMaker training job for jpcite SimCSE BERT fine-tune.

This driver wraps ``aws sagemaker create-training-job`` with the
HuggingFace PyTorch training image, packaging
``simcse_train_entry.py`` (+ requirements) as a ``source_dir``
tarball uploaded to S3 and referenced via the canonical HuggingFace
hyperparameter triple (``sagemaker_program`` / ``sagemaker_submit_directory``
...
- Training instance ml.g4dn.12xlarge × 1 × MaxRuntime=12h ≈ $46 hard cap.
...
#: ap-northeast-1 HuggingFace PyTorch GPU training image (2.1.0 / 4.36.0 / py310 / cu121).
#: NOTE: The ap-northeast-1 mirror only carries the ``cu121`` build of the
#: 2.1.0-transformers4.36.0 line; the ``cu118`` tag from the upstream README
#: 404s in this region (confirmed via ``aws ecr describe-images``).
...
def _build_source_tar(entry_file: Path, requirements: Path | None) -> bytes:
    """Pack source_dir tar with entry script + requirements.txt."""
...
def submit(
    *,
    ...
    spec: dict[str, Any] = {
        ...
        "HyperParameters": {
            # HuggingFace estimator convention.
            "sagemaker_program": "simcse_train_entry.py",
            "sagemaker_submit_directory": source_uri,
            "sagemaker_container_log_level": "20",
            "sagemaker_region": region,
            # Training hyperparams (strings; entrypoint argparse casts).
            "epochs": str(epochs),
        ...
        "Tags": [
            {"Key": "lane", "Value": "solo"},
            {"Key": "wave", "Value": "M5"},
            {"Key": "purpose", "Value": "jpcite-bert-v1-simcse-finetune"},
        ],
    }
    if dry_run:
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Submit jpcite SimCSE BERT fine-tune training job.")
    ...
    p.add_argument(
        "--job-name",
        default=f"jpcite-bert-simcse-finetune-{dt.datetime.now(dt.UTC).strftime('%Y%m%dT%H%M%SZ')}",
    )
    p.add_argument("--train-uri", default=f"s3://{DEFAULT_BUCKET}/finetune_corpus/train.jsonl")
    p.add_argument("--val-uri", default=f"s3://{DEFAULT_BUCKET}/finetune_corpus/val.jsonl")
    p.add_argument("--output-prefix", default="models/jpcite-bert-v1")
    p.add_argument("--source-prefix", default="finetune_corpus/source")
    p.add_argument(
        "--entry-file",
    ...
    p.add_argument("--max-runtime", type=int, default=12 * 3600)
