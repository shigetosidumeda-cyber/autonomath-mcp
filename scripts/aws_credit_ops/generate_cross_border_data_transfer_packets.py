#!/usr/bin/env python3
"""Generate ``cross_border_data_transfer_v1`` packets (Wave 64 #9 of 10).

業種 (JSIC major) ごとに 越境データ移転 compliance 関連制度を集約し、
descriptive sectoral cross-border data transfer compliance density proxy
として packet 化する。個人情報保護法 第28条 / 越境移転制限 + 同意取得
方法判断は個人情報保護委員会 + 弁護士の一次確認が前提。
...
PACKAGE_KIND: Final[str] = "cross_border_data_transfer_v1"
PER_AXIS_RECORD_CAP: Final[int] = 12

SECTOR_KEYWORDS: Final[tuple[str, ...]] = (
    "越境", "越境移転", "クラウド", "データ越境", "国外移転", "海外移転",
    "データガバナンス", "データ保護", "DPA", "クロスボーダー",
    "SCC", "BCR",
)

DEFAULT_DISCLAIMER: Final[str] = (
    "本 cross-border data transfer compliance packet は jpi_programs name + "
    "越境/データ越境 keyword 検索による descriptive 指標です。個情保護法第28"
    "条 / 越境移転制限 + 同意取得方法判断は個人情報保護委員会 + 弁護士の"
    "一次確認が前提 (弁護士法 §72)。"
)
    ...
    *,
    ...
    for emitted, (jsic_code, jsic_name) in enumerate(industries):
        ...
        record = {
            ...
            "cross_border_data_programs": matches,
...
def _render(row: dict[str, Any], generated_at: str) -> tuple[str, dict[str, Any], int]:
    ...
    matched = list(row.get("cross_border_data_programs", []))
        ...
        known_gaps.append(
            {
                ...
                "description": (
                    "該 jsic_major で 越境データ移転 keyword + 行政処分密度 観測無し"
                ),
            }
        )
    ...
    sources: list[dict[str, Any]] = [
        {
            ...
            "publisher": "個人情報保護委員会 個情保護法第28条",
            "license": "gov_standard",
        },
        {
            "source_url": "https://www.ppc.go.jp/personalinfo/internationalmove/",
            "source_fetched_at": None,
            "publisher": "個人情報保護委員会 越境移転",
            "license": "gov_standard",
        },
    ]
    body: dict[str, Any] = {
        ...
        "jsic_major": jsic_code,
        "jsic_name_ja": jsic_name,
        "cross_border_data_programs": matched,
        "enforcement_distribution_n": enforcement_n,
    ...
    envelope = jpcir_envelope(
        ...
        metrics={
            "cross_border_data_program_count": len(matched),
            "enforcement_distribution_n": enforcement_n,
            "candidate_pool_size": int(row.get("candidate_pool_size") or 0),
        },
        body=body,
