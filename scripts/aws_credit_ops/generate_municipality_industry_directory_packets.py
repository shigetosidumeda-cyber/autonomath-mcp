#!/usr/bin/env python3
"""Generate ``municipality_industry_directory_v1`` packets (Wave 70 #4 of 10).

政令市 (municipality) × 業種 × houjin directory を houjin universal key で
記録。 ``jpi_adoption_records.municipality`` の入力がある行のみ抽出。
...
PACKAGE_KIND: Final[str] = "municipality_industry_directory_v1"

DEFAULT_DISCLAIMER: Final[str] = (
    "本 municipality × industry × houjin directory packet は "
    "jpi_adoption_records.municipality がある行のみで構成した descriptive "
    "directory で、自治体所在地と業種帰属の確認は M&A advisor / 公認会計士 "
    "一次確認が前提 (税理士法 §52)。"
)
    ...
    known_gaps: list[dict[str, str]] = [
        {
            ...
            "description": "municipality × industry directory は所在地統計、与信判断には未対応",
        }
    ]
    if rows_in_packet == 0:
        known_gaps.append(
