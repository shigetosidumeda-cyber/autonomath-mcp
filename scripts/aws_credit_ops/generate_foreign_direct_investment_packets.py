#!/usr/bin/env python3
"""Generate ``foreign_direct_investment_v1`` packets (Wave 64 #1 of 10).

国 (am_tax_treaty) ごとに 外資参入 + 業種規制 関連制度を集約し、
descriptive cross-border FDI entry intensity proxy として packet 化する。
外為法 / 業種別外資規制判断は所管官庁 + 弁護士の一次確認が前提。
...
PACKAGE_KIND: Final[str] = "foreign_direct_investment_v1"
PER_AXIS_RECORD_CAP: Final[int] = 10

PROGRAM_KEYWORDS: Final[tuple[str, ...]] = (
    "外資", "海外", "外国", "対日投資", "インバウンド投資", "FDI",
    "外国法人", "海外進出", "国際展開", "進出支援", "JETRO",
)

DEFAULT_DISCLAIMER: Final[str] = (
    "本 foreign direct investment packet は am_tax_treaty 条約国 × jpi_programs "
    "name + 外資 keyword 検索による descriptive 指標です。外為法 / 業種別 "
    "外資規制 / 事前届出判断は所管官庁 (経済産業省 / 財務省 / 業所管庁) + "
    "弁護士の一次確認が前提 (弁護士法 §72)。"
)
...
def _render(row: dict[str, Any], generated_at: str) -> tuple[str, dict[str, Any], int]:
    ...
    matched = list(row.get("fdi_programs", []))
    rows_in_packet = len(matched) + 1  # treaty country row counts itself

    known_gaps: list[dict[str, str]] = [
        {
            "code": "professional_review_required",
            "description": (
                "外為法 / 業種別 外資規制 / 事前届出判断は所管官庁 + 弁護士の"
                "一次確認が前提"
            ),
        }
    ]
        ...
        known_gaps.append(
            {
                ...
                "description": "公開 jpi_programs に 外資 keyword 制度 未観測",
            }
        )

    sources: list[dict[str, Any]] = [
        {
            "source_url": "https://www.jetro.go.jp/invest/",
            "source_fetched_at": None,
            "publisher": "JETRO 対日投資・外資導入支援",
