#!/usr/bin/env python3
"""Generate ``construction_public_works_v1`` packets (Wave 62 #5 of 10).

業種 (JSIC major) ごとに 建設業 × 公共工事入札 + 業法認可 を集約し、
descriptive sectoral construction public works intensity proxy として packet 化する。
建設業法 / 入札制度判断は所管官庁 + 行政書士の一次確認が前提。
...
PACKAGE_KIND: Final[str] = "construction_public_works_v1"
PER_AXIS_RECORD_CAP: Final[int] = 12

SECTOR_KEYWORDS: Final[tuple[str, ...]] = (
    "建設", "建築", "土木", "公共工事", "入札", "施工", "下請", "工事",
    "公共調達", "業法認可", "技能", "改修",
)

DEFAULT_DISCLAIMER: Final[str] = (
    "本 construction × public works packet は jpi_programs name + 建設 keyword "
    "検索 + jpi_pc_enforcement_industry_distribution から業種別 建設業 × 公共工事入札 "
    "+ 業法認可 制度密度を集計した descriptive 指標です。建設業法 / 入札制度判断は "
    "所管官庁 + 行政書士の一次確認が前提 (行政書士法 §1)。"
)
...
def _render(row: dict[str, Any], generated_at: str) -> tuple[str, dict[str, Any], int]:
    ...
    matched = list(row.get("construction_programs", []))
        ...
        known_gaps.append(
            {
                ...
                "description": (
                    "該 jsic_major で construction keyword + 行政処分密度 観測無し"
                ),
            }
        )
    ...
    sources: list[dict[str, Any]] = [
        ...
        {
            "source_url": "https://www.kkj.njss.info/",
            "source_fetched_at": None,
            "publisher": "公共工事 入札情報サービス",
            "license": "gov_standard",
        },
    ]
    body: dict[str, Any] = {
    ...
    envelope = jpcir_envelope(
        ...
        metrics={
            "construction_program_count": len(matched),
            "enforcement_distribution_n": enforcement_n,
            "candidate_pool_size": int(row.get("candidate_pool_size") or 0),
        },
        body=body,
