#!/usr/bin/env python3
"""Generate ``municipality_industry_cluster_v1`` packets (Wave 70 #10 of 10).

政令市 × 業種 cluster + houjin sample を houjin universal key で記録。
``jpi_adoption_records`` の municipality を地名キーとして 業種 cohort cluster
の代表 houjin を保持。
...
PACKAGE_KIND: Final[str] = "municipality_industry_cluster_v1"

DEFAULT_DISCLAIMER: Final[str] = (
    "本 municipality × industry × houjin cluster packet は "
    "jpi_adoption_records を地名 + 業種で cluster 化した descriptive sample "
    "で、産業集積判断は自治体担当者・経済学者 一次確認が前提。"
)
    ...
    known_gaps: list[dict[str, str]] = [
        {
            ...
            "description": "industry cluster proxy は集計、行政・産業判断は要 advisor 確認",
        }
    ]
    if rows_in_packet == 0:
        known_gaps.append(
            {
                ...
                "description": "該 houjin × municipality × industry の observation 無し",
            }
        )
