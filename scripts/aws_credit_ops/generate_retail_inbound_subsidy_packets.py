#!/usr/bin/env python3
"""Generate ``retail_inbound_subsidy_v1`` packets (Wave 62 #7 of 10).

業種 (JSIC major) ごとに 小売業 × インバウンド対応補助 を集約し、
descriptive sectoral retail × inbound subsidy intensity proxy として packet 化する。
インバウンド対応補助判断は所管官庁 + 行政書士の一次確認が前提。
...
PACKAGE_KIND: Final[str] = "retail_inbound_subsidy_v1"
PER_AXIS_RECORD_CAP: Final[int] = 12

SECTOR_KEYWORDS: Final[tuple[str, ...]] = (
    "小売", "卸売", "商業", "店舗", "百貨店", "インバウンド", "観光",
    "免税", "多言語", "キャッシュレス", "JNTO", "免税店",
)

DEFAULT_DISCLAIMER: Final[str] = (
    "本 retail × inbound subsidy packet は jpi_programs name + 小売 / インバウンド "
    "keyword 検索 + jpi_pc_enforcement_industry_distribution から業種別 小売業 × "
    "インバウンド対応補助 制度密度を集計した descriptive 指標です。"
    "インバウンド対応補助判断は所管官庁 + 行政書士の一次確認が前提 (行政書士法 §1)。"
)
...
def _render(row: dict[str, Any], generated_at: str) -> tuple[str, dict[str, Any], int]:
    ...
    matched = list(row.get("retail_programs", []))
        ...
        known_gaps.append(
            {
                ...
                "description": (
                    "該 jsic_major で retail keyword + 行政処分密度 観測無し"
                ),
            }
        )
    ...
    sources: list[dict[str, Any]] = [
        ...
        {
            "source_url": "https://www.meti.go.jp/policy/economy/distribution/",
            "source_fetched_at": None,
            "publisher": "経済産業省 流通・物流政策",
            "license": "gov_standard",
        },
    ]
    body: dict[str, Any] = {
    ...
    envelope = jpcir_envelope(
        ...
        metrics={
            "retail_program_count": len(matched),
            "enforcement_distribution_n": enforcement_n,
            "candidate_pool_size": int(row.get("candidate_pool_size") or 0),
        },
        body=body,
