#!/usr/bin/env python3
"""Generate ``regional_industry_export_intensity_v1`` packets (Wave 70 #9 of 10).

地域 × 業種 × houjin に輸出 intensity overlay を houjin universal key で
記録。 経産省 ジェトロ 公表の地域別 export ratio を descriptive overlay。

Cohort
------
::

    cohort = (houjin_bangou, prefecture, industry_jsic_medium, export_intensity_proxy)
"""
...
PACKAGE_KIND: Final[str] = "regional_industry_export_intensity_v1"

DEFAULT_DISCLAIMER: Final[str] = (
    "本 regional × industry × houjin export intensity overlay packet は "
    "経産省 + ジェトロ 公開資料を descriptive に集計した proxy で、"
    "貿易戦略判断は弁護士・貿易 advisor 一次確認が前提 (関税法 §76 等)。"
)
...
def _render(row: dict[str, Any], generated_at: str) -> tuple[str, dict[str, Any], int]:
    ...
    known_gaps: list[dict[str, str]] = [
        {
            ...
            "description": "export intensity proxy は公開資料集計、戦略判断は要 advisor 確認",
        }
    ]
    if rows_in_packet == 0:
        known_gaps.append(
    ...
    sources: list[dict[str, Any]] = [
        ...
        {
            "source_url": "https://www.jetro.go.jp/world/statistics/",
            "source_fetched_at": None,
            "publisher": "JETRO 統計",
