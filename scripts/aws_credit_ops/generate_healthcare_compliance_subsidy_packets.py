#!/usr/bin/env python3
"""Generate ``healthcare_compliance_subsidy_v1`` packets (Wave 62 #1 of 10).

業種 (JSIC major) ごとに 医療法人 × 厚労省補助 × 行政処分 を集約し、
descriptive sectoral healthcare compliance + subsidy overlay として packet 化する。
医療法 / 厚労省告示判断は所管官庁 + 医療法務専門家確認が前提。
...
PACKAGE_KIND: Final[str] = "healthcare_compliance_subsidy_v1"
PER_AXIS_RECORD_CAP: Final[int] = 12

SECTOR_KEYWORDS: Final[tuple[str, ...]] = (
    "医療", "病院", "診療", "看護", "介護", "歯科", "薬局", "MHLW",
    "厚生労働", "保険医", "在宅医療",
)

DEFAULT_DISCLAIMER: Final[str] = (
    "本 healthcare compliance + subsidy packet は jpi_programs name + 医療 keyword "
    "検索 + jpi_pc_enforcement_industry_distribution から業種別 医療法人 × 厚労省補助 "
    "× 行政処分 proxy を集計した descriptive 指標です。医療法 / 厚労省告示判断は "
    "所管官庁 + 医療法務専門家 (弁護士) の一次確認が前提 (行政書士法 §1 / 医療法 §7)。"
)
...
def _render(row: dict[str, Any], generated_at: str) -> tuple[str, dict[str, Any], int]:
    ...
    matched = list(row.get("healthcare_programs", []))
    ...
    known_gaps: list[dict[str, str]] = [
        {
            ...
            "description": (
                "医療法 / 厚労省告示判断は所管官庁 + 医療法務専門家の一次確認が前提"
            ),
        }
    ]
    if rows_in_packet == 0:
        known_gaps.append(
            {
                ...
                "description": (
                    "該 jsic_major で healthcare keyword + 行政処分密度 観測無し"
                ),
            }
        )
    ...
    envelope = jpcir_envelope(
        ...
        metrics={
            "healthcare_program_count": len(matched),
            "enforcement_distribution_n": enforcement_n,
            "candidate_pool_size": int(row.get("candidate_pool_size") or 0),
        },
        body=body,
