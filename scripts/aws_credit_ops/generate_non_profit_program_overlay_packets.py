#!/usr/bin/env python3
"""Generate ``non_profit_program_overlay_v1`` packets (Wave 62 #10 of 10).

業種 (JSIC major) ごとに NPO法人 × 認定NPO + 助成金 overlay を集約し、
descriptive sectoral non-profit program overlay proxy として packet 化する。
特定非営利活動促進法 / 認定NPO法人制度判断は所管官庁 + 行政書士確認が前提。
...
PACKAGE_KIND: Final[str] = "non_profit_program_overlay_v1"
PER_AXIS_RECORD_CAP: Final[int] = 12

SECTOR_KEYWORDS: Final[tuple[str, ...]] = (
    "NPO", "非営利", "公益", "社会福祉", "認定NPO", "ボランティア",
    "地域貢献", "社会的事業", "ソーシャル", "市民活動", "公益法人",
    "一般社団",
)

DEFAULT_DISCLAIMER: Final[str] = (
    "本 non-profit program overlay packet は jpi_programs name + NPO / 非営利 keyword "
    "検索 + jpi_pc_enforcement_industry_distribution から業種別 NPO法人 × 認定NPO "
    "+ 助成金 overlay 制度密度を集計した descriptive 指標です。特定非営利活動促進法 / "
    "認定NPO法人制度判断は所管官庁 + 行政書士の一次確認が前提 (行政書士法 §1)。"
)
...
def _render(row: dict[str, Any], generated_at: str) -> tuple[str, dict[str, Any], int]:
    ...
    matched = list(row.get("non_profit_programs", []))
    ...
    known_gaps: list[dict[str, str]] = [
        {
            ...
            "description": (
                "特定非営利活動促進法 / 認定NPO法人制度判断は所管官庁 + 行政書士の一次確認が前提"
            ),
        }
    ]
    if rows_in_packet == 0:
        known_gaps.append(
            {
                ...
                "description": (
                    "該 jsic_major で non_profit keyword + 行政処分密度 観測無し"
                ),
            }
        )
    ...
    sources: list[dict[str, Any]] = [
        {
            "source_url": "https://www.npo-homepage.go.jp/",
            "source_fetched_at": None,
            "publisher": "内閣府 NPOホームページ",
            "license": "gov_standard",
        },
        {
            "source_url": "https://www.nta.go.jp/taxes/tetsuzuki/shinsei/annai/houjin/ninteinpo/",
            "source_fetched_at": None,
            "publisher": "国税庁 認定NPO法人",
            "license": "gov_standard",
        },
    ]
    body: dict[str, Any] = {
    ...
    envelope = jpcir_envelope(
        ...
        metrics={
            "non_profit_program_count": len(matched),
            "enforcement_distribution_n": enforcement_n,
            "candidate_pool_size": int(row.get("candidate_pool_size") or 0),
        },
        body=body,
