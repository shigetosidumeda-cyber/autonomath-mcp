#!/usr/bin/env python3
"""Generate ``prefecture_industry_inbound_v1`` packets (Wave 70 #8 of 10).

都道府県 × 業種 × houjin に インバウンド 観測 overlay を houjin universal
key で記録。 prefecture 観光局 / JNTO の publicly available 受入数を地域 ×
業種 cohort に descriptive overlay。

Cohort
------
::

    cohort = (houjin_bangou, prefecture, industry_jsic_medium, inbound_overlay)
"""
...
PACKAGE_KIND: Final[str] = "prefecture_industry_inbound_v1"

DEFAULT_DISCLAIMER: Final[str] = (
    "本 prefecture × industry × houjin インバウンド overlay packet は "
    "公開 JNTO + 都道府県観光統計を descriptive に重ねた集計 proxy で、"
    "需要予測・営業判断は M&A advisor / 業界 analyst 一次確認が前提。"
)
...
def _render(row: dict[str, Any], generated_at: str) -> tuple[str, dict[str, Any], int]:
    ...
    known_gaps: list[dict[str, str]] = [
        {
            ...
            "description": "inbound overlay は地域 descriptive proxy、需要予測判断は要 review",
        }
    ]
    if rows_in_packet == 0:
        known_gaps.append(
            {
                ...
                "description": "該 houjin × prefecture × industry の observation 無し",
            }
        )
    ...
    sources: list[dict[str, Any]] = [
        ...
        {
            "source_url": "https://www.jnto.go.jp/jpn/statistics/",
            "source_fetched_at": None,
            "publisher": "JNTO 統計",
