#!/usr/bin/env python3
"""Generate ``finance_fintech_regulation_v1`` packets (Wave 62 #9 of 10).

業種 (JSIC major) ごとに 金融業 × FSA 認可 + Fintech 補助 を集約し、
descriptive sectoral finance × fintech regulation intensity proxy として packet 化する。
銀行法 / 資金決済法 / 金商法判断は所管官庁 + 弁護士の一次確認が前提。
...
PACKAGE_KIND: Final[str] = "finance_fintech_regulation_v1"
PER_AXIS_RECORD_CAP: Final[int] = 12

SECTOR_KEYWORDS: Final[tuple[str, ...]] = (
    "金融", "銀行", "保険", "証券", "Fintech", "FSA", "金融庁", "資金決済",
    "電子マネー", "暗号資産", "資産運用", "信用",
)

DEFAULT_DISCLAIMER: Final[str] = (
    "本 finance × fintech regulation packet は jpi_programs name + 金融 keyword "
    "検索 + jpi_pc_enforcement_industry_distribution から業種別 金融業 × FSA 認可 "
    "+ Fintech 補助 制度密度を集計した descriptive 指標です。銀行法 / 資金決済法 / "
    "金商法判断は所管官庁 + 弁護士の一次確認が前提 (弁護士法 §72)。"
)
...
def _render(row: dict[str, Any], generated_at: str) -> tuple[str, dict[str, Any], int]:
    ...
    matched = list(row.get("finance_programs", []))
        ...
        known_gaps.append(
            {
                ...
                "description": (
                    "該 jsic_major で finance keyword + 行政処分密度 観測無し"
                ),
            }
        )
    ...
    sources: list[dict[str, Any]] = [
        ...
        {
            "source_url": "https://www.fsa.go.jp/policy/fintech/",
            "source_fetched_at": None,
            "publisher": "金融庁 Fintech 関連政策",
            "license": "gov_standard",
        },
    ]
    body: dict[str, Any] = {
    ...
    envelope = jpcir_envelope(
        ...
        metrics={
            "finance_program_count": len(matched),
            "enforcement_distribution_n": enforcement_n,
            "candidate_pool_size": int(row.get("candidate_pool_size") or 0),
        },
        body=body,
