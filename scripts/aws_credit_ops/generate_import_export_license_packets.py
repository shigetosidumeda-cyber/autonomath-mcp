#!/usr/bin/env python3
"""Generate ``import_export_license_v1`` packets (Wave 64 #3 of 10).

業種 (JSIC major) ごとに 輸出入許可 + 経産省 認可 関連制度を集約し、
descriptive sectoral import/export license density proxy として packet 化する。
外為法 / 関税法 / 輸出貿易管理令判断は所管官庁 + 弁護士の一次確認が前提。
...
PACKAGE_KIND: Final[str] = "import_export_license_v1"
PER_AXIS_RECORD_CAP: Final[int] = 12

SECTOR_KEYWORDS: Final[tuple[str, ...]] = (
    "輸出", "輸入", "通関", "貿易", "関税", "経産省", "経済産業",
    "保税", "輸出許可", "輸出管理", "EPA", "FTA", "貿易管理",
)

DEFAULT_DISCLAIMER: Final[str] = (
    "本 import/export license packet は jpi_programs name + 輸出入 keyword "
    "検索 + jpi_pc_enforcement_industry_distribution から業種別 輸出入許可 + "
    "経産省 認可 制度密度を集計した descriptive 指標です。外為法 / 関税法 / "
    "輸出貿易管理令判断は所管官庁 (経済産業省 / 税関) + 弁護士の一次確認が"
    "前提 (弁護士法 §72)。"
)
    ...
    *,
    ...
    for emitted, (jsic_code, jsic_name) in enumerate(industries):
        ...
        record = {
            ...
            "import_export_programs": matches,
...
def _render(row: dict[str, Any], generated_at: str) -> tuple[str, dict[str, Any], int]:
    ...
    matched = list(row.get("import_export_programs", []))
        ...
        known_gaps.append(
            {
                ...
                "description": (
                    "該 jsic_major で import/export keyword + 行政処分密度 観測無し"
                ),
            }
        )
    ...
    body: dict[str, Any] = {
        ...
        "jsic_major": jsic_code,
        "jsic_name_ja": jsic_name,
        "import_export_programs": matched,
        "enforcement_distribution_n": enforcement_n,
    ...
    envelope = jpcir_envelope(
        ...
        metrics={
            "import_export_program_count": len(matched),
            "enforcement_distribution_n": enforcement_n,
            "candidate_pool_size": int(row.get("candidate_pool_size") or 0),
        },
        body=body,
