#!/usr/bin/env python3
"""Generate ``eu_gdpr_overlap_v1`` packets (Wave 64 #5 of 10).

業種 (JSIC major) ごとに EU GDPR + 個人情報保護法 overlap 関連制度を集約し、
descriptive sectoral EU GDPR × Japanese PIPA overlap density proxy として
packet 化する。GDPR / 個人情報保護法判断は所管官庁 + 弁護士の一次確認が前提。
...
PACKAGE_KIND: Final[str] = "eu_gdpr_overlap_v1"
PER_AXIS_RECORD_CAP: Final[int] = 12

SECTOR_KEYWORDS: Final[tuple[str, ...]] = (
    "GDPR", "個人情報保護", "プライバシー", "データ保護", "PPC",
    "個人情報", "PII", "個情委", "個人情報保護委員会", "越境移転",
    "十分性認定", "EU",
)

DEFAULT_DISCLAIMER: Final[str] = (
    "本 EU GDPR × 個人情報保護法 overlap packet は jpi_programs name + GDPR/"
    "個情保護 keyword 検索による descriptive 指標です。GDPR / 個人情報保護法"
    "適用判断 + 十分性認定運用 + 越境移転制限判断は個人情報保護委員会 + "
    "EU 当局 + 弁護士の一次確認が前提 (弁護士法 §72)。"
)
...
def _render(row: dict[str, Any], generated_at: str) -> tuple[str, dict[str, Any], int]:
    ...
    matched = list(row.get("gdpr_overlap_programs", []))
        ...
        known_gaps.append(
            {
                ...
                "description": (
                    "該 jsic_major で GDPR / 個情保護 keyword + 行政処分密度 観測無し"
                ),
            }
        )
    ...
    envelope = jpcir_envelope(
        ...
        metrics={
            "gdpr_overlap_program_count": len(matched),
            "enforcement_distribution_n": enforcement_n,
            "candidate_pool_size": int(row.get("candidate_pool_size") or 0),
        },
        body=body,
