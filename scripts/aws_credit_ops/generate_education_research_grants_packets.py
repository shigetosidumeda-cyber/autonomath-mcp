#!/usr/bin/env python3
"""Generate ``education_research_grants_v1`` packets (Wave 62 #8 of 10).

業種 (JSIC major) ごとに 教育法人 × 文科省 + JSPS grant を集約し、
descriptive sectoral education × research grant intensity proxy として packet 化する。
学校教育法 / 私立学校法判断は所管官庁 + 行政書士の一次確認が前提。
...
PACKAGE_KIND: Final[str] = "education_research_grants_v1"
PER_AXIS_RECORD_CAP: Final[int] = 12

SECTOR_KEYWORDS: Final[tuple[str, ...]] = (
    "教育", "学校", "大学", "研究", "学術", "JSPS", "MEXT", "文科",
    "科学研究", "学術振興", "私立学校", "高等教育",
)

DEFAULT_DISCLAIMER: Final[str] = (
    "本 education × research grant packet は jpi_programs name + 教育 / 研究 keyword "
    "検索 + jpi_pc_enforcement_industry_distribution から業種別 教育法人 × 文科省 "
    "+ JSPS grant 制度密度を集計した descriptive 指標です。学校教育法 / "
    "私立学校法判断は所管官庁 + 行政書士の一次確認が前提 (行政書士法 §1)。"
)
...
def _render(row: dict[str, Any], generated_at: str) -> tuple[str, dict[str, Any], int]:
    ...
    matched = list(row.get("education_programs", []))
        ...
        known_gaps.append(
            {
                ...
                "description": (
                    "該 jsic_major で education keyword + 行政処分密度 観測無し"
                ),
            }
        )
    ...
    sources: list[dict[str, Any]] = [
        {
            ...
            "publisher": "文部科学省 MEXT",
            "license": "gov_standard",
        },
        {
            "source_url": "https://www.jsps.go.jp/",
            "source_fetched_at": None,
            "publisher": "日本学術振興会 JSPS",
            "license": "gov_standard",
        },
    ]
    body: dict[str, Any] = {
    ...
    envelope = jpcir_envelope(
        ...
        metrics={
            "education_program_count": len(matched),
            "enforcement_distribution_n": enforcement_n,
            "candidate_pool_size": int(row.get("candidate_pool_size") or 0),
        },
        body=body,
