#!/usr/bin/env python3
"""Generate ``wto_subsidy_compliance_v1`` packets (Wave 64 #4 of 10).

業種 (JSIC major) ごとに WTO 補助金 (ASCM) compliance keyword 該当制度を
集約し、descriptive sectoral WTO subsidy compliance density proxy として
packet 化する。 ASCM (補助金及び相殺措置に関する協定) 適合性判断は所管
官庁 (経産省 / 外務省) + 国際通商弁護士の一次確認が前提。
...
PACKAGE_KIND: Final[str] = "wto_subsidy_compliance_v1"
PER_AXIS_RECORD_CAP: Final[int] = 12

SECTOR_KEYWORDS: Final[tuple[str, ...]] = (
    "輸出補助金", "WTO", "ASCM", "通商", "国際", "貿易摩擦", "対外",
    "国際展開", "海外展開", "輸出促進", "産業競争力", "国家戦略",
)

DEFAULT_DISCLAIMER: Final[str] = (
    "本 WTO subsidy compliance packet は jpi_programs name + 輸出補助金/WTO/"
    "ASCM keyword 検索による descriptive 指標です。ASCM (補助金及び相殺措置に"
    "関する協定) 違反リスク判断は所管官庁 (経産省 / 外務省) + 国際通商弁護士"
    "の一次確認が前提 (弁護士法 §72)。"
)
    ...
    *,
    ...
    for emitted, (jsic_code, jsic_name) in enumerate(industries):
        ...
        record = {
            ...
            "wto_compliance_programs": matches,
...
def _render(row: dict[str, Any], generated_at: str) -> tuple[str, dict[str, Any], int]:
    ...
    matched = list(row.get("wto_compliance_programs", []))
    ...
    known_gaps: list[dict[str, str]] = [
        {
            ...
            "description": (
                "ASCM 違反リスク判断は所管官庁 + 国際通商弁護士の一次確認が前提"
            ),
        }
    ]
    if rows_in_packet == 0:
        known_gaps.append(
            {
                ...
                "description": (
                    "該 jsic_major で WTO 関連 keyword + 行政処分密度 観測無し"
                ),
            }
        )
    ...
    sources: list[dict[str, Any]] = [
        {
            "source_url": "https://www.meti.go.jp/policy/trade_policy/wto_agreements/",
            "source_fetched_at": None,
            "publisher": "経済産業省 WTO 協定",
    ...
    body: dict[str, Any] = {
        ...
        "jsic_major": jsic_code,
        "jsic_name_ja": jsic_name,
        "wto_compliance_programs": matched,
        "enforcement_distribution_n": enforcement_n,
    ...
    envelope = jpcir_envelope(
        ...
        metrics={
            "wto_compliance_program_count": len(matched),
            "enforcement_distribution_n": enforcement_n,
            "candidate_pool_size": int(row.get("candidate_pool_size") or 0),
        },
        body=body,
