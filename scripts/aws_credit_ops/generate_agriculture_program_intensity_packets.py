#!/usr/bin/env python3
"""Generate ``agriculture_program_intensity_v1`` packets (Wave 62 #2 of 10).

業種 (JSIC major) ごとに 農業法人 × MAFF + 公庫 + 共済 制度を集約し、
descriptive sectoral agriculture program intensity proxy として packet 化する。
農地法 / 食料・農業・農村基本法判断は所管官庁 + 行政書士確認が前提。
...
PACKAGE_KIND: Final[str] = "agriculture_program_intensity_v1"
PER_AXIS_RECORD_CAP: Final[int] = 12

SECTOR_KEYWORDS: Final[tuple[str, ...]] = (
    "農業", "農林水産", "MAFF", "畜産", "漁業", "林業", "農家", "農協",
    "農地", "農産物", "公庫農林", "農林漁業",
)

DEFAULT_DISCLAIMER: Final[str] = (
    "本 agriculture program intensity packet は jpi_programs name + 農業 keyword "
    "検索 + jpi_pc_enforcement_industry_distribution から業種別 農業法人 × MAFF "
    "+ 日本政策金融公庫 + 共済 制度密度を集計した descriptive 指標です。農地法 / "
    "食料・農業・農村基本法判断は所管官庁 + 行政書士の一次確認が前提 "
    "(行政書士法 §1)。"
)
...
def _render(row: dict[str, Any], generated_at: str) -> tuple[str, dict[str, Any], int]:
    ...
    matched = list(row.get("agriculture_programs", []))
    ...
    known_gaps: list[dict[str, str]] = [
        {
            ...
            "description": (
                "農地法 / 食料・農業・農村基本法判断は所管官庁 + 行政書士の一次確認が前提"
            ),
        }
    ]
    if rows_in_packet == 0:
        known_gaps.append(
            {
                ...
                "description": (
                    "該 jsic_major で agriculture keyword + 行政処分密度 観測無し"
                ),
            }
        )
    ...
    sources: list[dict[str, Any]] = [
        {
            ...
            "publisher": "農林水産省 MAFF",
            "license": "gov_standard",
        },
        {
            "source_url": "https://www.jfc.go.jp/n/finance/sougou/",
            "source_fetched_at": None,
            "publisher": "日本政策金融公庫 農林水産事業",
            "license": "gov_standard",
        },
    ]
    body: dict[str, Any] = {
    ...
    envelope = jpcir_envelope(
        ...
        metrics={
            "agriculture_program_count": len(matched),
            "enforcement_distribution_n": enforcement_n,
            "candidate_pool_size": int(row.get("candidate_pool_size") or 0),
        },
        body=body,
