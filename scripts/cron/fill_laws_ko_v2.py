#!/usr/bin/env python3
"""Wave 41 Agent I — extended `am_law.body_ko` source discovery.

Base implementation Wave 35 Axis 5d (`fill_laws_ko.py`); v2 mirrors the
zh_v2 strategy:

  1. Authoritative KO index discovery — MOFA Korea hub, JETRO kr/,
     MOFA韓国課 mirror, JNTO Korean.
  2. Manual review queue export to `data/laws_ko_review_queue.csv`.

NEVER calls an LLM API. 一次資料 only. Catch-rate is intentionally small
(~10-50 laws); the operator queue is the auditable path.
"""
...
LOG = logging.getLogger("fill_laws_ko_v2")
DEFAULT_DB = os.environ.get("AUTONOMATH_DB_PATH", str(_REPO / "autonomath.db"))
DEFAULT_REVIEW_CSV = str(_REPO / "data" / "laws_ko_review_queue.csv")

PRIMARY_DOMAINS = (
    "japaneselawtranslation.go.jp", "mofa.go.jp",
    "jetro.go.jp", "jnto.go.jp", "mhlw.go.jp", "mlit.go.jp",
    "meti.go.jp", "moj.go.jp", "soumu.go.jp",
)
...
KO_HUB_URLS = (
    ...
    "https://www.jetro.go.jp/kr/",
    "https://www.jnto.go.jp/kr/",
)
...
def _ensure_tables(conn: sqlite3.Connection) -> None:
    for slug in ("240_law_en_full", "243_law_ko"):
...
def _harvest_ko_hubs() -> dict[str, str]:
    found: dict[str, str] = {}
    for hub_url in KO_HUB_URLS:
        ...
        for m in re.finditer(r'href="([^"#?]+)"', body):
            ...
            if seg and len(seg) >= 4 and seg not in found:
                found[seg] = href
    LOG.info("KO hub harvest: %d slug candidates", len(found))
    return found


def _ko_mirror_urls(jp_url: str, canonical_id: str,
                    ko_hub_map: dict[str, str] | None = None) -> list[str]:
    candidates: list[str] = []
    if jp_url:
        parsed = urllib.parse.urlparse(jp_url)
        candidates.append(urllib.parse.urlunparse(parsed._replace(path="/ko" + parsed.path)))
        candidates.append(urllib.parse.urlunparse(parsed._replace(path="/korean" + parsed.path)))
    q = urllib.parse.quote(canonical_id)
    candidates.append(f"https://www.japaneselawtranslation.go.jp/ko/laws/?search={q}")
    candidates.append(f"https://www.jetro.go.jp/kr/{q}.html")
    if ko_hub_map is not None:
        slug = canonical_id.lower()
        if slug in ko_hub_map:
            candidates.insert(0, ko_hub_map[slug])
    seen: set[str] = set()
    uniq: list[str] = []
    for c in candidates:
...
def _candidate_laws(conn: sqlite3.Connection, max_laws: int | None) -> list[sqlite3.Row]:
    sql = "SELECT canonical_id, canonical_name AS name, body_ko FROM am_law WHERE body_ko IS NULL"
...
def _audit_counts(conn: sqlite3.Connection) -> dict[str, int]:
    try:
        ...
        present = conn.execute(
            "SELECT COUNT(*) FROM am_law WHERE body_ko IS NOT NULL"
        ).fetchone()[0]
    except sqlite3.Error:
        return {"laws_total": 0, "body_ko_present": 0}
    return {"laws_total": int(total), "body_ko_present": int(present)}
...
def _process_law(conn: sqlite3.Connection, law: sqlite3.Row, *, dry_run: bool,
    ...
    for url in _ko_mirror_urls("", canonical_id, ko_hub_map):
        ...
        if status == 200 and body:
            ...
            if not dry_run:
                conn.execute(
                    "UPDATE am_law SET body_ko = ?, body_ko_source_url = ?, "
                    "body_ko_fetched_at = ?, body_ko_license = 'gov_public' "
                    "WHERE canonical_id = ?",
...
def run(db_path: str, *, max_laws: int | None, dry_run: bool,
    ...
    ko_hub_map: dict[str, str] | None = None
    if network:
        ko_hub_map = _harvest_ko_hubs()
    audit_before = _audit_counts(conn)
    if not dry_run:
        try:
            conn.execute(
                "INSERT INTO am_law_translation_refresh_log "
                "(refresh_id, target_lang, started_at, mode) VALUES (?, 'ko', ?, ?)",
    ...
    if review_rows and not dry_run:
        _write_review_csv(review_rows, review_csv, "ko")
    ...
    return {
        ...
        "ko_hub_size": len(ko_hub_map) if ko_hub_map is not None else None,
