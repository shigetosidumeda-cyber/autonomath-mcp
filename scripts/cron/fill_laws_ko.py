#!/usr/bin/env python3
"""Wave 35 Axis 5d — fill am_law.body_ko from ministry official 한국어 pages.
...
def _ko_mirror_urls(jp_url: str, canonical_id: str) -> list[str]:
    ...
    if jp_url:
        ...
        candidates.append(
            urllib.parse.urlunparse(parsed._replace(path="/ko" + parsed.path))
        )
        candidates.append(
            urllib.parse.urlunparse(parsed._replace(path="/korean" + parsed.path))
        )
...
def _process_law(conn: sqlite3.Connection, law: sqlite3.Row, *, dry_run: bool,
    ...
    if not network:
        primary_urls = [
            u for u in _ko_mirror_urls("", canonical_id)
