...
from official English ministry sub-paths.

NEVER calls an LLM API. Aggregator URLs refused (noukaweb / hojyokin-portal /
biz.stayway / jgrants-portal). 一次資料 only.
"""
...
PRIMARY_DOMAINS = (
    ...
    "nichigin.go.jp", "jstage.jst.go.jp", "jetro.go.jp",
)
BANNED_DOMAINS = (
...
def _english_mirror_urls(source_url: str) -> list[str]:
    ...
    if not parsed.netloc.startswith("en."):
        candidates.append(
            urllib.parse.urlunparse(parsed._replace(netloc="en." + parsed.netloc))
        )
    seen: set[str] = set()
    uniq: list[str] = []
    for c in candidates:
...
def _candidate_programs(
    conn: sqlite3.Connection, max_programs: int | None
) -> list[sqlite3.Row]:
    try:
        sql = (
            "SELECT unified_id, primary_name, source_url, tier, "
            "       translation_status FROM programs "
...
def _process_program(
    ...
    counters = {"filled": 0, "review_added": 0,
                "skipped_no_english_page": 0, "refused_aggregator": 0}
    ...
    if not dry_run:
        conn.execute(
            "UPDATE programs SET title_en = ?, summary_en = ?, "
            "source_url_en = ?, translation_fetched_at = ?, "
            "translation_status = 'full' WHERE unified_id = ?",
...
def main(argv: list[str] | None = None) -> int:
    ...
    totals = run(args.jpintel_db, max_programs=args.max_programs,
