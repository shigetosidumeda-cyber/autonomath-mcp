#!/usr/bin/env python3
"""Wave 35 Axis 5a — fill am_law.body_en / am_law_article.body_en from
e-Gov Japanese Law Translation (japaneselawtranslation.go.jp, CC-BY 4.0).
...
Invocation:
  python scripts/cron/fill_laws_en_full.py --autonomath-db <path>
                                           [--max-laws N] [--dry-run]
                                           [--resume] [--no-network]
"""
...
def _candidate_laws(
    ...
    try:
        ...
        LOG.warning("am_law candidate fetch failed: %s", exc)
        return []
...
def _similarity_candidate(
    ...
    for r in rows:
        if r["body_en"] and len(r["body_en"]) > 256:
            return {
                "text": r["body_en"],
                "source_url": r["body_en_source_url"] or "",
                "score": 0.0,
...
def _process_law(
    ...
    *,
    ...
    if not network:
        primary_urls = [
            u for u in _build_translation_urls(canonical_id)
...
def _update_progress(conn: sqlite3.Connection, canonical_id: str, target_lang: str) -> None:
    ...
    conn.execute(
        ...
        " coverage_pct, last_refreshed_at) "
        "VALUES (?, ?, ?, ?, ?, ?) "
...
def run(db_path: str, *, max_laws: int | None, dry_run: bool, resume: bool,
    ...
    for law in _candidate_laws(conn, max_laws, resume):
        try:
            c = _process_law(conn, law, dry_run=dry_run,
                             refresh_id=refresh_id, network=network)
