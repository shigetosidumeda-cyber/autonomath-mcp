...
-- author: Wave 35 Axis 5d — 法令 한국어 corpus extension
...
CREATE INDEX IF NOT EXISTS ix_am_law_body_ko_present
    ON am_law(canonical_id) WHERE body_ko IS NOT NULL;
CREATE INDEX IF NOT EXISTS ix_am_law_title_ko_present
    ON am_law(canonical_id) WHERE title_ko IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS ix_am_law_article_body_ko_present
    ON am_law_article(law_canonical_id) WHERE body_ko IS NOT NULL;
