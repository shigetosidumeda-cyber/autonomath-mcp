...
into ``am_audit_workpaper`` (migration 270) so the lighter ¥3/req cohort
...
def _candidate_houjin(
    conn: sqlite3.Connection, fy_start: str, fy_end: str, limit: int
) -> list[str]:
    """Active 法人 = had ≥1 jpi_adoption_records row in FY window."""
    try:
        rows = conn.execute(
            """
            SELECT DISTINCT applicant_houjin_bangou AS hb
             ...
             LIMIT ?
            """,
            (fy_start, fy_end, fy_start, fy_end, limit),
        ).fetchall()
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.only:
            ...
            cohort_all: list[str] = []
            for fy in fys:
                fy_start, fy_end = f"{fy:04d}-04-01", f"{fy + 1:04d}-03-31"
                cohort_all.extend(_candidate_houjin(conn, fy_start, fy_end, args.limit_houjin))
