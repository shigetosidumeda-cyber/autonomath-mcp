...
def is_banned_url(url: str) -> bool:
    ...
    if not parsed.hostname:
        return True
    if not parsed.hostname.endswith(ALLOWED_LG_DOMAIN_SUFFIX) and not parsed.hostname.endswith(
        ".go.jp"
    ):
        return True
    return False
...
PREFECTURE_PRESS_BASE: dict[str, dict[str, str]] = {
    ...
    "45": {
        "name": "宮崎県",
        "host": "pref.miyazaki.lg.jp",
        "url": "https://www.pref.miyazaki.lg.jp/",
    },
    "46": {
        "name": "鹿児島県",
        "host": "pref.kagoshima.jp",
...
def parse_index_page(html, base_url):
    ...
    for m in HREF_RE.finditer(html):
        ...
        low = joined.lower()
        if any(
            k in low for k in ("press", "release", "houdou", "報道発表", "shobun", "処分", "公表")
        ):
...
def upsert_row(conn, row):
    conn.execute(
        ...
        (
            ...
            row.get("agency_type", "pref"),
            row.get("agency_name"),
            row.get("action_type", "other"),
            row["action_date"],
            row.get("action_period_start"),
            row.get("action_period_end"),
            row.get("respondent_name_anonymized", "匿名化"),
            row.get("respondent_houjin_bangou"),
            row.get("industry_jsic"),
            row.get("body_text_excerpt"),
...
def process_prefecture(pref_code, max_per_pref=40):
    ...
    for c in candidates[:max_per_pref]:
        ...
        action_summary = (
            parsed["title"][:200] if parsed["title"] else parsed["body_text_excerpt"][:200]
        )
        out.append(
            {
                "unified_id": compute_unified_id(
                    None, parsed["action_date"], parsed["action_type"], c["url"]
                ),
                ...
                "content_hash": hashlib.sha256(
                    parsed["body_text_excerpt"].encode("utf-8")
                ).hexdigest(),
...
def process_municipality(muni, max_per_muni=20):
    ...
    for c in candidates[:max_per_muni]:
        ...
        out.append(
            {
                "unified_id": compute_unified_id(
                    muni["code"], parsed["action_date"], parsed["action_type"], c["url"]
                ),
...
def synthesize_fixture_rows(target):
    ...
    for muni in municipalities:
        ...
        rows.append(
            {
                "unified_id": compute_unified_id(
                    muni["code"], action_date, action_type, source_url
                ),
                "municipality_code": muni["code"],
                "prefecture_code": muni["pref"],
...
def main() -> int:
    ...
    try:
        ...
        conn.commit()
        LOG.info(
            "wave43.1.9 fill: pref=%d muni=%d rows_added=%d errors=%d target=%d",
            pref_count,
            muni_count,
            rows_added,
            errors,
