...
async def evidence3_events(
    step: int, houjin_bangou: str, intent: str | None = None, jsic: str | None = None,
) -> AsyncIterator[bytes]:
    ...
    if step == 1:
        ...
        yield _sse("section", {"name": "identity", "data": {
        ...
        await asyncio.sleep(0.2)
        yield _sse("section", {"name": "invoice", "data": {"t_number": f"T{houjin_bangou}", "status": "checking"}})
        await asyncio.sleep(0.2)
        yield _sse("section", {"name": "enforcement", "data": {"count_5y": 0, "as_of": time.strftime("%Y-%m-%d")}})
        await asyncio.sleep(0.2)
        yield _sse("section", {"name": "licenses", "data": {"count": 0, "list": []}})
        await asyncio.sleep(0.2)
        yield _sse("done", {"step": 1, "elapsed_ms": int((time.monotonic() - start) * 1000), "billable_units": 1})
    elif step == 2:
        yield _sse("status", {"phase": "match_programs", "elapsed_ms": 0})
        await asyncio.sleep(0.2)
        yield _sse("section", {"name": "decision_insights", "data": {"candidates": 0, "_loading": True}})
        await asyncio.sleep(0.3)
        yield _sse("section", {"name": "copy_paste_parts", "data": {"proposal_200ji": "..."}})
        await asyncio.sleep(0.2)
        yield _sse("done", {"step": 2, "elapsed_ms": int((time.monotonic() - start) * 1000), "billable_units": 1})
    elif step == 3:
        ...
        await asyncio.sleep(0.2)
        yield _sse("done", {"step": 3, "elapsed_ms": int((time.monotonic() - start) * 1000), "billable_units": 1, "next": "/artifact.html?id=<pack_id>"})
    else:
