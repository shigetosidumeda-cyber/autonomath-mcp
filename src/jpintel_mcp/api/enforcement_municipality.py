...
def _row_to_model(row: sqlite3.Row) -> EnforcementMunicipality:
    return EnforcementMunicipality(
        ...
        action_period_start=row["action_period_start"],
...
@router.get(
    "/search",
    response_model=EnforcementMunicipalitySearchResponse,
...
def search_enforcement_municipality(
    ...
    if industry_jsic:
        where.append("industry_jsic = ?")
        params.append(industry_jsic)
    if action_from:
        where.append("action_date >= ?")
        params.append(action_from)
    if action_to:
        where.append("action_date <= ?")
        params.append(action_to)
    if q:
        like = f"%{q}%"
        where.append(
            "(COALESCE(action_summary,'') LIKE ? OR COALESCE(body_text_excerpt,'') LIKE ? OR COALESCE(agency_name,'') LIKE ?)"
        )
    ...
    try:
        try:
            (total,) = conn.execute(
                f"SELECT COUNT(*) FROM v_enforcement_municipality_public WHERE {where_sql}", params
            ).fetchone()
        except sqlite3.OperationalError as exc:
            raise HTTPException(
                status.HTTP_503_SERVICE_UNAVAILABLE,
                f"am_enforcement_municipality not available: {exc}",
            )
        rows = conn.execute(
        ...
        log_usage(
            ...
            ctx,
            "enforcement_municipality.search",
            params={
...
def get_enforcement_municipality(unified_id: str, ctx: ApiContextDep) -> EnforcementMunicipality:
    ...
    try:
        try:
            row = conn.execute(
                "SELECT * FROM v_enforcement_municipality_public WHERE unified_id = ?",
                (unified_id,),
            ).fetchone()
        except sqlite3.OperationalError as exc:
            raise HTTPException(
        ...
        if row is None:
            raise HTTPException(
                status.HTTP_404_NOT_FOUND, f"enforcement municipality not found: {unified_id}"
            )
        log_usage(
            cast("sqlite3.Connection", None),
            ctx,
            "enforcement_municipality.get",
