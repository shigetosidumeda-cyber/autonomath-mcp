...
def record_received(conn: sqlite3.Connection, event_id: str, event_type: str, customer_id: str | None = None) -> None:
    conn.execute(
...
def mark_success(conn: sqlite3.Connection, event_id: str, api_key_id: str | None = None, extra: dict | None = None) -> None:
    conn.execute(
        ...
        (datetime.now(timezone.utc).isoformat(), api_key_id, json.dumps(extra or {}, ensure_ascii=False), event_id),
    )


def mark_failure(conn: sqlite3.Connection, event_id: str, error: str, permanent: bool = False) -> None:
