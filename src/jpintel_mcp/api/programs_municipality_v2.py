...
Endpoints:
* GET /v1/programs/by_municipality/{municipality_code} - 5-digit JIS
* GET /v1/programs/by_prefecture/{prefecture_code}   - 2-digit JIS

Backed by am_program_source_municipality_v2 (migration 248, target_db=autonomath).
NO LLM API. NO destructive write. NO full-scan op on multi-GB DB.
"""
...
_DISCLAIMER = (
    "本レスポンスは jpcite が自治体 web の公開情報を機械的に収集・整理した"
    "結果を返却するものであり、税理士法 §52 / 公認会計士法 §47条の2 / "
    "行政書士法 §1 / 弁護士法 §72 に基づく個別具体的な税務助言・監査意見・"
    "申請書面作成・法律相談の代替ではありません。掲載情報は ``last_verified`` "
    "時点の snapshot です — 最新の募集要項は必ず一次資料 (各自治体公式 web) "
    "で確認してください。"
)
...
def _fetch_rows(
    ...
    try:
        cur = db.execute(
            "SELECT program_id, municipality_code, grant_type, "
            "       prefecture_code, source_url, source_fetched_at, "
            "       last_verified "
            "FROM am_program_source_municipality_v2 " + where
            + " ORDER BY last_verified DESC, program_id "
              "LIMIT ? OFFSET ?",
            [*params, limit, offset],
        )
...
async def list_programs_by_municipality(
    ctx: ApiContextDep, db: DbDep,
    municipality_code: Annotated[
        str, Path(description="5-digit JIS X 0401/0402 (e.g. 13104)")
    ],
    ...
    rows, total = _fetch_rows(db, where=where, params=params,
                              limit=limit, offset=offset)
    latency_ms = int((time.monotonic_ns() - started_ns) / 1_000_000)
    log_usage(db, ctx, endpoint="programs_by_municipality_v2",
              quantity=1, latency_ms=latency_ms, result_count=len(rows))
...
async def list_programs_by_prefecture(
    ...
    rows, total = _fetch_rows(db, where=where, params=params,
                              limit=limit, offset=offset)
    latency_ms = int((time.monotonic_ns() - started_ns) / 1_000_000)
    log_usage(db, ctx, endpoint="programs_by_prefecture_v2",
              quantity=1, latency_ms=latency_ms, result_count=len(rows))
    return JSONResponse(_wrap({
        "items": rows, "total": total, "limit": limit, "offset": offset,
