"""HE-6 cohort ultra-deep — 行政書士 (gyouseishoshi). D+-tier ¥100."""
...
def agent_cohort_ultra_gyouseishoshi(
    query: Annotated[
        str,
        Field(min_length=1, max_length=512, description="行政書士 ultra-deep query."),
    ],
    entity_id: Annotated[
        str | None,
        Field(
    ...
    """[AUDIT, SENSITIVE — §52/§47条の2/§72/§1/§3] Moat HE-6 cohort ultra — 行政書士.

    D+-tier ¥100 (33 units × ¥3). Returns 15 sections.

    Cost-saving claim: equivalent to ~21-turn Opus 4.7 multi-round
    reasoning (¥1,500). This endpoint: ¥100 = 1/15. Cohort: 行政書士.
...
__all__ = ["agent_cohort_ultra_gyouseishoshi"]
