...
from . import (
    ...
    chain_wave51_b,  # noqa: F401  — Wave 51 chain B (2026-05-16): 5 per-dim primitive composition chain MCP wrappers (predictive_subscriber_fanout_chain / session_multi_step_eligibility_chain / rule_tree_batch_eval_chain / anonymized_cohort_query_with_redact_chain / time_machine_snapshot_walk_chain), 179 → 184. AUTONOMATH_WAVE51_CHAIN_B_ENABLED gate (default ON). Heavy compound tier — 3 ¥3 units (税込 ¥9.90) per call. NO LLM. §52/§47条の2/§72/§1/§3 envelope.
    ...
    dd2_municipality_tools,  # noqa: F401  — DD2 Geo expansion (2026-05-17): 1,714 市町村 補助金 PDF + Textract OCR structured corpus. 1 tool (find_municipality_subsidies) over am_municipality_subsidy (autonomath.db, migration wave24_217). 4-axis cohort filter + 5-axis citation. AUTONOMATH_DD2_MUNICIPALITY_ENABLED gate (default ON). NO LLM, ¥3/req. §13.
    ...
    foreign_fdi_mcp,  # noqa: F401  — Wave 46 dim 19 EJ booster (2026-05-12): foreign_fdi_list_am + foreign_fdi_country_am — MCP wrappers over api/foreign_fdi_v2.py (Dim J, Wave 43.2.10). 80-country cohort filter + 1-country detail. AUTONOMATH_FOREIGN_FDI_MCP_ENABLED gate (default ON). NO LLM, single ¥3/req billing event. §72/§1/出入国管理 envelope. REST companions at GET /v1/foreign_fdi/v2/countries + /country/{iso}.
    ...
    opensearch_hybrid_tools,  # noqa: F401  — Moat Lane M10 (2026-05-17): opensearch_hybrid_search MCP wrapper over jpcite-xfact-2026-05 (r5.4xlarge x3 + r5.large x3 + ultrawarm1.medium x3 multi-AZ). BM25 kuromoji + corpus_kind/prefecture/tier/min_amount filters + reserved knn_vector. AUTONOMATH_OPENSEARCH_HYBRID_ENABLED gate (default ON). NO LLM, SigV4 _search. §52/§47条の2/§72/§1/§3 envelope.
    outcome_wave59_b,  # noqa: F401  — Wave 59 Stream B (2026-05-16): 10 top outcome MCP wrappers (outcome_houjin_360 / outcome_program_lineage / outcome_acceptance_probability / outcome_tax_ruleset_phase_change / outcome_regulatory_q_over_q_diff / outcome_enforcement_seasonal_trend / outcome_bid_announcement_seasonality / outcome_succession_event_pulse / outcome_prefecture_program_heatmap / outcome_cross_prefecture_arbitrage). Reads S3 derived packet skeleton index + emits JPCIR Evidence + OutcomeContract + citations + known_gaps. AUTONOMATH_OUTCOME_WAVE59_B_ENABLED gate (default ON). NO LLM, ¥300 / ¥600 / ¥900 billing band (1 / 2 / 3 ¥3 units). §52/§47条の2/§72/§1/§3 envelope.
    ...
    semantic_search_legacy,  # noqa: F401  — Wave 46 dim 19 / Dim A booster (2026-05-12): 1024-dim e5-large legacy v1 MCP wrap (semantic_search_legacy_am). Wraps POST /v1/semantic_search canonical vec corpus (am_canonical_vec_program/law/case_study + 4 wired-not-launched). AUTONOMATH_SEMANTIC_SEARCH_LEGACY_ENABLED gate (default ON). 1 ¥3/req unit. §52/§72/§1 envelope. NO LLM — client supplies embedding.
    semantic_search_mcp,  # noqa: F401  — Wave 46 dim 19 EJ booster (2026-05-12): semantic_search_v2_am — alias MCP wrapper for semantic_search_am with v2-suffix in module name to satisfy dim 19 audit grep (semantic_search_v2 + _mcp glob). Pure delegation to semantic_search_v2._semantic_search_impl. AUTONOMATH_SEMANTIC_SEARCH_MCP_ENABLED gate (default ON). NO LLM. 2 ¥3/req units when rerank=True.
    ...
    wave51_chains,  # noqa: F401  — Wave 51 chains (2026-05-16): 4 cross-dim service composition chain MCP wrappers (evidence_with_provenance_chain / session_aware_eligibility_check_chain / federated_handoff_with_audit_chain / temporal_compliance_audit_chain). AUTONOMATH_WAVE51_CHAINS_ENABLED gate (default ON). Heavy compound tier — 3 ¥3 units (税込 ¥9.90) per call. NO LLM. §52/§47条の2/§72/§1/§3 envelope.
