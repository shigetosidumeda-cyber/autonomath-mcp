...
Three MCP tools that read from the daily-refreshed precompute tables
written by ``scripts/cron/precompute_cohort_5d.py``,
``precompute_program_risk_4d.py`` and ``precompute_supplier_chain.py``:
...
Each tool docstring carries the canonical
**いつ使うか / 入力 / 出力 / エラー** sections so the customer LLM can pick
the right tool without re-reading the schema.

Constraints (CLAUDE.md + memory)
--------------------------------
* NO LLM call — pure SQLite SELECTs over the precompute tables.
* NO full-scan op against the 9.7GB autonomath.db at request time
  (memory ``feedback_no_quick_check_on_huge_sqlite``).
* §52 / §47条の2 / 行政書士法 §1 / 弁護士法 §72 disclaimer envelope
  on every 2xx body — output is information retrieval.

Gate
----
``AUTONOMATH_COHORT_RISK_CHAIN_ENABLED`` (default ``1``) and the parent
``settings.autonomath_enabled``. Setting either to ``0`` removes all
three tools from ``mcp.list_tools()`` without altering cohort_match_tools.
"""
...
def _open_autonomath_ro() -> sqlite3.Connection | dict[str, Any]:
    """Open autonomath.db read-only via the shared connector. Soft-fail to error."""
    try:
...
# --------------------------------------------------------------------------- #
# Implementations (importable directly by tests)
# --------------------------------------------------------------------------- #
...
def match_cohort_5d_impl(
    ...
    """Pure-function impl exposed for tests + REST adapter."""
...
if _ENABLED and settings.autonomath_enabled:
    ...
    def match_cohort_5d_am(
        jsic_major: Annotated[
            ...
            Field(
                min_length=1,
                max_length=1,
                description="JSIC 大分類 1 char (A-T).",
            ),
        ],
        employee_band: Annotated[
            str,
            Field(
                description="Size band — '1-9' / '10-99' / '100-999' / '1000+'.",
            ),
        ],
        prefecture_code: Annotated[
            ...
            Field(
                default=None,
                max_length=2,
                description="2-digit JIS pref code, e.g. '13' = 東京都. None = nationwide.",
            ),
        ] = None,
        limit: Annotated[
            int,
            Field(ge=1, le=100, description="Max programs to return, [1,100]."),
        ] = 20,
    ) -> dict[str, Any]:
        """[COHORT-5D] いつ使うか: 法人 × 業種 × 規模 × 地域 × 制度の 5 軸で「私と同じカテゴリの企業が通った制度」を即座に返す。入力: jsic_major (A-T)、employee_band (4 段階)、prefecture_code (47 都道府県 + null)、limit (1-100)。出力: eligible_program_ids[]、cohort_meta (last_refreshed_at, eligible_count)。エラー: missing jsic/band → invalid_enum、autonomath.db 不在 → db_unavailable。1 ¥3/req。NO LLM。§52 / §47条の2 / §1 sensitive。"""
    ...
    def program_risk_score_am(
        program_id: Annotated[
            str,
            Field(description="制度 unified_id, e.g. 'UNI-it-2025-...'"),
        ],
    ) -> dict[str, Any]:
        """[PROGRAM-RISK-4D] いつ使うか: 制度 × 業法 × 行政処分 × 取消理由の 4 軸 risk score (0-100) を返す。weighted: 業法 0.5 / 行政処分 0.3 / 取消理由 0.2。入力: program_id (unified_id)。出力: top_risk + all_axes[]、weights、_disclaimer。エラー: program_id 未指定 → invalid_enum、precompute 不在 → axis_count=0 で返却 (200 OK)。1 ¥3/req。NO LLM。§52 / §47条の2 / §1 sensitive — 行政書士法 §1 / 税理士法 §52 の boundary を crossed 制度を識別する用途。"""
