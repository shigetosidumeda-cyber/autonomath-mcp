...
実際に動く 8 つのクエリ例。本番 API (`https://api.autonomath.ai`) に対して
`curl` で叩ける形式と、MCP ツール経由で呼ぶ形式の両方を示す。レスポンス例は
データベース行から組み立てた **expected behavior** であり、行の追加 / 削除で
若干変わる。料金は **¥3/req 一律** (税込 ¥3.30)、anonymous は IP 単位で
50 req/月 (JST 月初 00:00 リセット) まで無料。

```bash
export AUTONOMATH_API_BASE=https://api.autonomath.ai
...
curl -s "$AUTONOMATH_API_BASE/v1/programs/search?q=新規就農&prefecture=青森県&tier=S&tier=A&limit=10" \
  -H "Authorization: Bearer $AUTONOMATH_API_KEY"
```
...
/ 第三者保証人を独立 enum 化済み (migration 013)。各軸の enum は
...
の歴史照会は「公布日時点の名目的状態」までで、**条件文の point-in-time
復元は信用しない**。
