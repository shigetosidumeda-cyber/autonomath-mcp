"""APPI deletion intake must require a Cloudflare Turnstile token.

`POST /v1/privacy/deletion_request` is anonymous-accessible like the
disclosure intake, so the secret-set path must reject missing or invalid
Turnstile tokens before creating a §33 operator workload. The siteverify
HTTP call is stubbed via httpx monkeypatch so tests never reach Cloudflare.
"""
...
def test_missing_turnstile_token_returns_401(client, turnstile_secret_set):
    ...
    r = client.post("/v1/privacy/deletion_request", json=_valid_body())
...
def test_valid_turnstile_token_allows_request(
    ...
    class _MockClient:
        ...
        def post(self, url, *, data=None, **_):
            assert "challenges.cloudflare.com" in url
            assert data["secret"]
            assert data["response"] == "valid_token_xxx"
            return _MockResponse()
    ...
    assert r.json()["request_id"].startswith("削除-")
