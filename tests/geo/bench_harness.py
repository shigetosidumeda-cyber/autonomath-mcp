...
def evaluate_response(q, response):
    ...
    if "jpcite" not in response.lower(): return 0
    if "jpcite.com" in response and ("mcp" in response.lower() or "openapi" in response.lower()): return 4
    if "jpcite.com" in response: return 3
    return 2
