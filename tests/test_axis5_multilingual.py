...
Covers:
  - migrations 240/241/242/243 apply cleanly + carry target_db hints
  - rollback companions exist for each forward migration
  - dry-run / --no-network cron run for 4 fill scripts (10 each)
  - aggregator URL refusal (banned list enforcement)
  - LLM API import scan (anthropic / openai / google.generativeai /
    claude_agent_sdk must not appear in any of the 5 new cron files
    nor the 4 new migrations).

The dry-run path uses --no-network so the test is hermetic and does NOT
hit live ministry servers. The integration / production runs (weekly
GHA workflow) DO hit the network on the live volume.
"""
...
AXIS5_MIGRATIONS_JPINTEL = [
    "241_programs_en",
]
ALL_AXIS5_MIGRATIONS = (
    AXIS5_MIGRATIONS_AUTONOMATH + AXIS5_MIGRATIONS_JPINTEL
)
...
def test_axis5_migrations_apply(tmp_path: Path) -> None:
    ...
    assert expected.issubset(tables), f"missing autonomath tables: {expected - tables}"
    conn.close()
...
def test_axis5_cron_dry_run(
    tmp_path: Path, script: str, db_flag: str, seed: str
) -> None:
...
# ---------------------------------------------------------------------------
# Aggregator URL refusal — fill_programs_en
# ---------------------------------------------------------------------------
...
def test_aggregator_url_refused(tmp_path: Path) -> None:
    ...
    conn = _seed_jpintel_db(db)
    # Wipe + insert an aggregator-source-URL row
    conn.execute("DELETE FROM programs")
    conn.execute(
        "INSERT INTO programs VALUES (?, ?, 'A', 0, ?)",
...
# ---------------------------------------------------------------------------
# LLM API import scan — feedback_no_operator_llm_api enforcement
# ---------------------------------------------------------------------------
...
def test_axis5_migrations_no_llm_refs() -> None:
    """Migration SQL must not embed LLM-API hints either."""
...
# ---------------------------------------------------------------------------
# translate_review_queue export round-trip
# ---------------------------------------------------------------------------
