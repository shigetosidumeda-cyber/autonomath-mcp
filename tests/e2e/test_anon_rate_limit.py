...
`src/jpintel_mcp/api/anon_limit.py` enforces 3 calls per IP per JST day
on the anonymous (no X-API-Key, no Bearer) path of
`/v1/programs/search`, `/v1/exclusions/*`, `/meta`, `/v1/ping`,
`/v1/feedback`. The 4th call should 429 with `Retry-After` +
`{"limit": 3, "resets_at": "..."}`.
...
Guardrails:
  1. Skip unless explicitly enabled with `JPINTEL_E2E_ANON_LIMIT=1`.
  2. Skip when the middleware isn't wired on the target (detected via a
     probe: if /meta doesn't expose the anon-limit behaviour, we skip).
  3. NEVER run against production — `is_local_target` OR an explicit
...
# Number of calls to issue. The default anon limit is 3/day, so 5
# guarantees trailing 429s (modulo earlier bucket state).
_N_CALLS = 5
_ANON_LIMIT = 3
...
async def _probe_anon_limit_installed(page: Page, base_url: str) -> bool:
    ...
    """
    r = await page.request.get(base_url + "/v1/ping", timeout=5_000)
...
async def test_anon_rate_limit_flips_to_429_after_quota(
    ...
    #   - at least one 429 in the trailing calls, OR
    #   - all calls 200 AND we are below the configured limit (in which
    #     case the test effectively no-ops because the bucket wasn't there
    #     yet — still a useful signal).
    trailing_429 = sum(1 for s in results[-2:] if s == 429)

    assert trailing_429 >= 1 or count_200 == _N_CALLS, (
        f"expected at least one 429 in trailing calls after {_N_CALLS} "
