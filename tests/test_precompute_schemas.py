...
PC_TABLES_C3: dict[str, set[str]] = {
    "pc_top_subsidies_by_industry": {
        "industry_jsic", "rank", "program_id", "relevance_score",
        "cached_payload", "refreshed_at",
    },
    "pc_top_subsidies_by_prefecture": {
        "prefecture_code", "rank", "program_id", "relevance_score",
        "cached_payload", "refreshed_at",
    },
    "pc_law_to_program_index": {
        "law_id", "program_id", "citation_kind", "refreshed_at",
    },
    "pc_program_to_amendments": {
        "program_id", "amendment_id", "severity", "observed_at",
        "summary", "refreshed_at",
    },
    "pc_acceptance_stats_by_program": {
        "program_id", "fiscal_year", "round_label",
        "applied_count", "accepted_count", "acceptance_rate", "refreshed_at",
    },
    "pc_combo_pairs": {
        "program_a", "program_b", "compat_kind", "rationale", "refreshed_at",
    },
    "pc_seasonal_calendar": {
        "month_of_year", "program_id", "deadline_date",
        "deadline_kind", "refreshed_at",
    },
    "pc_industry_jsic_aliases": {
        "alias_text", "industry_jsic", "confidence", "source", "refreshed_at",
    },
    "pc_authority_to_programs": {
        "authority_id", "program_id", "role", "refreshed_at",
    },
    "pc_law_amendments_recent": {
        "amendment_id", "law_id", "severity", "effective_date",
        "observed_at", "summary", "refreshed_at",
    },
    "pc_enforcement_by_industry": {
        "industry_jsic", "enforcement_id", "severity",
        "observed_at", "headline", "refreshed_at",
    },
    "pc_loan_by_collateral_type": {
        "collateral_type", "rank", "loan_program_id",
        "cap_amount_yen", "refreshed_at",
    },
    "pc_certification_by_subject": {
        "subject_code", "rank", "certification_id", "refreshed_at",
    },
    "pc_starter_packs_per_audience": {
        "audience", "rank", "program_id", "note", "refreshed_at",
    },
}
...
PC_TABLES_D8: dict[str, set[str]] = {
    "pc_amendment_recent_by_law": {
        "law_id", "amendment_id", "severity", "effective_date",
        "summary", "refreshed_at",
    },
    "pc_program_geographic_density": {
        "prefecture_code", "tier", "program_count", "refreshed_at",
    },
    "pc_authority_action_frequency": {
        "authority_id", "month_yyyymm", "action_count", "refreshed_at",
    },
    "pc_law_to_amendment_chain": {
        "law_id", "amendment_id", "parent_amendment_id", "position",
        "effective_date", "refreshed_at",
    },
    "pc_industry_jsic_to_program": {
        "industry_jsic", "rank", "program_id", "relevance_score", "refreshed_at",
    },
    "pc_amount_max_distribution": {
        "bucket", "program_count", "refreshed_at",
    },
    "pc_program_to_loan_combo": {
        "program_id", "loan_program_id", "compat_kind", "rationale", "refreshed_at",
    },
    "pc_program_to_certification_combo": {
        "program_id", "certification_id", "requirement_kind", "refreshed_at",
    },
    "pc_program_to_tax_combo": {
        "program_id", "tax_ruleset_id", "applicability", "refreshed_at",
    },
    "pc_acceptance_rate_by_authority": {
        "authority_id", "fiscal_year", "applied_count", "accepted_count",
        "acceptance_rate", "program_coverage", "refreshed_at",
    },
    "pc_application_close_calendar": {
        "month_of_year", "program_id", "close_date", "days_until", "refreshed_at",
    },
    "pc_amount_to_recipient_size": {
        "amount_bucket", "smb_size_class", "recipient_count", "refreshed_at",
    },
    "pc_law_text_to_program_count": {
        "law_id", "program_count", "last_cited_at", "refreshed_at",
    },
    "pc_court_decision_law_chain": {
        "court_id", "law_id", "decision_id", "relation_kind",
        "decided_at", "refreshed_at",
    },
    "pc_enforcement_industry_distribution": {
        "industry_jsic", "severity", "five_year_count", "refreshed_at",
    },
    "pc_loan_collateral_to_program": {
        "collateral_type", "program_id", "rank", "refreshed_at",
    },
    "pc_invoice_registrant_by_pref": {
        "prefecture_code", "registrant_count", "last_seen_at", "refreshed_at",
    },
    "pc_amendment_severity_distribution": {
        "severity", "month_yyyymm", "amendment_count", "refreshed_at",
    },
}
...
def test_cron_real_run_keeps_tables_empty_pre_launch(
    migrated_db: Path, cron_module
):
