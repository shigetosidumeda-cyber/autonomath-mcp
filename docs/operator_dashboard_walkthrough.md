# 税務会計AI — Operator Dashboard Walkthrough
...
URL: `https://zeimu-kaikei.ai/dashboard.html` (requires API key in `Authorization: Bearer …`).
...
URL: `https://zeimu-kaikei.ai/stats.html` (anonymous OK).
...
URL: `https://zeimu-kaikei.ai/testimonials.html`.
...
- admin link: `https://zeimu-kaikei.ai/testimonials.html?admin=<OPS_ADMIN_TOKEN>` (env var で持つ、URL に直接埋めない)
...
URL: `https://status.zeimu-kaikei.ai` (= `site/status.html` を Cloudflare Worker で expose)。
