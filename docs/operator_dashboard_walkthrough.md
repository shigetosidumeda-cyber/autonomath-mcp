# jpcite — Operator Dashboard Walkthrough
...
で確認する。dashboard.html を operator が開くのは「customer に同意更新済の screenshare 中」のみ。
...
URL: `https://jpcite.com/stats.html` (anonymous OK).
...
URL: `https://jpcite.com/testimonials.html`.
...
- admin link: `https://jpcite.com/testimonials.html?admin=<OPS_ADMIN_TOKEN>` (env var で持つ、URL に直接埋めない)
...
URL: `https://status.jpcite.com` (= `site/status.html` を Cloudflare Worker で expose)。
