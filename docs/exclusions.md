...
税務会計AI は主要なルールを構造化データとして API で返すことで、この検証を **候補選定フェーズで自動化** する。
...
```bash
curl https://api.zeimu-kaikei.ai/v1/exclusions/rules
```
...
```bash
curl -X POST https://api.zeimu-kaikei.ai/v1/exclusions/check \
...
| heuristic-extracted (`excl-ext-*`) | 146 | 要綱 / 公募要領 PDF から rule-based heuristic で抽出、人手レビュー済み (取りこぼし / 誤検出の可能性は残る) |
| **合計** | **181** | — |
...
要綱 / 公募要領 PDF から rule-based heuristic で抽出した 146 ルール。いずれも `source_url` (当該 要綱 原典) + `source_excerpt` 付。**地域創生推進交付金系 / 雇用助成金系 / 業種別補助金 (医療・介護・建設・造船 等)** の同一資産重複・交付期限重複・クーリング期間違反などをカバー。

> **限界:** rule-based heuristic 抽出のため、 取りこぼし (未検出) / 誤検出 の両方が起こり得る。 人手レビュー済みではあるが、 確定的な exclusion 判定には `source_url` 一次資料の人手確認を必ず行うこと。 また「ルール未登録 = 安全」ではない。

個別ルールの全量は `list_exclusion_rules` または `GET /v1/exclusions/rules` で取得可能。 API ユーザーは `GET /v1/meta` の `exclusion_rules_count` で現在のルール数を確認し、自前の補完が必要かを判断してください。
...
3. **法的アドバイスではない。** 税務会計AI は構造化情報の提供のみ。個別案件の可否判断は行政書士・税理士等の専門家、または担当窓口へ。
