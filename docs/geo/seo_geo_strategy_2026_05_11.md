...
## 0. 出発点 (snapshot 2026-05-11)

| asset | 数 | source |
|---|---|---|
| MCP tools (default gates) | 139 | `mcp-server.json` / `pyproject.toml` (runtime 146、manifest hold-at-139) |
| OpenAPI paths | 219 | `docs/openapi/v1.json` (canonical manifest pinned) |
...
honest gap (data drift): `EXPECTED_OPENAPI_PATH_COUNT = 186` in test (stale), 公開 site の MCP tool 数表記は 139 / runtime 146 / 内部源 146 (manifest hold-at-139 で意図的遅延)。
...
3. JSON-LD `Dataset.variableMeasured` に `MCP tools = 139` が hard-code、runtime 146 / manifest hold-at-139 の drift を Schema.org 側で同期する仕組みがない。
...
- `scripts/etl/sync_jsonld_counts.py` 起案 → index.html JSON-LD の `Dataset.variableMeasured` を runtime 146 / programs 11,601 / 法令 6,493 等から自動 patch。
...
- llms.txt 内に **3 surface 別 call-order** (Claude Code / ChatGPT GPT / Cursor) + cost example 5 ケース + fence-aware quote 規約 (7 業法) + Evidence-to-Expert Handoff rule + 業法 fence 9 line。
...
**Evidence**:
- 業界 thesis として llms.txt + index.html JSON-LD で「Evidence prefetch = jpcite」「外部 LLM API を呼ばない (¥3/req の構造上当然)」「aggregator ban」「業法 fence 7 種」「source_url + fetched_at + known_gaps 必須」を全面に出している。
