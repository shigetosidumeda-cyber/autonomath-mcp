...
Hand-curation rules for the English half of the jpcite message
...
- Inline URLs without anchor text are fine in error messages:
  "See https://jpcite.com/docs/api-reference/."
...
- **No "official partner" / "endorsed by"** language re. ministries —
  jpcite is not endorsed by any ministry.
...
- **Sibling files.**
  - `site/llms.en.txt` (short index, ~40 lines) points at
    `https://jpcite.com/llms-full.en.txt` as the canonical full dump.
