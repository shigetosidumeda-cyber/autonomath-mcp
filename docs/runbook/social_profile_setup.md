...
6. **About / Description (2,000 chars max)**:
   > jpcite (, 適格請求書発行事業者番号 T8010001213708) は、日本の補助金・融資・税制・認定制度を REST API + MCP server で提供するサービスです。14,472 制度 + 採択事例 2,286 + 融資 108 + 行政処分 1,185 を一次資料ベース で正規化、集約サイト不採用、AI agent が士業・経営者の代わりに制度横断の照合を行えるよう設計しました。料金は ¥3/billable unit 完全従量 (税込 ¥3.30)、匿名 3 req/日 per IP は登録不要で無料。本サービスは情報検索であり、税理士法 §52 / 弁護士法 §72 に基づく個別具体的な税務助言・法律相談ではありません。
