...
- `site/index.html` / `pricing.html` / `dashboard.html` / `success.html` / `privacy.html` / `tos.html` / `tokushoho.html` — skip link + `href="#"` → `https://github.com/shigetosidumeda-cyber/autonomath-mcp` (公開準備中の旨 aria-label に明示) に置換
