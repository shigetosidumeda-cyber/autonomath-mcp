...
jpcite は日本国の公的制度 (補助金・融資・税制・認定・行政処分等) のメタデータを REST API + MCP server で配信する evidence-first context layer。Bookyou株式会社単独運営、¥3/req 完全従量。返り値は「制度候補・出典 URL・取得時刻・content hash・併用ルール・根拠パケット」のみで、自然言語回答生成は顧客側 LLM。当社は法律・税務・労務助言を一切提供せず、検索インデックスとして機能する。
...
- **特商法 `https://jpcite.com/tokushoho.html`**: 事業者名・所在地・適格請求書番号 T8010001213708 / ¥3/req 税込 ¥3.30 / 通信販売 (特商法 §15の3 によりクーリングオフ適用除外)
