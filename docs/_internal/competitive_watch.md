...
- **取得方法**: `httpx` + `User-Agent: AutonoMath-competitive-watch/1.0 (+https://autonomath.ai)`、robots.txt 尊重、1 host あたり 1 req/5sec、`If-Modified-Since` + ETag、エラー時 fail-open (他 host は継続)。
...
- 対象: `github.com/AutonoMath/AutonoMath` の全 fork
