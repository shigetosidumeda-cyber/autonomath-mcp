...
- **mitigation**:
  - すべての公開素材 (blog/docs/JSON-LD/email/Stripe/GitHub) で `https://jpcite.com` 表記を厳守
...
- ユーザー教育を docs/blog で `jpcite.com` のみに絞る (URL 表記の徹底)
...
- Name: `autonomath.com → jpcite.com canonical 301`
- When incoming requests match: `(http.host eq "autonomath.com") or (http.host eq "www.autonomath.com")`
- Then:
  - Type: Dynamic
  - Expression: `concat("https://jpcite.com", http.request.uri.path, http.request.uri.query)`
  - Status: 301
...
- Always Use HTTPS: ON
- HSTS: max-age=31536000 (jpcite.com と同等)

### 3-7. 検証
```bash
curl -sLI https://autonomath.com/        # → 301 Location: https://jpcite.com/
curl -sLI https://autonomath.com/pricing # → 301 Location: https://jpcite.com/pricing
curl -sLI https://www.autonomath.com/    # → 301 Location: https://jpcite.com/
```
...
- registrar 標準の "Domain Forwarding" で `autonomath.com` → `https://jpcite.com` 301 設定
...
site/index.html:
- L22: `<link rel="canonical" href="https://jpcite.com/">` OK
...
- L429: `data-api-base="https://api.jpcite.com"` OK
...
### 9-1. DNSSEC 有効化 (`jpcite.com`)

1. Cloudflare Dashboard → `jpcite.com` zone → DNS → Settings
...
4. DS 登録後、`dig jpcite.com DNSKEY +dnssec` と
   `dig jpcite.com +dnssec` で `ad` flag が立つこと確認
5. https://dnssec-analyzer.verisignlabs.com/jpcite.com で
...
2. https://hstspreload.org/ で `jpcite.com` を入力 → 自動で要件チェック
3. 要件: HTTPS redirect / max-age ≥ 31536000 / includeSubDomains /
   preload directive — 全て middleware で満たし済
4. `Submit jpcite.com to the preload list` クリック
5. Chromium / Firefox / Safari の preload list に取り込まれるまで
   ...
   **取り消しコストが高い** (delist 申請 + 数ヶ月 propagation) ので、
   `https://api.jpcite.com` も含む全 subdomain が確実に HTTPS 専用
   であることを再確認してから申請すること。
...
dig jpcite.com DNSKEY +dnssec +short
dig api.jpcite.com +dnssec | grep -E "flags|ad"

# HSTS header (middleware が出している)
curl -sI https://api.jpcite.com/meta | grep -i strict-transport-security
# → Strict-Transport-Security: max-age=31536000; includeSubDomains; preload

# preload registry status (申請後)
curl -s https://hstspreload.org/api/v2/status?domain=jpcite.com
```
