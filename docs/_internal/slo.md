...
|---|---|---|---|---|
| **S1** | API availability (`api.jpcite.com/v1/*`) | **99.5%** monthly | UptimeRobot 1-min probe of `/healthz` (multi-region majority vote) | カレンダー月 |
...
3. 30 min 応答不可と判明した瞬間: status page (`jpcite.com/status`)
   に手動で incident を起票し、`service@jpcite.com` 自動応答に
   "investigating" を一時掲示
