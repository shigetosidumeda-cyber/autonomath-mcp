...
## 3. Price の `tax_behavior` 確認 (metered ¥3/req)
...
|-----------------------------|--------------|------------------------------|-----------------|--------------|
| `STRIPE_PRICE_PER_REQUEST`  | ¥3 / req     | ¥0.30 / req (外税)           | `exclusive`     | Metered (legacy `usage_records`, `aggregate_usage=sum`) |
...
**exclusive (本番構成)** — 10,000 req を ¥3/req で月次請求:
```
