...
| 時刻 | ゲート | アクション | 成功判定 |
|------|-------|-----------|---------|
| **07:00** | smoke | `BASE_URL=https://autonomath.ai ./scripts/smoke_test.sh` + `python tests/mcp_smoke.py` + `curl -s https://autonomath.ai/meta \| jq .total_programs` | 全 probe green / `total_programs >= 6658` / Sentry 過去 12h で P0=0 |
...
| **08:00** | Zenn 公開 | 長文 (`docs/launch_content.md` Zenn 版) を **scheduled publish**。朝通勤タイミングに当てる | Zenn 記事 URL 確定、記事内 canonical は `autonomath.ai/articles/launch-announce` |
...
| 20:00 | HN pre-flight | HN draft 最終確認 (`docs/launch_assets/hn_show_post.md`)。title 80 chars 以下、URL `https://autonomath.ai` 200、GitHub placeholder 置換済、first comment 800-1500 chars。`https://news.ycombinator.com/showhn.html` 再読 | 22:30 投下前の最終 sanity check |
...
1. **確認** — ローカル DNS 問題を除外 (`dig autonomath.ai @1.1.1.1` 他 resolver)
2. **DNS flip** — Cloudflare dashboard → `autonomath.ai` → DNS → apex レコードを CNAME `[BRAND]-fallback.pages.dev` に編集 (proxied, TTL 300)
...
**アプローチ禁止**: cold DM でリプライ乞食。**やること**は、Zenn 記事末尾と GitHub README 下部に「取材・寄稿歓迎、hello@autonomath.ai」を 1 行、それだけ。
