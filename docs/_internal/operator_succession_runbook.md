...
| Domain registrar (zeimu-kaikei.ai, bookyou.net) | 1Password vault `Bookyou`, item `domain_registrar` | Email recovery |
...
<div class="status-banner status-warn">
  ...
  https://zeimu-kaikei.ai/succession.html をご覧ください。
</div>
```
...
Bookyou 株式会社 (法人番号 T8010001213708) より、本サービスに関する
重要なお知らせをいたします。
...
- 本日 〜 {{ trigger_date_plus_15 }}:
    ...
    詳細: https://zeimu-kaikei.ai/data-export.html
...
【返金について】
ご利用月の従量課金 (¥3/req) は日割り計算でご返金いたします。
...
DNS records: leave `zeimu-kaikei.ai` and `bookyou.net` resolving to the
...
pypi-cli deprecate autonomath-mcp \
  "Service discontinued 2026-MM-DD. See https://zeimu-kaikei.ai/succession.html"
