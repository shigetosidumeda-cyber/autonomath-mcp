...
| Domain registrar (autonomath.ai, bookyou.net) | 1Password vault `Bookyou`, item `domain_registrar` | Email recovery |
...
<div class="status-banner status-warn">
  ...
  https://autonomath.ai/succession.html をご覧ください。
</div>
```
...
- 本日 〜 {{ trigger_date_plus_15 }}:
    ...
    詳細: https://autonomath.ai/data-export.html
...
DNS records: leave `autonomath.ai` and `bookyou.net` resolving to the
...
pypi-cli deprecate autonomath-mcp \
  "Service discontinued 2026-MM-DD. See https://autonomath.ai/succession.html"
