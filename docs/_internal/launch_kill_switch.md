...
4. 確認: `curl -sS https://api.jpcite.com/v1/programs/search | jq '.error.code'` → `"service_unavailable"`。
...
2. **アプリ挙動**:
   - api.jpcite.com は Fly 502 / unreachable
   - jpcite.com (Cloudflare Pages) は静的に動き続ける — ランディング、tos, privacy, /status, /docs (mkdocs static) は配信継続
   - DNS: `api.jpcite.com` を Cloudflare Pages の fallback バナーへ flip するなら `docs/_internal/fallback_plan.md` 参照
...
- **Always announce on https://jpcite.com/status/ before un-killing.** これは brand 5-pillar (透明・誠実) の運用面の核。
...
7. **Smoke test**: `BASE_URL=https://jpcite.com ./scripts/smoke_test.sh` を流して全 probe green を確認。
...
- **3 lever 全部試して効かないとき**: DNS で api.jpcite.com を `jpcite.com/status` に CNAME リダイレクト。これは緊急時のみ。
