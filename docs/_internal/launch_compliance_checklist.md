...
本書は Stripe live mode を ON にし、DNS を本番 machine へ切り替える前に通す最後のゲートである。各項目に `[ ]` を付け、`what / how / owner` の 3 カラムで進める。`jpcite.com` は商標リブランド決着後に確定する (`project_jpintel_trademark_intel_risk.md` 参照、現在コード中に `jpcite.com` がハードコードされているのは全て要置換)。
...
- [ ] **Checkout 前 TOS 同意** — `api/billing.py:65-74` に `consent_collection={"terms_of_service": "required"}` 追加、Dashboard → Settings → Checkout に TOS URL `https://jpcite.com/tos.html` 登録 / owner: you
...
- [ ] **開示・削除請求 SLA** — `privacy.html` 第 7 条の窓口 `hello@jpcite.com` の応答 SLA (最大 2 週間) を internal runbook に明記 / owner: you

---

## 6. DNS / domain / TLS final checks

`jpcite.com` は商標レビュー待ち (`project_jpintel_trademark_intel_risk.md`)。以下は placeholder、確定値で `.github/workflows/tls-check.yml:27` を置換。all owner: you.

- [ ] **Primary CNAME → Fly** — `jpcite.com` apex を Fly A/AAAA or proxied CNAME、flip 48h 前に TTL 30-60s 短縮
- [ ] **Backup (Cloudflare Pages)** — `jpcite.com-fallback.pages.dev` warm 確認、`.github/workflows/pages-preview.yml` Actions グリーン
- [ ] **MX / SPF / DKIM / DMARC** — `hello@jpcite.com` 送受信。`dig MX/TXT`、`_dmarc` に `v=DMARC1; p=quarantine`、Gmail / Outlook に test 送信し 3 Pass
...
- [ ] Dry-run 6 step: (1) `flyctl status` (2) staging で `flyctl scale count 0` で擬似障害 (3) Cloudflare DNS `@` を `jpcite.com-fallback.pages.dev` CNAME に差し替え (4) `site/status.html` の `active` class を `.state.down` に移動→push→Pages redeploy (5) `curl -I https://jpcite.com/` で Cloudflare `cf-ray` header 確認 (6) `fallback_plan.md` Recovery § 逆順で復旧
