...
```
件名: jpcite 料金体系について (Re: お問い合わせ)

ご連絡ありがとうございます。jpcite の梅田です。
...
### A-2. 「Free 3 req/日 使い切った後、paid に切替方法?」
...
jpcite は tier (プラン) を切り替える仕組みではなく、
...
- 全件メタ付与: 各 program に source_url + source_fetched_at
  (出典更新日時) を記録し、API レスポンスに含めて返却します
- 出典更新日時の意味: source_fetched_at は「最終確認日」ではなく
...
```
件名: [予定] jpcite メンテナンス (YYYY-MM-DD HH:MM JST)

jpcite をご利用いただきありがとうございます。
...
復旧後 https://status.jpcite.com に完了通知を掲載します。
...
```
件名: [障害] jpcite API 障害のご報告 (YYYY-MM-DD)
...
- 恒久対策と詳細 RCA: 7 営業日以内に
  status.jpcite.com/incidents/YYYY-MM-DD に掲載予定
...
```
件名: [障害] jpcite DB 復旧のご報告 (YYYY-MM-DD)
...
- 対応:
  ...
  - 後継 URL を web archive または同省庁の現行ページから探索のうえ、
    24 時間以内に置換予定です
  - 後継が見つからない場合は当該 program を検索対象外 (内容確認中) へ
    移動し、検索結果から除外します
...
- 対象 program ID: prog_XXX (現状 検索対象外 = 内容確認中)
...
- 利用規約: https://jpcite.com/docs/compliance/terms_of_service/
- プライバシーポリシー: https://jpcite.com/docs/compliance/privacy_policy/
- データガバナンス: https://jpcite.com/docs/compliance/data_governance/
- データ主体の権利: https://jpcite.com/docs/compliance/data_subject_rights/
