...
> **要約:** 2026-05-06 launch 直後から回す A/B テスト用のコピー素材集。`docs/conversion_funnel.md` §4 の P0 レバー「Pricing 上部 curl snippet 常設」「Hero 明瞭化」に対応する paste-ready variant。Japanese primary、Do-not-edit-HTML 原則 (本稿は投入前の素材庫)。ドメイン名は rebrand pending のため `jpcite.com` placeholder で固定。
...
<section class="code-demo" aria-labelledby="pricing-demo-title">
  ...
  <pre class="code-block" aria-label="curl コマンド例"><code>$ curl https://jpcite.com/v1/programs/search?q=農業&limit=5 \
