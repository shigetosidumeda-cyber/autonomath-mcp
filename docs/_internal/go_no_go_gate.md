# jpcite — Go / No-Go Gate (operator-only)
...
- [ ] PASS / [ ] FAIL
- ≪`autonomath-mcp` 起動 → MCP `tools/list` で 89 tool 出力 ≫
...
- [ ] PASS / [ ] FAIL
- ≪INV-04 (¥3/billable unit metering) / INV-21 (data hygiene aggregator block) / INV-22 (10 keyword honesty block) / INV-23 (claim_strength gating) / INV-25 (PII redaction) が middleware で active≫
...
### 17. docs.jpcite.com/compliance/* (critical)
...
- 確認: `curl -s https://jpcite.com/docs/compliance/ | grep -c "href=\"./"` で >= 6

### 18. tokushoho (critical)

- [ ] PASS / [ ] FAIL
- ≪`https://jpcite.com/tokushoho.html` (or `/legal/tokushoho`) で特商法 8 必須項目 + 適格請求書発行事業者番号 T8010001213708 表示≫
...
- ≪`pricing.md` の月額 calculator が 50 / 1k / 10k req 表示 + ¥3/billable unit 完全従量明示≫
