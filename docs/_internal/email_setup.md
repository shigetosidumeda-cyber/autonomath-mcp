...
| 用途 | アドレス | Postmark 設定 |
| --- | --- | --- |
| From (all transactional) | `no-reply@autonomath.ai` | Sender Signature を DKIM 込みで verify |
| Reply-To | `hello@autonomath.ai` | 受信専用 — forward to `info@bookyou.net` inbox or Postmark Inbound |
| Bounce Return-Path | `pm-bounces.autonomath.ai` | 下記 DNS §3-4 参照 |
| Digest footer support | `support@autonomath.ai` | 受信のみ |
...
```
autonomath.ai    TXT   "v=spf1 include:spf.mtasv.net ~all"
```
...
```
20260506pm._domainkey.autonomath.ai        CNAME  20260506pm.autonomath.ai.dkim.mtasv.net
20260506pm-bounces._domainkey.autonomath.ai CNAME  20260506pm-bounces.autonomath.ai.dkim.mtasv.net
```
...
```
pm-bounces.autonomath.ai   CNAME   pm.mtasv.net
```
...
```
_dmarc.autonomath.ai   TXT   "v=DMARC1; p=quarantine; rua=mailto:dmarc-reports@autonomath.ai; pct=100; adkim=r; aspf=r"
```
...
```
URL:     https://api.autonomath.ai/v1/email/webhook
...
- Body 骨格:
  ...
  Getting started: https://autonomath.ai/docs/
  Support:        support@autonomath.ai
  ```
...
- Body 骨格:
  ...
  配信停止: https://autonomath.ai/v1/subscribers/unsubscribe?scope=digest&email={{email}}&token={{unsub_token}}
  ```
...
- [ ] `POSTMARK_FROM_TRANSACTIONAL=no-reply@autonomath.ai` 投入
- [ ] `POSTMARK_FROM_REPLY=hello@autonomath.ai` 投入
