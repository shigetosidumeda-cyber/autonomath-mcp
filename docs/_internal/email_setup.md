...
| 用途 | アドレス | Postmark 設定 |
| --- | --- | --- |
| From (all transactional) | `no-reply@zeimu-kaikei.ai` | Sender Signature を DKIM 込みで verify |
| Reply-To | `hello@zeimu-kaikei.ai` | 受信専用 — forward to `info@bookyou.net` inbox or Postmark Inbound |
| Bounce Return-Path | `pm-bounces.zeimu-kaikei.ai` | 下記 DNS §3-4 参照 |
| Digest footer support | `support@zeimu-kaikei.ai` | 受信のみ |
...
```
zeimu-kaikei.ai    TXT   "v=spf1 include:spf.mtasv.net ~all"
```
...
```
20260506pm._domainkey.zeimu-kaikei.ai        CNAME  20260506pm.zeimu-kaikei.ai.dkim.mtasv.net
20260506pm-bounces._domainkey.zeimu-kaikei.ai CNAME  20260506pm-bounces.zeimu-kaikei.ai.dkim.mtasv.net
```
...
```
pm-bounces.zeimu-kaikei.ai   CNAME   pm.mtasv.net
```
...
```
_dmarc.zeimu-kaikei.ai   TXT   "v=DMARC1; p=quarantine; rua=mailto:dmarc-reports@zeimu-kaikei.ai; pct=100; adkim=r; aspf=r"
```
...
```
URL:     https://api.zeimu-kaikei.ai/v1/email/webhook
...
- Body 骨格:
  ...
  Getting started: https://zeimu-kaikei.ai/docs/
  Support:        support@zeimu-kaikei.ai
  ```
...
- Body 骨格:
  ...
  配信停止: https://zeimu-kaikei.ai/v1/subscribers/unsubscribe?scope=digest&email={{email}}&token={{unsub_token}}
  ```
...
- [ ] `POSTMARK_FROM_TRANSACTIONAL=no-reply@zeimu-kaikei.ai` 投入
- [ ] `POSTMARK_FROM_REPLY=hello@zeimu-kaikei.ai` 投入
