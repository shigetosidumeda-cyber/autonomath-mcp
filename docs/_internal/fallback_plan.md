...
- DNS `autonomath.ai` A/AAAA → Fly Anycast (TTL = 300).
...
2. Flip apex A record to Pages:
   - Cloudflare dashboard → `autonomath.ai` → **DNS → Records**.
...
4. Post status on X / HN thread pointing at `autonomath.ai/status`.
