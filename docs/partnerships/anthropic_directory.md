...
4. Claude が jpcite MCP の 96 tools  + 30 autonomath at default gates) を自動選択して呼出
...
```
jpcite — 日本の制度情報 MCP サーバー (96 tools, protocol 2025-06-18)。
...
```
jpcite — MCP server (96 tools, protocol 2025-06-18) over Japanese public-program
...
JP:
```
日本の制度情報 (補助金 / 融資 / 税制 / 認定) を 96 ツールで横断検索。181 排他ルール、3 軸融資リスク、出典付き。¥3/req、3 req/日 free。
```

EN:
```
66-tool MCP over Japanese public programs (subsidy/loan/tax) with 181 exclusion rules, 3-axis loan risk, primary-source citations. ¥3/req; 50/mo free.
```
