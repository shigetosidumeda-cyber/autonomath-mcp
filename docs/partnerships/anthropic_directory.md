...
4. Claude が jpcite MCP の 155 tools  + 30 autonomath at default gates) を自動選択して呼出
...
JP:
```
日本の制度情報 (補助金 / 融資 / 税制 / 認定) を 155 ツールで横断検索。181 排他ルール、3 軸融資リスク、出典付き。¥3/billable unit、3 req/日 free。
```

EN:
```
155-tool MCP over Japanese public programs (subsidy/loan/tax) with 181 exclusion rules, 3-axis loan risk, source citations. ¥3/billable unit; 3 req/day free.
```
