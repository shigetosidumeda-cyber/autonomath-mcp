...
> **要約 (summary):** Money Forward Cloud (国内中堅企業中心、20 万社) の API integration として **AutonoMath REST + MCP** を組込。ME (経費精算 / 会計) ユーザーが自社の科目データを context に補助金 / 税制を発見。¥3/req metered。月次売上の 10% を MF に referral 還元。
...
- 配布: AutonoMath REST `/v1/programs/search` + MCP server (既存)
- MF 側統合点: **MF クラウド連携 API** (`https://expense-api.moneyforward.com/`) の partner 登録
- 認証: OAuth2 (MF) + AutonoMath API key (X-API-Key)
- データ授受: pass-through (MF 顧客データは AutonoMath DB に **保存しない**)
...
| 適格請求書 | T8010001213708 で AutonoMath が MF 宛に発行 |
...
法令: 電帳法対応 (法令第 7 条 真実性・可視性) を AutonoMath 自身でも遵守
データ: pass-through、AutonoMath は MF 顧客データを保存しない (privacy policy)
```
...
- MF の決算データを AutonoMath DB に保存しない
- 個別契約書 / 紙押印は永久 NG
