...
The 155 tools enumerate immediately in the tool picker. `/mcp` to verify connection, `/tools` to list, `/usage jpcite` to check this month's metered total.
...
agent = Agent(name="jp_subsidy", tools=mcp.list_tools())  # 155 tools immediately
```
