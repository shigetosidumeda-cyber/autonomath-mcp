# AutonoMath — Rollback Runbook (operator-only)
...
- `https://status.autonomath.ai/` (Cloudflare Worker、`site/status.html` 編集) に「監視中」のお知らせを 5 分以内に表示
...
site (autonomath.ai) 側で landing copy / docs に異常がある場合:
...
- `BASE_URL=https://api.autonomath.ai ./scripts/smoke_test.sh` PASS 確認
- `curl -s https://api.autonomath.ai/meta` で `build_sha` が **旧** SHA に戻ったことを確認
- `https://status.autonomath.ai/` に「障害復旧」を 10 分以内に表示
...
# 7. /meta 数値確認
curl -s https://api.autonomath.ai/meta | jq '.total_programs'
# → 13,578 程度 (前日比 ±1% 以内)
```
