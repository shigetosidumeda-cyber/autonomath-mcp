...
### 使う tool
- **MCP tool**: `check_invoice_status (revoke watch)`
- **REST endpoint**: `/v1/invoice/bulk_watch?event=revoke`
- **jpcite.com docs**: <https://jpcite.com/recipes/r30-invoice-revoke-watch/>

### expected output
- JSON: revoked[]/expired[]/name_changed[] + revoked_date + reason
...
- **429 Too Many Requests**: Idempotency-Key 必須、parent/child fan-out
- **5xx / timeout**: 60s wait、NTA upstream 障害時は invoice-kohyo.nta.go.jp
