...
- 研究開発税制 (措置法 §42-4) の知財関連加算は税理士領域、本 recipe は scaffold + 一次 URL まで
...
- 行政書士法 §1 — 申請書面作成は行政書士 / 認定支援機関、本 recipe は scaffold + 一次 URL まで
- 中小企業診断士登録規則 — 経営助言 / 事業計画 strategy は診断士領域
...
### 使う tool
- **MCP tool**: `search_programs (知財 fence)`
- **REST endpoint**: `/v1/programs/search?keyword=知財,特許,商標&tier=S,A`
- **jpcite.com docs**: <https://jpcite.com/recipes/r08-benrishi-ip-grant-monitor/>

### expected output
- JSON: programs[].name (特許費用補助 / 海外出願補助 等) + 期限 + 補助率
...
### 失敗時 recovery
- **404 Not Found**: JPO 個別事業 未 ingest — issue で報告
