...
**Path**: AutonoMath コア (¥3/req metered) を **不動産・建築・登記**
...
- AutonoMath は 2026-05-06 に農業 + SMB 制度 13,578 件で launch、
...
- **データ衛生**: `source_url` には一次情報 (国交省 / e-Gov / 法務省 /
  自治体 / 都市計画情報) のみ。アグリゲータ (suumo / homes / athome /
...
- アグリゲータ ban list を強化 (suumo / homes / athome / lifull /
...
| ツール名                              | 系統        | 概要                                                              |
|---------------------------------------|-------------|-------------------------------------------------------------------|
| `search_real_estate_programs`         | jpintel     | `real_estate_programs` 全文検索 (FTS5 + program_kind + law_basis filter) |
...
- `AUTONOMATH_REAL_ESTATE_ENABLED` env (default `False`) で gate。
  launch (2026-05-06) 時点では disabled、manifest は **72 tools** のまま。
- operator が `True` に flip すると 66 + 5 = **71 tools** に増えるので、
  T+150d 程度で partner 向け OpenAPI 事前公開・契約面の peer review が可能。
- `tests/test_real_estate_tools.py` が
  ① env-False で 72 tools / ② env-True で 71 tools / ③ sentinel envelope
...
4. **FTS5 trigram の単漢字偽陽性** (例: `地` で `団地・農地・林地・敷地`
