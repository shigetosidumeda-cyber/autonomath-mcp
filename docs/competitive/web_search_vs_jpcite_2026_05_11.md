...
|----|---------|------|------------------|
| **jpcite** | jpcite | Evidence prefetch (日本公的制度 DB + MCP) | REST `/v1/evidence/packets/query` + MCP 155 tools |
...
| AI agent ワークフローへの組込み (MCP / Custom GPT Actions / Cursor) | **jpcite (MCP 155 tools / OpenAPI Actions)** | first-class MCP server + agent-safe OpenAPI subset |
