...
programs 11,601 (S 114 / A 1,340 / B 4,186 / C 5,961) + 採択 2,286 + 融資 108 + 行政処分 1,185 + 法令 6,493 全文 + 9,484 catalog + 50 tax_rulesets + 通達 3,221 + 国税不服審判所 裁決 137 + 適格事業者 13,801 delta + 排他/前提ルール 181 + am_compat_matrix 43,966。MCP 139 tool 公開 (146 runtime)。
...
AI+jpcite = 同じ士業本人が Claude / Cursor / ChatGPT GPT を使うが、jpcite の MCP server (139 tool) を経由する。 jpcite tool は SQLite + 一次資料 URL のみで答えるため LLM 推論を一切経由しない、幻覚は構造的に発生し得ない。
...
| **Cline / Continue.dev** | 開発者向け検証 | MCP 互換 stdio | プロンプト変えながら 139 tool を素早く比較 |
...
### Q3. jpcite が幻覚率 0% は本当か?

jpcite は SQLite (programs 11,601 / 採択 2,286 / 融資 108 / 行政処分 1,185 / 法令 6,493 etc.) + 一次資料 URL のみ。 LLM 推論を一切経由しない (`tests/test_no_llm_in_production.py` で CI guard)。 構造的に幻覚は発生し得ない。 ただし「DB row の元データが古い」リスクはあり、これは `source_fetched_at` 列で透明化、月次 ETL で refresh。
