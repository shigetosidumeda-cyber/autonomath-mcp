...
v2 では **「AI 単体 (Claude / GPT 等を jpcite なしで使った場合) vs AI+jpcite (jpcite tool 経由)」** に比較軸を統一する。 これは AI agent 時代の正しい比較で、(a) 顧客がもう紙やマニュアル巡回には戻らない前提、(b) AI 単体だと公的制度情報の幻覚率が高く誤情報リスクが残る、(c) jpcite はその幻覚率を 0% に落とす一次資料 layer、という構造を数値化する。

> **重要な前提**: 下表の「AI 単体 baseline」は典型的な実務時間 (TKC 戦略経営者通信・日行連標準額・JICPA 監査報酬指針・中小機構 J-Net21 ヒアリング)・LLM ベンチマーク (公的制度 QA 幻覚率) からの **推定値**。 業務内容・LLM model・prompt 構造で増減する。 「絶対にこの数字になる」と保証するものではない、上限/下限 range とその構造的根拠を示す。
...
| 精度 (幻覚率 %) | 公的制度 QA で 15-40% (model + prompt 依存) | 0% (SQLite + 一次 URL のみ、LLM 推論なし) |
| 誤り対応コスト (¥/月) | 幻覚 1 件 × 信頼回復コスト × 月件数 | 0 (誤情報構造的に出ない) |
| 月コスト (¥/月) | LLM API 不要 (士業本人時間) | ¥3.30 × N req + 士業時間 |

**ROI 倍率** = (AI 単体 月時間コスト + 誤り対応コスト − AI+jpcite 月時間コスト − AI+jpcite ¥3.30 × N) ÷ (AI+jpcite ¥3.30 × N)
...
- `search_tax_incentives` / `get_am_tax_rule`: tax_rulesets 50 件から該当行を JSON で返す、改正前後の混同なし
- `apply_eligibility_chain_am`: 排他/前提ルール 181 を chain 評価、要件見落としなし
- `check_enforcement_am`: 行政処分 1,185 件を houjin_bangou で完全一致、捏造なし
- `bundle_application_kit`: 一次資料 URL を直接添付、捏造 URL なし

LLM 推論なし = 数字の幻覚も構造的に発生しない (programs 11,601 / 採択 2,286 / 融資 108 等の数値はすべて DB row から直接転記)。
...
ROI 倍率 (税理士事務所): ¥340,175 / ¥9,825 = **34.6 倍/月** = **約 35 倍 (月次 ROI)**

- 月額 jpcite コスト ¥9,825 (うち API ¥825 + 士業時間 ¥9,000)
- 月額 削減効果 ¥340,175
- 顧問先 100 社で年間 ¥4,082,100 の確保 (失敗回避 + 工数削減)
...
> - **AI+jpcite**: Claude が `get_am_tax_rule({"rule_id": "sochi_42_12_5"})` を呼ぶ、jpi_tax_rulesets に 20% の改正後値が直接返る、幻覚 0%。 月 100 req × ¥3.30 = ¥330。

**sensitivity 分析 (税理士)**

| 顧問先数 | AI 単体 月コスト | AI+jpcite 月コスト | ROI |
|---|---:|---:|---:|
| 30 社 | ¥105,000 | ¥3,300 | 31 倍 |
| 50 社 | ¥175,000 | ¥5,200 | 33 倍 |
| 100 社 | ¥350,000 | ¥9,825 | 35 倍 |
| 200 社 | ¥700,000 | ¥18,650 | 37 倍 |
...
ROI 倍率 (会計士事務所): ¥476,422 / ¥18,578 = **25.6 倍/月** = **約 26 倍 (月次 ROI)**
...
|---|---:|---:|---:|
| 5 社 | ¥247,500 | ¥10,289 | 23 倍 |
| 10 社 | ¥495,000 | ¥18,578 | 26 倍 |
| 20 社 | ¥990,000 | ¥35,156 | 28 倍 |
| 30 社 | ¥1,485,000 | ¥51,734 | 28 倍 |
...
ROI 倍率 (行政書士): ¥409,740 / ¥15,260 = **26.9 倍/月** = **約 27 倍 (月次 ROI)**
...
|---|---:|---:|---:|
| 10 件 | ¥170,000 | ¥7,100 | 23 倍 |
| 25 件 | ¥425,000 | ¥15,260 | 27 倍 |
| 50 件 | ¥850,000 | ¥29,500 | 28 倍 |
| 100 件 | ¥1,700,000 | ¥57,000 | 29 倍 |
...
ROI 倍率 (中小企業診断士): ¥496,839 / ¥14,161 = **35.1 倍/月** = **約 35 倍 (月次 ROI)**

- 中小企業診断士は最高 ROI cohort、月次 saved search の fan-out 構造が ¥/req 効率を最大化する
...
> - **AI+jpcite**: Claude が `find_complementary_programs_am()` + `apply_eligibility_chain_am()` を呼ぶ、排他/前提ルール 181 + am_compat_matrix 43,966 から chain 評価。 採択率は公表値のみ表示 (推測値出さない)、提案根拠が一次資料に張り付く。 ¥3.30 × 30 req = ¥99/社。

**sensitivity 分析 (中小企業診断士)**

| 顧問先数 | AI 単体 月コスト | AI+jpcite 月コスト | ROI |
|---|---:|---:|---:|
| 20 社 | ¥204,400 | ¥5,664 | 35 倍 |
| 50 社 | ¥511,000 | ¥14,161 | 35 倍 |
| 80 社 | ¥817,600 | ¥22,658 | 35 倍 |
| 100 社 | ¥1,022,000 | ¥28,322 | 35 倍 |

顧問先数に対してほぼ線形、ROI 倍率が安定して 35 倍前後を維持する (構造的に最適)。
...
ROI 倍率 (M&A advisor): ¥1,556,691 / ¥53,309 = **29.2 倍/月** = **約 29 倍 (月次 ROI)**

- 月額 削減効果 ¥155 万、年間 ¥1,870 万の確保
...
> - **AI+jpcite**: Cursor が `intel_houjin_full` + `cross_check_jurisdiction` + `check_enforcement_am` + `match_due_diligence_questions` を 1 ディールで 47 req 呼ぶ (mcp.json cost_examples と整合)、¥3.30 × 47 = ¥155/ディール。 houjin_bangou 完全一致、jurisdiction 3 軸 (法務局 / NTA / 採択) 不一致を自動検出、補助金返還義務期間を am_enforcement_detail から正確抽出。

**sensitivity 分析 (M&A advisor)**

| 月ディール数 | AI 単体 月コスト | AI+jpcite 月コスト | ROI |
|---|---:|---:|---:|
| 1 ディール | ¥536,667 | ¥17,770 | 29 倍 |
| 3 ディール | ¥1,610,000 | ¥53,309 | 29 倍 |
| 5 ディール | ¥2,683,333 | ¥88,848 | 29 倍 |
| 10 ディール | ¥5,366,667 | ¥177,697 | 29 倍 |
...
| 6.2 融資前 DD (3 軸保証) | 8h | 25% | ¥35,000 | ¥71,000 | 50 | ¥165 + 0.3h × ¥4.5k = ¥1,515 | ¥69,485 |
...
ROI 倍率 (信用金庫渉外): ¥314,841 / ¥6,659 = **47.3 倍/月** = **約 47 倍 (月次 ROI)**

- 信用金庫渉外は最高 ROI、時間単価が低い分 API ¥/req 効率が突出する
...
> - **AI+jpcite**: Claude が `houjin_watch` (mig 088) + `dispatch_webhooks.py` で 100 社の corp amendment + 採択 alert を Slack 即時配信。 ¥3.30 × 100 = ¥330/月 alert + 50 社 sweep 個別 DD × ¥3.30 = ¥165 = ¥495 API、検証工数 0.7h × ¥4,500 = ¥3,150。 3 軸 (担保 / 個人保証人 / 第三者保証人) で稟議書欄が直接埋まる。

**sensitivity 分析 (信用金庫渉外)**

| 取引先数 | AI 単体 月コスト | AI+jpcite 月コスト | ROI |
|---|---:|---:|---:|
| 30 社 | ¥96,450 | ¥2,000 | 47 倍 |
| 80 社 | ¥257,200 | ¥5,327 | 47 倍 |
| 100 社 | ¥321,500 | ¥6,659 | 47 倍 |
| 200 社 | ¥643,000 | ¥13,318 | 47 倍 |

信用金庫は時間単価 ¥4,500 が他業種比で最も低いため、jpcite API 効率の絶対値が突出する。 1 渉外 = 100 取引先標準で月 ¥6,659、年 ¥80,000 程度の投資で月 ¥314,841 削減。
...
これらは LLM model や prompt で改善はするが、構造的にゼロにはならない。 jpcite は SQLite + 一次 URL で構造的にゼロ。

### 2. ROI 倍率は 26-47 倍に集中する

業種別では信用金庫 (47 倍) > 税理士・診断士 (35 倍) > M&A (29 倍) > 行政書士 (27 倍) > 会計士 (26 倍) と並ぶ。 これは:
...
ROI 倍率の絶対値 26-47 倍は一般 SaaS の 3-8 倍を構造的に超える。 これは jpcite が「LLM 推論を持たず、士業の時間を直接 API ¥/req に置換する」 architecture の必然。
...
5 万円以下が大多数、年間でも ¥10-¥60 万。 これに対して削減効果月 ¥30 万-¥155 万 = ROI 2 桁倍は構造的に保たれる。
...
|---|---:|---:|---:|---:|
| 税理士事務所 | ¥350,000 | ¥9,825 | ¥340,175 | **35 倍** |
| 会計士事務所 (ピーク) | ¥495,000 | ¥18,578 | ¥476,422 | **26 倍** |
| 行政書士 | ¥425,000 | ¥15,260 | ¥409,740 | **27 倍** |
| 中小企業診断士 | ¥511,000 | ¥14,161 | ¥496,839 | **35 倍** |
| M&A advisor | ¥1,610,000 | ¥53,309 | ¥1,556,691 | **29 倍** |
| 信用金庫渉外 | ¥321,500 | ¥6,659 | ¥314,841 | **47 倍** |
...
LLM 単体で扱うと幻覚率 20-40% は不可避。 これは LLM 性能の問題ではなく、training data の reach と reasoning の構造の問題。 jpcite はこの「LLM が苦手な領域」だけを SQLite + 一次 URL で receive する layer。
...
LLM model がより強力になっても (b)+(c) は完全には消えない。 士業の責任ある業務である以上、verify を省略はできない。 jpcite は (b)+(c) を 1/3-1/5 に圧縮する (LLM 推論を経由しないため verify が citation 確認だけで済む)。

### 観察 3: 「リスク shield」と「月次 ROI」は別軸

旧版 (v1) は「ディール崩壊 1 件 ¥2,500 万」のような最悪事例回避 / API コストで計算 → ROI 2,688 倍のような数字。 これは「失敗を 1 回でも避ければ年単位で元が取れる」説明として正しい。

v2 (本文書) は「月次の工数削減 + 幻覚誤り対応削減 / 月額総コスト」で計算 → 26-47 倍。 これは「毎月構造的に発生する効果」の説明として正しい。

両方とも事実だが、目的が違う。 v1 = リスク shield 説明 (CFO / オーナー向け)、v2 = 月次構造的 ROI 説明 (現場の士業 / IT 管理 向け)。
...
- 削減しないのは「最終判断の責任」
- 士業の責任ある業務は構造的に jpcite では代行できない、これは設計、規制でなく構造

業法 fence があるからこそ士業は安心して jpcite を使える、これが長期的な ARR 安定の源泉。

---

## v1 (5/11 旧版) との対応

旧版の ROI 倍率 (税理士 67-222 / 会計士 312 / M&A 2,688 等) は **失敗 1 件あたりの最大ロス÷月額 API コスト** で計算されていた。 これは「最悪事例 1 件回避すれば API は元が取れる」という説明としては正しいが、月次の構造的 ROI ではない。

v2 (本文書) の 25-47 倍は「月次の工数削減 + 幻覚誤り対応削減 ÷ 月額総コスト (API + 士業時間)」で計算する。 構造的に毎月発生する効果、これが投資対効果の中央値。 v1 の数値はリスク shield (失敗回避) としての説明に retain。

両方とも有効。 v1 = リスク shield 説明、v2 = 月次 構造的 ROI 説明、目的別に使い分け。
...
これに対して jpcite は構造的に 0%、これは model upgrade で変わらない構造的優位。
...
3. **業法 fence**: scaffold + 一次 URL のみ、最終判断は有資格者 (税理士法 §52 / 弁護士法 §72 / 行政書士法 §1 / 司法書士法 §3 / 公認会計士法 §47条の2)。
4. **DPA / 営業 / CS 一切なし**、API キー 1 本で完結 (Solo + zero-touch ops)。
5. **誇大広告 fence**: 「100% 確実に削減」とは言わない、推定 range と構造的根拠で論じる。 景表法 §5 / 消費者契約法 §4 配慮。
...
旧版 by_industry_2026_05_11.md (ROI 67-2,688 倍) を v2 に張り替える場合は以下。

| 旧版表現 | v2 表現 | 理由 |
|---|---|---|
| "ROI 倍率 67-222 倍 (税理士)" | "月次 ROI 35 倍 (構造的) + リスク shield 67-222 倍 (失敗 1 件回避)" | 2 つを並べて記載 |
| "ROI 2,688 倍 (M&A 最高)" | "月次 ROI 29 倍 + ディール崩壊 ¥2,500 万回避時 825+ 倍" | 失敗単発の数字は分離 |
...
### Q1. 「ROI 35 倍」は誇大広告ではないか?

景表法 §5 不当表示の懸念に配慮し、本表のすべての数値は (a) 推定 baseline + 中央値、(b) sensitivity 分析で range 提示、(c) 「業務内容で増減」明示、を厳守。 「絶対に 35 倍になる」とは記載していない、「典型業務で 26-47 倍が観測される構造」と書く。

### Q2. AI 単体 baseline は本当にそんなに高いのか?

LLM 幻覚率 22-40% は公開された LLM bench (`tools/offline/aeo_citation_bench.py` 等) の典型値。 士業本人の verify 工数も TKC 戦略経営者通信ヒアリング (税理士業務時間)・日行連標準額 (行政書士)・JICPA 監査報酬指針 (会計士) からの推定。 過大に出る幻覚率はあるが、低めに出ることはない。

### Q3. jpcite は回答の誤りを完全に防げるのか?
...
両方とも本物。 ただし計算根拠と用途が違う:
- **リスク shield**: 失敗 1 件あたり最大ロス ÷ 月額 API。 CFO / オーナー向け説明、「保険として元が取れる」論。
- **月次 ROI**: 月次の工数削減 ÷ 月額総コスト。 現場の士業 / IT 管理向け、「毎月構造的に削減される」論。

v2 (本文書) は月次 ROI に焦点を当てる。 v1 (旧版) はリスク shield に焦点を当てた。 セールス pitch では用途で使い分ける。

### Q5. LLM model が upgrade されたら ROI 下がるのでは?

LLM 幻覚率は確実に下がるが、(a) training cutoff の構造的限界、(b) 改正速度との競合 (毎年税制改正 + 通達四半期更新)、(c) 業法 fence の存在で 15% 未満には構造的にならない。 これに対して jpcite は 0%。 model upgrade で ROI 倍率は 26-47 倍 → 20-35 倍に緩やかに下がるが、絶対値は依然 SaaS 一般の 3-8 倍を大きく超える。

### Q6. 既存 SaaS との比較は?

業界別 SaaS と比較した場合:
- TKC FX シリーズ (税理士向け 会計 SaaS): ¥5-10 万/月、ROI 5-8 倍
- 弥生会計 / freee 会計: ¥3-5 千/月、ROI 3-6 倍
- jGrants / ミラサポ plus (中小機構): 無料、ROI 計測不可 (検索網羅率 30-40%)
- 補助金ポータル系 aggregator (noukaweb / hojyokin-portal): 詐欺リスク (一次資料担保なし)、ROI 議論対象外
...
信用金庫渉外担当は (a) 銀行員 35 才 中央値で年収 ¥6 百万 + 福利厚生、(b) 月稼働 168h でなく実稼働 140h 程度、(c) 制度提案は 1 業務 component、で計算すると時間単価 ¥4,500 が妥当。 上位の経営支援部 / 営業推進部 は時間単価 ¥6,500-¥8,000 になり、その場合 ROI は 35-40 倍に張り付く (信金の最高 ROI 47 倍は最も低い時間単価で計算した下限)。

### Q10. なぜ士業時間を coast に含めるのか?

「士業の verify を自動化したい (士業を排除)」とは jpcite は主張しない。 業法 fence で士業の最終判断は必須、そのための verify 時間は構造的に発生する。 jpcite の役割は (a) 検索 + 一次 URL 提供を爆速化、(b) 幻覚を構造的にゼロ、 = verify 時間を 1/3-1/5 に圧縮する layer。 士業時間を coast に含めるのは士業の責任を可視化するため、これを抜くと AI 単体 vs AI+jpcite の delta が誤って大きく見える。

---

## MCP tool mapping (30 task × 推奨 tool)

各 task で推奨される jpcite tool。 LLM agent の prompt engineering の参考に。 すべての tool で response 末尾に `_disclaimer` field (業法 fence 表明) が付与される。
...
| 6.2 融資前 DD | `loan-programs/search` (3 軸保証) | `intel_houjin_full` | (信金内部稟議) |
...
- [ ] LLM model upgrade (Claude Opus 4.7 など) で AI 単体 baseline が下がっても、jpcite 投資は維持 (構造的に 0% 幻覚は不変)
