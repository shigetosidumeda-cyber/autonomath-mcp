...
| 100 顧問先 月次顧問売上 | ¥300-1,000 万/月 |
| jpcite 課金 / 月次顧問売上 | **0.003 - 0.011%** |
| 顧問契約 1 件あたりの jpcite 課金 | ¥3.30/月 (税込) |

## ROI 倍率 — 取りこぼし 1 件回避で何が起きるか
...
jpcite 年間課金 ¥3,960 に対し、顧問契約 1 件解除を 1 度回避するだけで **回収倍率 90 - 300 倍**。100 顧問先のうち 1 件でこの構造が成立すれば年間 ROI が確定する。
...
- **汎用 RAG (Perplexity 等)**: 自治体補助金・公庫融資・税制特例の網羅性が著しく薄い + 税理士法 §52 fence なし
...
- `previewCost` — 実行前見積もり
- `audit_seal` — 月次 audit-seal pack PDF + RSS (税理士 cohort 専用、migration 089)
...
100 顧問先月次レビュー、月額 ¥330 (顧問契約 1 件あたり ¥3.30/月)。顧問契約 1 件解除を年 1 回避けるだけで ROI 倍率 90-300 倍が確定する構造。anonymous 枠・web search・汎用 RAG では物理的・構造的に達成不能。
