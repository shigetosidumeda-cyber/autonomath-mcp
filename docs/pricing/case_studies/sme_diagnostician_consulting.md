...
DD-style の深掘り 1 件で 47 req。pricing.html の「申請準備パック: 受付案件 200 件を 3 call で一次整理」(600 units = ¥1,980) のような bulk triage ではなく、1 件で深く掘る構成。
...
- **業種別統計を取らず汎用的な診断を出した結果、顧客から「相場感覚が無い」と評価される** — 継続契約に影響する可能性

明示的な利用前提では、1 案件 47 req、税込 **¥155.10/件**。補助金・税制特例・採択後 monitoring・DD 質問デッキを同じ手順で揃え、診断士本人が最終判断する前の根拠集約コストとして扱う。
...
- 「経営診断書を出さないなら何のための tool か?」 → 経営診断書のドラフトを作るための **根拠集約 (Evidence Packet)** + 質問デッキ + 業種コホート統計を提供する。診断書そのものは診断士本人が書く。作業時間は案件・確認範囲・利用する AI / 士業レビュー体制に依存し、jpcite は根拠集約と質問デッキ作成の前処理を短くするための材料を返す
- 「47 req は多いのでは?」 → 業種統計 (3-5 req) + 採択事例照合 (5-10 req) + 税制特例適用可否 (5-8 req) + DD 質問デッキ (1-3 req) + 行政処分照合 (2-3 req) + 排他ルール (3-5 req) + Evidence Packet 引照 (8-12 req) で 1 案件 47 req は典型的な分布
