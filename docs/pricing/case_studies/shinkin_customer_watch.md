...
| 項目 | 値 |
|---|---|
| 信用金庫 渉外 1 顧客 LTV (融資・為替・投資信託の総合、業界実勢) | 数百万円 / 顧客 |
| 営業訪問 1 回の自社コスト (人件費 + 移動 + 機会費用) | ¥10,000-30,000/回 |
| 渉外担当 1 人の月給 | ¥30-60 万/月 (賞与・福利込みで実質コスト ¥50-100 万/月) |
| jpcite 課金 / 渉外 1 顧客 LTV | **0.001% 未満 (LTV 数百万円ベース)** |

## ROI 倍率 — 取りこぼし 1 件で何が起きるか

信用金庫渉外で「公開情報の見落とし」が起こす典型損害:

- **取引先の行政処分発覚を遅らせて与信判断を誤る** — 不良債権化・回収不能による直接損失 (数百万円 - 数千万円)
- **取引先が新設補助金で設備投資資金を満たし他行へ流出** — 顧客解約 (LTV 数百万円のロス + 信用金庫の地域シェア低下)
...
¥39.60/月/顧客 を払うことで、年間 ¥475.20/顧客。顧客 1 件の不良債権化 (数百万円規模) を 1 度回避するだけで、100 顧客 100 年分の課金が回収される計算。

ROI 倍率は **数千倍 - 数万倍 (運用ベース)**。不良債権化回避ベースだと **桁外れに大きい**。
...
- **汎用 RAG**: 商号変遷 / 役員変遷 / 適格事業者番号 / 補助金交付決定取消 の curated DB は jpcite 固有。houjin_watch + webhook によるリアルタイム検出も汎用 RAG では構造的に不可
...
- 信用金庫 cohort (cohort #7、CLAUDE.md) として organic 経路で住み着く設計
...
信用金庫渉外 1 顧客 watch、月額 ¥39.60 (税込)。年間 ¥475.20/顧客、LTV 数百万円比で 0.001% 未満。不良債権 1 件回避で課金 100 年分回収、運用ベースでも数千倍-数万倍の ROI。信用情報法 fence により与信判断領域は犯さず、渉外担当の事前 awareness を高める補助線として組み込み安全。信用金庫 cohort (cohort #7) は organic + zero-touch 設計で住み着く想定。
