# AutonoMath — Go / No-Go Gate (operator-only)
...
- [ ] PASS / [ ] FAIL
- ≪`autonomath-mcp` 起動 → MCP `tools/list` で 72 tool 出力 (38 jpintel + 28 autonomath)≫
- 数値 drift 0 (`mcp-tools.md` / `pyproject.toml` / `server.json` 全部 55)
- 観察値: __
...
### 17. docs.autonomath.ai/compliance/* (critical)
...
- 確認: `curl -s https://autonomath.ai/docs/compliance/ | grep -c "href=\"./"` で >= 6

### 18. tokushoho (critical)

- [ ] PASS / [ ] FAIL
- ≪`https://autonomath.ai/tokushoho.html` (or `/legal/tokushoho`) で特商法 8 必須項目 + 法人番号 T8010001213708 表示≫
