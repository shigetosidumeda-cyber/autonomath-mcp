...
> 編集方針: ¥0/req tier や paid-plan ラベル、広告、フェーズ提案、工数の話、旧 brand (jpintel/AutonoMath/税務会計AI/zeimu-kaikei.ai) を本文に出さない。
...
- **配信面**: MCP 151 tools (default gate, protocol 2025-06-18) + REST 302 paths (full OpenAPI, `openapi.agent.json` は 34 paths agent-safe subset)。
- **業法 fence**: 8 canonical (税理士法§52 / 弁護士法§72 / 公認会計士法§47条の2 / 行政書士法§1の2・§19 / 司法書士法§3 / 社会保険労務士法§27 / 弁理士法§75 / 労働基準法§36)。税務・法律・申請・監査などの sensitive surface では `_disclaimer` / fence note を返し、agent dev が独占業務へ踏み込みにくい設計にできます。
...
- **header**: `X-API-Key` (必須) / `X-Client-Tag: <顧問先 ID>` (任意・原価集計) / `Idempotency-Key: <uuid>` (POST 再送安全) / `X-Cost-Cap-JPY: <整数>` (batch 上限) → agent dev は user / project 単位で利用料差分を計算しやすい。
- **金額フィールド caveat**: `amount_max_jpy` / `amount_max_man_yen` は正規化された参考値です。`null` / `0` は未収録・未指定を含み、ゼロ給付や採択可能性を意味しません。金額を user に出す前に `source_url`、`source_fetched_at`、`known_gaps`、対象類型・補助率・上限枠を確認してください。
...
| 出典 verifiable | URL 出るが fetched_at 不明、リンク切れ多発 | 対応レコードでは `source_url` + `source_fetched_at` を返し、`/v1/citations/verify` で到達性確認可 |
| 業法侵犯 risk | 「節税対策はこう」と GPT が断定 → 税理士法§52 抵触 | sensitive surface では fence note / `_disclaimer` を同梱し、user に専門家相談を案内しやすい |

### ¥3/req の経済性

- 1 query = 約 1 unit (¥3.30 inc-tax)。GPT の monthly ARPU が ¥1,000 の有料 GPT なら、303 query/月までは jpcite 利用料を単純計算で吸収できます。外部 LLM 側の token / search 料金は workload-dependent です。
- 1 GPT user が月 10 query なら ¥33 inc-tax / 月、100 query で ¥330 inc-tax / 月。Custom GPT 単発の有料化テンプレ (note / Boothで ¥980 売り) でも 30 user で月 ¥970 の jpcite 利用料です。販売価格、利用頻度、解約率、外部 LLM 料金、サポート工数を含む事業収支は提供者側で別途確認してください。
...
3. Authentication → API Key → Custom header name: X-API-Key → jc_xxxxx (発行は https://jpcite.com/pricing.html#api-paid)
```

### first call output サンプル (response shape)
  ...
  "results": [
    {
      ...
      "tier": "S",
      "amount_max_jpy": null,
      "amount_caveat": "金額条件は枠・類型で分岐するため、このサンプルでは上限額を表示しません。source_url の公募要領で確認してください。",
...
Claude Project で「税理士 / 会計士 / M&A」向けの業界特化 MCP server を自分で書くなら、まず公的制度コーパスは jpcite に bundle して、自分の server は「業界固有の差別化レイヤ」だけ書け。151 tool が `uvx autonomath-mcp` 1 行で 30 秒解禁、protocol 2025-06-18、Claude Desktop / Cursor / Cline と互換。`evidence/packets/query` / `prescreen_programs` / `check_exclusions` / `search_invoice_registrants` / `search_laws` / `search_court_decisions` を再実装する工数が消える。
...
| 出典 verifiable | source URL なし、Claude が「公式サイトより」と曖昧引用 | 対応レコードでは `source_url` + `source_fetched_at` + `corpus_snapshot_id` を返し、`/v1/citations/verify` で到達性 / 本文一致確認に使える |
...
- 顧問先 100 社月次 = 18 × 100 = 1,800 unit / ¥6,000 inc-tax / 月。MCP server 自体を ¥9,800/月 で提供する場合の jpcite 利用料差分は約 ¥3,800 です。実際の収支は外部 LLM 料金、決済手数料、サポート工数に依存します。
...
3. 動作確認: claude mcp list | grep jpcite  (Connected, 151 tools)
```
...
Cursor / Codex で「日本企業向け SaaS の AI 機能」を書くと、「補助金 / 法令 / 適格請求書 / 行政処分」の確認が必要になる場合が多いです。`.cursor/mcp.json` に jpcite を bundle すると、Cursor agent が `dd_profile_am` 1 call で法人番号 → 採択履歴 + invoice 登録 + enforcement + 関連法令を束ねた public DD packet を返します。`pack_construction` / `pack_manufacturing` / `pack_real_estate` の業種別 1-call wrapper は、条件が合う場合に複数 call を evidence packet にまとめ、入力文脈量の削減に寄与します。
...
- 受付前エビデンス整理 1,000 案件 triage = 16,000 unit / ¥52,800 inc-tax。1 案件あたり ¥52.8 で公的 DD layer が組める。
- agent dev の SaaS が ¥980/月 / user の subscription で、user 1 人月平均 30 query なら ¥99 jpcite コスト / user です。外部 LLM 料金、決済手数料、サポート工数を除いた単純な料金差分は ¥881 です。
...
2. curl -o .cursor/mcp.json https://jpcite.com/.cursor/mcp.example.json
   (内容: { "mcpServers": { "jpcite": { "command": "uvx",
     "args": ["autonomath-mcp"], "env": { "JPCITE_API_KEY": "jc_xxxxx" } } } })
3. Cursor 再起動 → Settings → MCP → jpcite が緑 dot で表示、tools count 151
```
...
## Persona 4 — Perplexity Spaces / Anthropic API direct integrator (独自 retrieval + LLM 実装)

### 30 秒 elevator pitch

Perplexity Spaces や Anthropic API + 独自 retrieval で日本市場向けプロダクトを作るなら、retrieval 層を全部自前で集める場合は、収集・更新・検証コストが大きくなりやすい。jpcite REST 302 paths (`https://api.jpcite.com/v1/*`) を Perplexity Spaces source / 独自 retrieval の前段に置けば、token 効率の良い evidence packet (median 566 tokens) が `compression.cost_savings_estimate.break_even_met` 付きで返ってくる。`/v1/evidence/packets/batch` (最大 100 lookup) で massive parallel fetch、`/v1/audit/proof/{evidence_packet_id}` で Merkle 監査チェーン (税理士・監査法人向け差別化)。

### bundle 後の agent 品質差 (before / after)

| 軸 | before (自前 scraping + embedding) | after (jpcite REST/MCP を evidence 前段) |
|---|---|---|
...
| 業法侵犯 risk | 長文 chunk が「節税スキーム」「契約書 template」を直接返す設計 → 業法抵触 | sensitive surface の `_disclaimer` envelope と evidence-to-expert handoff rule (`GET /v1/advisors/match`) で agent 側が責任分担しやすい |

### ¥3/req の経済性

- Perplexity Spaces / Anthropic API は token / search で課金される。jpcite は 1 query = 約 1 unit (¥3.30 inc-tax) で、自前 retrieval の embedding コスト + chunk token + LLM token を短い evidence packet に置き換えられる場合がある。
- M&A 1 社 public DD = 47 unit / ¥155.10 inc-tax。Perplexity Spaces で ¥9,800/月 subscription の M&A Space を提供する場合、user 1 人月 5 社 DD = ¥775.50 jpcite 利用料 / user です。外部 LLM 料金、決済手数料、サポート工数を除いた単純な料金差分は約 ¥9,024 です。
- `/v1/evidence/packets/batch` で 100 lookup → 1 HTTP req、Idempotency-Key で再送安全 → orchestrator 側の retry loop が単純化。
- `/v1/audit/proof/{evidence_packet_id}` は、対応する evidence packet について再確認用メタデータを確認する入口です。監査・DD では一次資料と専門家レビューを併用します。
...
Headers: X-API-Key: jc_xxxxx, X-Client-Tag: client_abc
...
| 出典 verifiable | SaaS dashboard で「補助金一覧」を出すが source URL なし、顧客が再確認できない | 対応レコードでは `source_url` + `source_fetched_at` を返し、SaaS 顧客が一次資料確認へ戻りやすい |
...
- SaaS が ¥30,000/月 の business plan で「補助金 AI tab」を有料化する場合、user 1 人月 300 query = ¥990 jpcite コスト / user です。外部 LLM 料金、決済手数料、サポート工数を除いた単純な料金差分は ¥29,010 です。
...
- `audit_seal` (migration 089) + `/v1/audit/proof/{evidence_packet_id}` で、対応する evidence packet の再確認用メタデータを SaaS 機能として扱えます。監査証跡としての利用可否は顧客の業務要件に依存します。
...
2. SaaS UI の「補助金 AI tab」「適格事業者確認」「監査 DD pack」から
   curl -H "X-API-Key: jc_xxxxx" \
        -H "X-Client-Tag: customer_id_${SaaS_user_id}" \
        "https://api.jpcite.com/v1/programs/search?q=..."
3. SaaS の billing dashboard で X-Client-Tag 集計 → 顧客別原価 / 再販時の利用料差分を可視化
```
...
Headers: X-API-Key: jc_xxxxx, X-Client-Tag: customer_id_42
→
{
  "total": 8,
  "limit": 3,
...
- **151 MCP tools (default gate) + 302 REST paths**: MCP も REST も同 corpus に hit、Custom GPT 30-path 上限向けに `openapi.agent.gpt30.json` slim も別途配信 (34 paths は agent-safe subset)。
