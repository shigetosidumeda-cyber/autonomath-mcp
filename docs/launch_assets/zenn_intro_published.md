...
description: "jpcite ローンチ — 14,472 件の補助金・融資・税制・認定を REST + MCP 184 ツールで横断検索。¥3/billable unit 完全従量、3 req/日 per IP 無料。"
tags:
  - api
  - mcp
  - llm
...
**184 ツール**  + 30 autonomath = 17 V1 + 4 メタデータ tools + 7 静的データセット tools、protocol 2025-06-18) が即座に Claude Desktop / Cursor / ChatGPT から呼べます。
...
1. **AI agent 開発者** — Claude / Cursor / ChatGPT の Manifest 1 行で 184 ツール、SDK 不要
...
2. **MCP プロトコル 2025-06-18 の 184 ツール schema validation**
...
aggregator (noukaweb / hojyokin-portal 等) は source_url の参照対象から外しています。過去の業界事例が 詐欺 risk を生んだ反省で、一次資料引用を優先しています。
