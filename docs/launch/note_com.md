...
**タイトル候補**:
- `日本の制度データ 11,601 件を AI エージェントから叩ける API を作った話 (¥3/billable unit)`
...
日本の制度公開情報 — 補助金 11,601 件 / 法令 9,484 件 / 判例 2,065 件 / 適格事業者 13,801 件 / 税制 50 件 / 採択事例 2,286 件 / 行政処分 1,185 件 — を REST API + MCP サーバーから検索できるサービスです。

主要な公開レコードに一次資料 URL (各省庁・都道府県・日本政策金融公庫・国税庁) が付いており、noukaweb / hojyokin-portal 等の集約サイトは `source_url` の参照対象から外しています。
...
- 集約サイトが SEO を独占しているが、データは数ヶ月遅延 + 互いに引用し合っているだけで、一次資料を辿れない
...
**¥3/billable unit 完全従量課金** (税込 ¥3.30)。

- 匿名 3 req/日 無料 (IP ベース、JST 翌日 00:00 リセット、サインアップ不要)
- API キー利用時も ¥3/billable unit のみ。月額固定 / 年間最低額 / シート課金はすべてありません
- カードでいつでも開始 / 解除
...
- 集約サイトを source_url の参照対象から除外 — `source_url` は省庁・都道府県・公庫・国税庁などの一次資料を優先
- URL 死亡監視 — 毎晩 source_url を巡回し、404 を検索対象外バケット (二次レビュー待ち) に隔離 (現在 2,871 件)
