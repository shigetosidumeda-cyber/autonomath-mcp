...
**タイトル候補**:
- `日本の制度データ 10,790 件を AI エージェントから叩ける API を作った話 (¥3/req)`
- `1 年かけて作った税務・補助金検索 API を本日公開しました — 税務会計AI`

**ハッシュタグ**: `#API` `#AI` `#補助金` `#MCP` `#個人開発`

**カバー画像**: zeimu-kaikei.ai のスクリーンショット または ロゴ

---

## 本文

本日、税務会計AI (autonomath-mcp) を公開しました。

日本の制度公開情報 — 補助金 10,790 件 / 法令 9,484 件 / 判例 2,065 件 / 適格事業者 13,801 件 / 税制 35 件 / 採択事例 2,286 件 / 行政処分 1,185 件 — を REST API + MCP サーバーから検索できるサービスです。
...
これだけで、東京都の農業関連補助金が JSON で返ってきます。匿名で月 50 req まで無料、サインアップ不要。
...
- 匿名 50 req/月 無料 (IP ベース、JST 月初 00:00 リセット、サインアップ不要)
...
- URL 死亡監視 — 毎晩 source_url を巡回し、404 を公開保留バケット (二次レビュー待ち) に隔離 (現在 1,923 件)
...
- サイト: https://zeimu-kaikei.ai
- 料金: https://zeimu-kaikei.ai/docs/pricing/
