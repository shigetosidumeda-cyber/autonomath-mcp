...
- 中小企業庁・経済産業省の補助金公募情報 (整形後 306 paths)
- 国税庁の通達・FAQ
...
第二に、税務・法律・申請・監査などの sensitive surface では、disclaimer header や `_disclaimer` field により、公開情報の機械整形であり最終判断は有資格者が行うべきことを示す。SDK / MCP server でも同じ境界表示を継承する設計として扱う。
