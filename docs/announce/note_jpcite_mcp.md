...
jpcite はこの「公開情報の最新 snapshot を AI に正確に渡す」一点を解決する API です。データ自体は政府が無料で出している公開情報なので、私たちが提供する価値は (a) 7 サイトの normalize、(b) `source_url + fetched_at + content_hash` の出典 Evidence 必須化、(c) 法人番号での横断 join、(d) Stripe ¥3/req metered billing、の 4 つに絞っています。

## 何ができるか — 5 商品

1. **会社フォルダ作成パック** — 法人番号 1 つで baseline / 採択履歴 / 行政処分 / インボイス登録 / 関連法令を 18 req (¥59.40 税込) で 1 PDF artifact に。M&A 前 DD / 新規取引 KYC / 内部監査用途で士業の 2-3 時間が 30 秒に。
2. **顧問先月次レビュー** — 100 社 × 月次 = ¥330/月。インボイス取消 / 行政処分新着 / 採択結果 monitor を月初 1 batch で fan-out。会計事務所の月次定例準備工数が消える。
3. **BPO 一次受付パック** — 1000 案件 triage = ¥52,800/月。顧客問合せの法人名から baseline + 業種 + 規模を 3 req で fetch、担当者振分を自動化。
4. **M&A 公開情報 DD** — 1 社 = ¥155.10 (47 req)。法人 6 源泉 + 行政処分 5 年遡及 + 適格事業者推移 + 採択補助金履歴 + 関連法令 + 役員兼任を 1 通の DD report に。
5. **相談前プレ診断** — 50 件 = ¥1,320 (400 req)。士業の初回相談前に相談者の法人状態を 8 req で先回り fetch。面談時間が「ヒアリング 50% → アドバイス 80%」に shift。
...
これだけで Claude Desktop / Claude Code の tool picker に jpcite の 139 tool が即時 enumerate されます。`/mcp` で接続状況、`/tools` で全 tool 一覧。
...
jpcite は「公開情報の Evidence handoff」までで止まる設計です。AI が個別案件の税務・法律助言を出力しないよう、以下 7 業法に対応するフェンスを実装しています。

- 税理士法 §52 (税務代理・税務書類作成の独占業務)
- 弁護士法 §72 (個別法律相談・代理の独占業務)
- 司法書士法 §73 (登記申請書類作成・代理の独占業務)
- 行政書士法 §19 (申請書類作成・提出代行の独占業務)
- 社会保険労務士法 §27 (労務書類作成・社会保険申請代行の独占業務)
- 中小企業診断士 登録規則 (個別経営診断書発行・認定支援機関業務代行)
- 弁理士法 §75 (特許・商標出願代理・鑑定の独占業務)

AI 出力の最後に必ず「本情報は公開情報の集約であり、個別案件は有資格者にご相談ください」の disclaimer が自動付与され、個別の税額計算 / 法的判断 / 適合性判定は出力しないようガードレールを敷いています。

## 価格 — ¥3/req、無料 3 req/IP/日
