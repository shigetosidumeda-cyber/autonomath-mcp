...
`uvx autonomath-mcp` の 1 行で Claude Code に統合できる MCP server を公開しました。日本の公的制度 11,601+ 補助金 / 9,484+ 法令メタ / 1,185+ 行政処分 / 13,801+ 適格事業者 / 22,258+ 行政処分詳細を、法人番号 1 つで横断照会できます。151 tool、`source_url + fetched_at + content_hash` 必須の Evidence Packet 設計、業法フェンス 7 種 (税理士法 §52 等) で個別税務・法律助言は出力しません。¥3/req 完全従量、無料 3 req/IP/日 (API key 不要)。Stripe Checkout 1 分で metered billing 開始、月額固定費ゼロ、解約は dashboard 1 click。Bookyou株式会社 (T8010001213708) 提供。
