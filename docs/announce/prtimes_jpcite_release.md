...
日本の公的制度情報 (補助金・許認可・行政処分・適格事業者・法令) は 7+ の政府サイトに分散しており、士業や中小企業の担当者は毎月の制度 monitor / 顧問先別 fit 判定 / DD 用 evidence 収集を手作業で巡回しています。AI に聞いても学習データのみで答える場合があり、出典 URL や最新性は別途確認が必要です。

jpcite は「公開情報の snapshot を AI に根拠付きで渡す」一点に絞った API です。データは政府が無料公開している情報を normalize し、`source_url + fetched_at + content_hash` を付与することで、AI の出力を再検証しやすくしています。DB/source freshness と caller 側 review は引き続き必要です。

## jpcite の特徴 5 つ

1. **法人番号 1 つで 6 源泉を横断照会** — 法人公開 baseline + 採択履歴 + 行政処分 + インボイス登録 + 許認可 + 関連法令を 1 API で取得
2. **5 商品の Artifact 出力** — 会社フォルダ / 顧問先月次レビュー / 受付前エビデンス整理 / M&A DD / 相談前プレ診断
3. **Evidence Packet** — `source_url + fetched_at + content_hash` を payload に同梱し、AI 出力の検証可能性を確保
4. **8 業法 fence** — 税理士法 §52 / 弁護士法 §72 / 公認会計士法 §47-2 / 行政書士法 §1の2 / 司法書士法 §3 / 社会保険労務士法 §27 / 弁理士法 §75 / 労働基準法 §36 の sensitive surface では、個別判断に踏み込まない設計
5. **¥3/billable unit 完全従量、月額固定費なし** — Stripe metered billing、無料 3 req/IP/日 (API key 不要)
...
- **受付前エビデンス整理** — 1000 案件 triage = ¥52,800/月。顧客問合せの法人名から baseline + 業種 + 規模を 3 req で fetch
- **士業の相談前プレ診断** — 50 件 = ¥1,320 (400 req)。初回相談前に相談者の法人状態を 8 req で先回り fetch する前提例。面談前の確認材料として使えます。
