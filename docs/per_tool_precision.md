...
**Source**: 79-query gold suite maintained with the repository tests. Every `expected_ids` list was generated by calling the live tool against the current jpcite dataset. CI re-runs the suite on every PR. Drift surfaces as a non-zero exit code.
...
Detailed precision gates per query are maintained in the gold suite, with each row carrying its own `min_precision_at_10` and a note explaining the regression being guarded. Selected highlights:
...
| `it_001_donyu_subsidy` | 7 + search-excluded rows | 0.7 | **Exclusion test** — `UNI-158e0bb965` (検索対象外) must NEVER appear given `tier=[S,A,B]`. |
...
- **answer() 統合 tool** — not in the canonical 151-tool count yet; gold suite excludes it.
