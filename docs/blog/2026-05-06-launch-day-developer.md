...
description: "jpcite ローンチ記事 (Article 1 of 2) — 検索対象制度 11,601 件、採択事例 2,286 件、融資詳細 108 件、排他ルール 181 本を 1 本の REST API で提供する設計背景。"
tags:
  - api
  - python
  - fastapi
...
集約サイトは source_url の参照対象から外しています。
...
FastAPI (REST /v1/*)
 └── SQLite 全文検索 (3-gram 分割)
      ...
      ├── laws             (9,484 メタデータ + 6,493 本文インデックス e-Gov CC-BY)
...
### 3. source_url は重要フィールド

主要な公開行に `source_url` と `fetched_at` が付いています。
...
2026-04-24 以降の拡張では法令 (e-Gov CC-BY, メタデータ 9,484 件 + 本文インデックス 6,493 件) と税務ルールセット (インボイス+電帳法, 50 件)
...
- 11,601 件の検索対象制度を `curl` 1 本で確認できる
