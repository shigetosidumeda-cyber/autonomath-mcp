...
> jpcite は公開情報を source-linked evidence/output として通知する API です。Webhook payload は税務助言・法律相談・申請代行ではありません。

## 概要

| 項目 | 値 |
|---|---|
| 登録時の認証 | `X-API-Key: jc_...` |
| 配信先 | `https://` の URL のみ |
...
| event_type | 推奨処理 |
|---|---|
| `program.created` | 保存検索・顧問先 profile と照合し、必要なら Evidence Packet を取得する |
| `program.amended` | `diffs` と `source_url` を確認し、既存の案内文・社内メモの更新要否を見る |
| `enforcement.added` | 関連法人・業種・地域に該当するかを確認し、DD pack に追加する |
| `tax_ruleset.amended` | 顧問先メモや税務チェックリストの根拠リンクを更新する。個別税務判断は資格者側で行う |
| `invoice_registrant.matched` | インボイス登録状態の照合結果として扱い、取引先台帳や会社フォルダに反映する |
