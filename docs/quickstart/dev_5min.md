...
`tools/list` should return 139 tools by default (146 with all post-manifest tools landed; verify with `len(await mcp.list_tools())`).
...
- MCP server @ `stdio` or `http://localhost:8081/mcp` (139 tools).
