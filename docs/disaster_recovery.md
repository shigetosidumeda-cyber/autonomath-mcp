# jpcite — Disaster Recovery Plan v2
...
- **Runbook**:
  ...
  4. Re-smoke: `BASE_URL=https://api.jpcite.com ./scripts/smoke_test.sh`.
...
- **Detection**: UptimeRobot fail on `https://jpcite.com/`. API may stay up via `api.jpcite.com` direct.
- **Runbook**:
  1. Confirm Cloudflare status page.
  2. If Cloudflare-wide: nothing to do — recovery is upstream.
  3. If only Pages: switch DNS for `jpcite.com` to a Cloudflare Worker that serves a cached `index.html` from KV.
  4. Post a pinned status note (no ETA) on X.
- **Blast radius**: Marketing + docs site only. API customers (using `api.jpcite.com` directly) unaffected.
...
- **Runbook**:
  ...
  4. DNS flip: Cloudflare dashboard → `api.jpcite.com` A record → fra IP. TTL 60 s.
