...
jpcite運営事業者 (以下「当社」) は、個人情報の保護に関する法律 (APPI) および関連法令を遵守し、利用者の個人情報を適切に取り扱います。本ポリシーは、当社が提供する jpcite サービス (以下「本サービス」) における個人情報の取り扱い方針を定めるものです。

本サービスは API / MCP を主用途とし、marketing cookie は使用していません。決済完了確認、CSRF 防止、ダッシュボード表示など、セキュリティおよびセルフサービス運用に必要な最小限のブラウザ保存領域を使用します。
...
- browser cookie / localStorage / session storage (決済状態確認、CSRF 防止、API キーの一時表示・任意保存に限定)
...
`/v1/stats/usage` API 経路を通じて、以下の集計統計を公開します。**個別利用者を識別できない形式に限定**します:

- 期間別総 request 数
- API 経路 別 request 数分布
- error class 別発生件数
- 平均レスポンス時間

最小集計単位: k-anonymity で k>=5 を確保。個別事業者が 5 未満となる統計項目は公開しません。
...
- 保護水準: 米国は APPI の「同等水準国」指定はないが、各委託先は GDPR 対応の DPA を公開し、ISO 27001 / SOC 2 Type II を更新しています。
...
本サービスは marketing cookie を使用しません。決済状態確認や CSRF 防止など、サービス提供に必要な最小限の cookie を発行する場合があります。

- 本ドキュメントを公開する静的サイト (jpcite.com) にも analytics tool (Google Analytics 等) を設置しない方針です。
- 将来 analytics を導入する場合は、本ポリシーを改訂し、明示的な opt-in を更新します。
...
- 2026-04-24: 初版策定 (jpcite launch 時)
