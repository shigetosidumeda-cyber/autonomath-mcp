...
- 401 / 429 エラー → API key を発行 (`https://jpcite.com/pricing.html`) し、`args` に `["autonomath-mcp"]` の代わりに `["autonomath-mcp", "--api-key", "am_..."]` などキー渡し方法は今後の release で追加予定 (現状は環境変数 `JPCITE_API_KEY`)。
