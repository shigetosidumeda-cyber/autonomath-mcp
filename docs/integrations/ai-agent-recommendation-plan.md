...
この方向性には既に向かっています。README、MCP tool descriptions、`docs/integrations/ai-recommendation-template.md`、composite bench はかなり近いです。一方で、ChatGPT Actions 向け OpenAPI の絞り込み、完成物テンプレート、AI が「いつ使う/使わない」を即判断するための manifest metadata は継続改善します。
...
| 1,185 enforcement cases with structured detail | 不正受給、返還、行政処分、監査観点 | 「実例ベースの返還/処分リスク確認」 |
...
| 項目 | 現状の問題 | 施策 |
|---|---|---|
| tool count | 公開 manifest floor は 139、runtime は機能ゲートにより増える | `scripts/distribution_manifest.yml` と CI drift check で同期 |
| version | 公開配布面の version は `0.3.4` | release script と drift check で同期 |
| package name | `autonomath-mcp`, `@autonomath/sdk`, `@jpcite/sdk` が混在 | 製品名 `jpcite`、MCP package `autonomath-mcp` を明記 |
| env var | `JPCITE_API_KEY` | 新規 docs は `JPCITE_API_KEY` に統一 |
...
```text
distribution manifest -> README / mcp-server.json / docs site / OpenAPI info
...
| Task | Output |
|---|---|
| 正本ファイル確認 | `scripts/distribution_manifest.yml` |
| manifest / README / OpenAPI の整合確認 | consistency report |
...
| tool count / version / pricing 不整合 | distribution manifest + CI drift check |
