...
匿名 IP 制限は discoverability 系 (`/meta`, `/v1/ping`, `/v1/programs/*`, `/v1/exclusions/*`, `/v1/feedback`) にのみ適用。`/healthz`, `/v1/subscribers/unsubscribe`, dashboard 系 (`/v1/me/*`, `/v1/session`) はカウントしない。
...
**認証:** 任意 (未認証は匿名扱い)。API key 付きの paid batch は二重課金防止のため `Idempotency-Key` header が必須。
...
**Billing / cap:** batch は ID 件数分を billable units として扱う。API key 付きの paid batch は `Idempotency-Key` がない場合 `428` で止め、リトライによる二重課金を防ぐ。実行前に月次 cap を projected units で検査し、cap 超過が見込まれる場合は処理前に 503 で止める。
