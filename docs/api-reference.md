...
`include_compression=true` は入力文脈の比較用メタデータを返すための指定です。`source_token_count` と `input_token_price_jpy_per_1m` は呼び出し側が置いた baseline で、外部 LLM の請求額を保証しません。
...
**¥3/billable unit の対価 (コスト削減を保証するものではなく、データ統合の対価):** ¥3/billable unit は LLM token 削減効果を保証するものではありません。token 使用量への影響は workload-dependent です。実際の対価は次の 3 点:
