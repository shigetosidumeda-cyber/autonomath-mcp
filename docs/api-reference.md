<script type="application/ld+json">
{
  ...
  "@type": "APIReference",
  "headline": "AutonoMath REST API Reference (112 endpoints)",
  "description": "AutonoMath REST API の全 112 endpoint 仕様。programs / exclusions / meta / billing / dashboard / laws / loans / court-decisions / bids / 行政処分 / 採択事例 / invoice 適格事業者 / tax_rulesets / V1+V4+Phase A 拡張を含む。",
  ...
  "mainEntityOfPage": {
    "@type": "WebPage",
    "@id": "https://autonomath.ai/docs/api-reference/"
  }
}
</script>
...
> **要約 (summary):** REST API の全 112 endpoint 仕様。programs / exclusions / meta / billing / dashboard / 各種データセット (laws / loans / court-decisions / bids / 行政処分 / 採択事例 / invoice 適格事業者 / tax_rulesets) / AutonoMath V1+V4+Phase A 拡張 / 内部ユーティリティ (alerts / advisors / widget / privacy) を含む。各 endpoint について params, response, 認証要否, rate limit, curl 例を記載。最上部の [目次](#目次-endpoint-catalogue) で全体一覧。
...
排他ルール全件を返す (現在 181 件: hand-seeded 35 + 要綱 PDF 一次資料 auto-extracted 146)。
...
- `total_programs` は `programs` 全件 (tier S/A/B/C 10,790 + tier X 隔離 1,923 = 13,578)
- `tier_counts` は tier 別件数。**tier X は隔離 (quarantine) なので search では常に除外**
...
```bash
curl -H "X-API-Key: am_..." https://api.autonomath.ai/v1/ping
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/programs/prescreen" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X GET "https://api.autonomath.ai/readyz"
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/meta/freshness?limit=50" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/billing/refund_request" \
  -H "Content-Type: application/json" \
...
```bash
curl -X GET "https://api.autonomath.ai/v1/me" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/me/billing-portal" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/me/billing_history" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/me/cap" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
  -d '{"monthly_cap_yen": 0}'
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/me/dashboard?days=30" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/me/rotate-key" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/me/tool_recommendation" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/me/usage?days=30" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/me/usage_by_tool?days=30" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/session" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
  -d '{"api_key": "string"}'
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/session/logout" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/usage" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/me/alerts/subscriptions" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X DELETE "https://api.autonomath.ai/v1/me/alerts/subscriptions/<sub_id>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/device/authorize" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "https://api.autonomath.ai/v1/device/complete" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "https://api.autonomath.ai/v1/device/token" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X GET "https://api.autonomath.ai/v1/email/unsubscribe" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/email/unsubscribe" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/email/webhook"
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/subscribers" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X GET "https://api.autonomath.ai/v1/subscribers/unsubscribe" \
  -H "X-API-Key: am_..."
```
...
```json
{
  "cancel_url": "https://autonomath.ai/alerts.html?status=canceled",
  "subscriber_id": 0,
  "success_url": "https://autonomath.ai/alerts.html?status=ok"
}
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/compliance/stripe-checkout" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
  -d '{"cancel_url": "https://autonomath.ai/alerts.html?status=canceled", "subscriber_id": 0, "success_url": "https://autonomath.ai/alerts.html?status=ok"}'
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/compliance/stripe-webhook"
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/compliance/subscribe" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "https://api.autonomath.ai/v1/compliance/unsubscribe/<unsubscribe_token>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/compliance/verify/<verification_token>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/stats/confidence" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/stats/coverage" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/stats/data_quality" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/stats/freshness" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/stats/usage" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/me/testimonials" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE "https://api.autonomath.ai/v1/me/testimonials/<testimonial_id>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/testimonials" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/advisors/match" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/advisors/report-conversion" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "https://api.autonomath.ai/v1/advisors/signup" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "https://api.autonomath.ai/v1/advisors/track" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "https://api.autonomath.ai/v1/advisors/verify-houjin/<advisor_id>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/advisors/<advisor_id>/dashboard-data" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/widget/enum_values" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/widget/search" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/widget/signup" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "https://api.autonomath.ai/v1/widget/stripe-webhook"
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/widget/<key_id>/usage" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/privacy/deletion_request" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "https://api.autonomath.ai/v1/privacy/disclosure_request" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X GET "https://api.autonomath.ai/v1/calendar/deadlines?within_days=30" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/case-studies/search" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/case-studies/<case_id>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/loan-programs/search" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/loan-programs/<loan_id>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/enforcement-cases/search" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/enforcement-cases/<case_id>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/invoice_registrants/search" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/invoice_registrants/<invoice_registration_number>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/laws/search" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/laws/<unified_id>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/laws/<unified_id>/related-programs" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/court-decisions/by-statute" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X GET "https://api.autonomath.ai/v1/court-decisions/search" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/court-decisions/<unified_id>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/tax_rulesets/evaluate" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
```bash
curl -X GET "https://api.autonomath.ai/v1/tax_rulesets/search" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/tax_rulesets/<unified_id>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/bids/search" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/bids/<unified_id>" \
  -H "X-API-Key: am_..."
```

---

## AutonoMath: Programs (Active / Related / Stats / GX)
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/acceptance_stats" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/active_at" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/gx_programs?theme=ghg_reduction" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/open_programs" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/programs/active_v2" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/related/<program_id>?relation_types=['string']" \
  -H "X-API-Key: am_..."
```

---

## AutonoMath: Intent / Reason / Enums
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/enums/<enum_name>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/intent" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/reason" \
  -H "X-API-Key: am_..."
```

---

## AutonoMath: Tax Incentives & Certifications
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/certifications" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/tax_incentives" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/tax_rule" \
  -H "X-API-Key: am_..."
```

---

## AutonoMath: Loans & Mutual Insurance
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/loans?loan_kind=ippan" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/mutual_plans?plan_kind=retirement_mutual" \
  -H "X-API-Key: am_..."
```

---

## AutonoMath: Laws & Enforcement
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/by_law" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/enforcement" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/law_article" \
  -H "X-API-Key: am_..."
```

---

## AutonoMath: Annotations / Validation / Provenance
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/annotations/<entity_id>?kinds=['string']" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/provenance/fact/<fact_id>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/provenance/<entity_id>?include_facts=false" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X POST "https://api.autonomath.ai/v1/am/validate" \
  -H "X-API-Key: am_..." \
  -H "Content-Type: application/json" \
...
## AutonoMath: Static Resources & Example Profiles
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/example_profiles" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/example_profiles/<profile_id>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/static" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/static/<resource_id>" \
  -H "X-API-Key: am_..."
```
...
```bash
curl -X GET "https://api.autonomath.ai/v1/am/health/deep?force=false" \
  -H "X-API-Key: am_..."
```
