...
RE_YEAR = re.compile(r"(?:19|20)\d{2}\s*年?" r"|(?:令和|平成|昭和|大正|明治)\s*\d{1,2}\s*年")
...
RE_LAW = re.compile(
    r"([一-鿿]{2,12}(?:法|令|規則|基本法|措置法|促進法|特例法))" r"(?:第\s*\d+\s*条(?:の\d+)?)?"
)

# Program: things ending with 補助金 / 助成金 / 給付金 / 交付金 / 支援事業 / 融資 / 制度
RE_PROGRAM = re.compile(
    r"([一-鿿゠-ヿｦ-ﾟaA-Za-z0-9\s・]{2,40}?" r"(?:補助金|助成金|給付金|交付金|支援事業|融資|制度))"
)
