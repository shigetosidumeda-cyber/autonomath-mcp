...
FIX_DOC_URLS: dict[str, str] = {
    ...
    "appi_disabled_or_turnstile_secret_confirmed":
        "docs/_internal/PRODUCTION_DEPLOY_OPERATOR_ACK_DRAFT_2026-05-07.md#AppI",
    ...
    "pre_deploy_verify_clean":
        "scripts/ops/pre_deploy_verify.py --json failures[] -> release_readiness fix",
}
...
def verify_fly_app_confirmed() -> VerifyResult:
    ...
    if rc != 0:
        return VerifyResult(name, False, f"flyctl rc={rc}: {err.strip()[:200]}",
                            {"rc": rc, "stderr": err})
    try:
        payload = json.loads(out)
    except json.JSONDecodeError as exc:
        return VerifyResult(name, False, f"flyctl JSON parse fail: {exc}",
                            {"stdout_head": out[:200]})
...
def verify_appi_or_turnstile(repo_root: pathlib.Path) -> VerifyResult:
    ...
    if os.environ.get("JPINTEL_APPI_DISABLED") == "1":
        return VerifyResult(name, True, "JPINTEL_APPI_DISABLED=1 in env",
                            {"path": "env_disabled"})
    if _which("flyctl") is None:
        return VerifyResult(name, False, "flyctl missing and APPI not env-disabled",
                            {"reason": "no_flyctl"})
    rc, out, err = _run_capture(["flyctl", "secrets", "list", "-a", APP_NAME, "-j"])
    if rc != 0:
        return VerifyResult(name, False, f"flyctl rc={rc}: {err.strip()[:200]}",
    ...
    try:
        ...
        return VerifyResult(name, False, "flyctl JSON parse fail",
                            {"stdout_head": out[:200]})
...
def verify_target_db_packet(repo_root: pathlib.Path) -> VerifyResult:
    """boolean 4: DEEP-52 migration_inventory.py --target-db-only --json exit 0."""
    name = "target_db_packet_reviewed"
    script = repo_root / "scripts" / "ops" / "migration_inventory.py"
    if not script.is_file():
        # DEEP-52 wiring may live in a sibling draft dir; do best-effort presence check.
        sibling = repo_root.parent / "verify_migration_targets" / "verify_migration_targets.py"
        if sibling.is_file():
            script = sibling
        else:
            return VerifyResult(name, False,
                                f"migration_inventory.py not found at {script}",
                                {"path": str(script)})
    rc, out, err = _run_capture(
        [sys.executable, str(script), "--target-db-only", "--json"]
    )
    if rc != 0:
        return VerifyResult(name, False, f"rc={rc}: {err.strip()[:200]}",
                            {"rc": rc, "stderr": err})
    try:
        payload = json.loads(out) if out.strip() else {}
    except json.JSONDecodeError:
        payload = {"stdout_head": out[:200]}
    return VerifyResult(name, True, "migration_inventory exit 0", payload)
...
def verify_rollback_packet(repo_root: pathlib.Path) -> VerifyResult:
    ...
    if not mig_dir.is_dir():
        return VerifyResult(name, False, f"missing dir: {mig_dir}",
                            {"path": str(mig_dir)})
...
def verify_live_gbiz_ingest(repo_root: pathlib.Path) -> VerifyResult:
    ...
    if os.environ.get("GBIZINFO_INGEST_APPROVED") == "1":
        return VerifyResult(name, True, "GBIZINFO_INGEST_APPROVED=1 in env",
                            {"path": "env_approved"})
    if _which("flyctl") is None:
        return VerifyResult(name, False, "flyctl missing and not env-approved",
                            {"reason": "no_flyctl"})
    rc, out, err = _run_capture(["flyctl", "secrets", "list", "-a", APP_NAME, "-j"])
    if rc != 0:
        return VerifyResult(name, False, f"flyctl rc={rc}: {err.strip()[:200]}",
...
def verify_dirty_lanes(repo_root: pathlib.Path) -> tuple[VerifyResult, dict[str, Any] | None]:
    """boolean 7: DEEP-56 compute_dirty_fingerprint.py --json -> fingerprint object."""
    ...
    if script is None:
        return (
            VerifyResult(name, False, "compute_dirty_fingerprint.py not found",
                         {"searched": [str(c) for c in candidates]}),
            None,
        )
    rc, out, err = _run_capture([sys.executable, str(script), "--json"])
    if rc != 0:
        return (
            VerifyResult(name, False, f"rc={rc}: {err.strip()[:200]}",
                         {"rc": rc, "stderr": err}),
            None,
        )
    try:
        ...
        return (
            VerifyResult(name, False, "fingerprint JSON parse fail",
                         {"stdout_head": out[:200]}),
            None,
        )
    return (
        VerifyResult(name, True, f"fingerprint keys={sorted(fingerprint)[:5]}",
...
def verify_pre_deploy(repo_root: pathlib.Path) -> VerifyResult:
    """boolean 8: pre_deploy_verify.py --json -> {"ok": true}."""
    name = "pre_deploy_verify_clean"
    script = repo_root / "scripts" / "ops" / "pre_deploy_verify.py"
    if not script.is_file():
        return VerifyResult(name, False, f"missing script: {script}",
                            {"path": str(script)})
    rc, out, err = _run_capture([sys.executable, str(script), "--json"])
    if rc != 0:
        return VerifyResult(name, False, f"rc={rc}: {err.strip()[:200]}",
    ...
    try:
        ...
        return VerifyResult(name, False, "pre_deploy JSON parse fail",
                            {"stdout_head": out[:200]})
...
def _prompt_signoff(name: str, *, auto_yes: bool) -> bool:
    ...
    if auto_yes:
        return True
    try:
        ans = input(f"  signoff? [y/N/skip]: ").strip().lower()
    except EOFError:
        return False
    return ans == "y"
...
def _generate_ack_yaml(
    ...
    *,
    ...
    meta_for_hash = {
        k: v for k, v in payload["_meta"].items() if k != "yaml_sha256"
    }
    body_no_sha += yaml.safe_dump({"_meta": meta_for_hash}, sort_keys=True,
                                  allow_unicode=True)
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    mode.add_argument("--all", action="store_true", default=True,
                      help="Run all 8 booleans (default).")
    mode.add_argument("--boolean", type=int, choices=range(1, 9),
                      help="Verify a single boolean (1-8) and exit.")
    p.add_argument("--commit", action="store_true",
                   help="Actually write the ACK YAML (default DRY_RUN).")
    p.add_argument("--dry-run", action="store_true",
                   help="Force DRY_RUN even if --commit was implied (default).")
    p.add_argument("--json", action="store_true",
                   help="Emit verify results as JSON to stdout.")
    p.add_argument("--ack-out", type=pathlib.Path, default=None,
    ...
    p.add_argument("--operator-email", default=os.environ.get("OPERATOR_EMAIL",
                                                              DEFAULT_OPERATOR_EMAIL),
                   help="Operator email (default: env OPERATOR_EMAIL or info@bookyou.net).")
    p.add_argument("--yes", action="store_true",
                   help="Auto-confirm signoff prompts (test/CI only).")
...
def main(argv: list[str] | None = None) -> int:
    ...
    for idx, name in enumerate(BOOLEAN_NAMES, start=1):
        ...
        if not result.passed:
            ...
            if args.json:
                print(json.dumps({"results": json_results, "ack_emitted": False},
                                 indent=2, ensure_ascii=False))
            return 1
