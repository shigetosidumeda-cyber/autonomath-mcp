# autonomath-freee-plugin

A **stateless glue layer** that lets a freee アプリストア plugin pull AutonoMath
...
```
                     ...
                     └────────────┬───────────────┬───────────────────┘
                                  │               │
                       freee OAuth2  AutonoMath API key
                       access_token   (caller-managed)
                          ...
                          └───┬──────────────────┬────────┘
                              │                  │
                              ▼                  ▼
                    api.freee.co.jp       api.autonomath.jp
                    (company / 経理 API)    (¥3/req metered)
```

## なぜこの形か (design rationale)

- **AutonoMath は API/MCP only**: freee の UI に乗せるのが正解、自社 UI は持たない。
- **¥3/req fully metered**: glue 自体は OSS (MIT) で配布 free。課金は AutonoMath
  API 側で 1 req = ¥3 の従量で発生する。
- **Solo + zero-touch**: freee OAuth token と AutonoMath API key は **plugin
  実装者が管理** する。我々 (Bookyou) は触れない / 預からない。
- **No Anthropic API call**: glue は推論しない。プロンプトを Anthropic に投げる
  のは plugin 側の Claude / 顧客の LLM。
...
results = recommend(
    ...
    autonomath_api_key=os.environ["AUTONOMATH_API_KEY"],   # caller-managed
    limit=5,
)
for r in results:
...
5. ストア掲載情報:
   - 製品名: `AutonoMath 補助金レコメンド` 等 (任意。"freee" 商標は logo OK 取得後のみ)
   - カテゴリ: 「業務効率化 / AI アシスタント」
   - 料金: glue は無料。AutonoMath API 利用料 ¥3/req は別契約と明記。
...
1. https://autonomath.jp/ から API key を発行 (匿名 50 req/月 free, それ以降 ¥3/req)。
2. plugin の secret manager に `AUTONOMATH_API_KEY` として登録。
3. 上記の `recommend(...)` を呼ぶ。リトライ / レート制限は AutonoMath 公式 SDK
   (`pip install autonomath-client`) を使う方が便利だが、本 glue は依存最小の
   ため httpx を直接呼んでいる。
...
- 商用 redistribute / fork 自由 (MIT)。我々の moat は AutonoMath API 課金。
...
mock 経路のみ (実 freee / 実 AutonoMath は叩かない)。
