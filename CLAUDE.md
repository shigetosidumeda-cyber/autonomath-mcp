...
- **`entrypoint.sh` §2 boot gate is SIZE-BASED, not SHA-based (2026-05-11 root fix).** `autonomath.db` is mutated in place by cron ETL + boot-time self-heal migrations, so a baked-image `AUTONOMATH_DB_SHA256` drifts from the live volume DB on every run. The legacy logic hashed the 9 GB volume DB on every boot, detected the inevitable drift, and forced a full R2 re-download — on a flaky link this looped indefinitely and took prod down for 30+ min. The new gate accepts any existing `/data/autonomath.db` ≥ 5 GB as authoritative without hashing or downloading; `PRAGMA integrity_check` in §4 remains the structural correctness probe. **Knobs**: `AUTONOMATH_DB_MIN_PRODUCTION_BYTES` (default 5000000000 ≈ 5 GB) tunes the threshold; `BOOT_ENFORCE_DB_SHA=1` restores the legacy SHA256-verification path for DR drills / snapshot-restore (still gated on `AUTONOMATH_DB_SHA256`). Downloaded R2 snapshots remain SHA256-verified inside `bootstrap_autonomath_db_snapshot()` because that hash IS meaningful (it gates the freshly-fetched blob, not the long-lived volume). **Do NOT** re-hardcode the image SHA or remove the size-gate to "be safer" — sha256summing a cron-mutated DB is a false signal and a known foot-gun.
