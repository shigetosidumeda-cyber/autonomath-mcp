...
src/jpintel_mcp/
  ...
  mcp/      FastMCP stdio server (89 tools at default gates, protocol 2025-06-18: 39 core + 50 autonomath [V1 + 4 V4 universal + Phase A + lifecycle/abstract/prerequisite/graph_traverse/rule_engine + related_programs + 5 Wave 21 composition tools + 5 Wave 22 composition tools + NTA corpus 4 tools + 3 Wave 23 industry pack wrappers (pack_construction / pack_manufacturing / pack_real_estate)], gated by AUTONOMATH_ENABLED. 3 broken tools gated off pending fix: query_at_snapshot, intent_of, reason_answer)
...
- `src/jpintel_mcp/mcp/server.py` — FastMCP entry point, 89 tools total at default gates (3 broken tools gated off pending fix — see Architecture above):
